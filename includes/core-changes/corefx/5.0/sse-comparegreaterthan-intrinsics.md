...
#### <a name="change-description"></a><span data-ttu-id="31a99-103">Descripción del cambio</span><span class="sxs-lookup"><span data-stu-id="31a99-103">Change description</span></span>

<span data-ttu-id="31a99-104">Antes, las entradas de `NaN` a los métodos <xref:System.Runtime.Intrinsics.X86.Sse> y <xref:System.Runtime.Intrinsics.X86.Sse2> enumerados devolvían un resultado incorrecto.</span><span class="sxs-lookup"><span data-stu-id="31a99-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="31a99-105">El resultado también era distinto del resultado generado por el método correspondiente en la clase <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="31a99-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="31a99-106">A partir de .NET 5.0, estos métodos controlan correctamente entradas de `NaN` y devuelven los mismos resultados que los métodos correspondientes de la clase <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="31a99-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="31a99-107">Las arquitecturas estándar de la industria SSE (Extensiones de SIMD de streaming) y SSE2 (Extensiones de SIMD de streaming 2) no proporcionan soporte de hardware directo para estos métodos de comparación, por lo que se implementan en el software.</span><span class="sxs-lookup"><span data-stu-id="31a99-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="31a99-108">Antes, los métodos se implementaban incorrectamente y las entradas de `NaN` se administraban incorrectamente.</span><span class="sxs-lookup"><span data-stu-id="31a99-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="31a99-109">En el caso del código trasladado desde el código nativo, el comportamiento incorrecto puede presentar errores.</span><span class="sxs-lookup"><span data-stu-id="31a99-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="31a99-110">En el caso de una ruta de acceso de código de 256 bits, los métodos también pueden producir resultados diferentes en los métodos equivalentes de la clase <xref:System.Runtime.Intrinsics.X86.Avx>.</span><span class="sxs-lookup"><span data-stu-id="31a99-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="31a99-111">Para ver un ejemplo de cómo los métodos eran incorrectos antes, puede implementar `CompareNotGreaterThan(x,y)` como `CompareLessThanOrEqual(x,y)` para enteros normales.</span><span class="sxs-lookup"><span data-stu-id="31a99-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="31a99-112">Pero para las entradas de `NaN`, esa lógica calcula un resultado incorrecto.</span><span class="sxs-lookup"><span data-stu-id="31a99-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="31a99-113">En su lugar, al usar `CompareNotLessThan(y,x)`, se comparan los números correctamente *y* se tienen en cuenta las entradas de `NaN`.</span><span class="sxs-lookup"><span data-stu-id="31a99-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="31a99-114">Versión introducida</span><span class="sxs-lookup"><span data-stu-id="31a99-114">Version introduced</span></span>

<span data-ttu-id="31a99-115">5.0 (versión preliminar 4)</span><span class="sxs-lookup"><span data-stu-id="31a99-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="31a99-116">Acción recomendada</span><span class="sxs-lookup"><span data-stu-id="31a99-116">Recommended action</span></span>

- <span data-ttu-id="31a99-117">Si el comportamiento anterior era un error, no es necesario hacer cambios.</span><span class="sxs-lookup"><span data-stu-id="31a99-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="31a99-118">Si quiere que se produzca el comportamiento anterior, puede mantener ese comportamiento cambiando la invocación pertinente, como se indica aquí:</span><span class="sxs-lookup"><span data-stu-id="31a99-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="31a99-119">Categoría</span><span class="sxs-lookup"><span data-stu-id="31a99-119">Category</span></span>

<span data-ttu-id="31a99-120">Bibliotecas de Core .NET</span><span class="sxs-lookup"><span data-stu-id="31a99-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="31a99-121">API afectadas</span><span class="sxs-lookup"><span data-stu-id="31a99-121">Affected APIs</span></span>
