---
title: Into (Cláusula, Visual Basic)
ms.date: 07/20/2015
helpviewer_keywords:
- Into keyword [Visual Basic]
ms.assetid: e1534550-8f4b-4978-b043-fad7b8fd79ac
ms.openlocfilehash: 84da5f2919192cfff60079075b4114bd1b21a9ca
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 05/04/2018
ms.locfileid: "33598497"
---
# <a name="into-clause-visual-basic"></a>Into (Cláusula, Visual Basic)
Identifica las funciones de agregado o agrupaciones para aplicar a una colección.  
  
## <a name="remarks"></a>Comentarios  
 El `Each` se utiliza la palabra clave en los contextos siguientes:  
  
 [Aggregate (cláusula)](../../../visual-basic/language-reference/queries/aggregate-clause.md)  
  
 [Group By (cláusula)](../../../visual-basic/language-reference/queries/group-by-clause.md)  
  
 [Group Join (cláusula)](../../../visual-basic/language-reference/queries/group-join-clause.md)  
  
## <a name="see-also"></a>Vea también  
 [Palabras clave](../../../visual-basic/language-reference/keywords/index.md)
