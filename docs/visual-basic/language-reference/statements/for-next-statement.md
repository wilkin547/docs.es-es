...
|`Next`|Requerido. Finaliza la definición del bucle `For`.|
...
Se usa un `For`... `Next` estructura cuando desee repetir un conjunto de instrucciones un número de veces.
...
> [!TIP]
> [While... End while instrucción](../../../visual-basic/language-reference/statements/while-end-while-statement.md) o [do... La instrucción de bucle](../../../visual-basic/language-reference/statements/do-loop-statement.md) funciona bien cuando no se sabe de antemano cuántas veces ejecutar las instrucciones en el bucle. Sin embargo, cuando espera ejecutar el bucle un número determinado de veces, una `For`... `Next` bucle es una mejor opción. El número de iteraciones se determina cuando se escribe el bucle por primera vez.

## <a name="nesting-loops"></a>Anidar bucles

Puede anidar `For` bucles colocando un bucle dentro de otro. En el ejemplo siguiente se muestra `For` anidadas... `Next` estructuras que tienen valores de pasos diferentes. El bucle exterior crea una cadena para cada iteración del bucle. El bucle interno reduce una variable de contador de bucle para cada iteración del bucle.
...
Puede incluir cualquier número de instrucciones `Exit For` en un `For`... `Next` realizar. Cuando se usa dentro de `For` anidadas... `Next` bucles, `Exit For` sale del bucle más interno y transfiere el control al siguiente nivel superior de anidamiento.
...
- Un `Try`... `Catch`... `Finally` instrucción detecta una excepción. Puede usar `Exit For` al final del bloque de `Finally`.
...
|¿Está `datatype` presente?|¿@No__t_0 ya está definido?|Resultado (si `counter` define una nueva variable local cuyo ámbito es el bucle de `For...Next` completo)|
|----------------------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------|
...
- @No__t_0 de tipo que tiene los operadores siguientes, donde `B` es un tipo que se puede utilizar en una expresión `Boolean`.
