---
title: Tipo de &lt;typename&gt; no es compatible con CLS | Documentos de Microsoft
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
ms.suite: 
...
Una variable, propiedad o valor devuelto de función se declara con un tipo de datos que no es compatible con CLS.  
  
 Para una aplicación sea compatible con la [independencia del lenguaje y componentes independientes del lenguaje](https://msdn.microsoft.com/library/12a7a7h3) (CLS), debe utilizar sólo tipos conformes a CLS.  
...
-   Si su aplicación necesita ser compatible con CLS, cambie el tipo de datos de este elemento al tipo conforme a CLS más próximo. Por ejemplo, en lugar de `UInteger` , quizá pueda usar `Integer` si no necesita que el intervalo de valores esté por encima de 2.147.483.647. Si necesita el intervalo extendido, puede reemplazar `UInteger` por `Long`.  
  
-   Si su aplicación no necesita ser compatible con CLS, no es necesario cambiar nada. Sin embargo debe ser consciente de que no.  
