...
## <a name="example"></a>Ejemplo  
 Las expresiones lambda subyacen a muchos de los operadores de consulta de [!INCLUDE[vbteclinqext](~/includes/vbteclinqext-md.md)] y se pueden usar explícitamente en las consultas basadas en métodos. En el ejemplo siguiente se muestra una consulta [!INCLUDE[vbteclinq](~/includes/vbteclinq-md.md)] típica, seguida de la conversión de la consulta en formato de método.  
