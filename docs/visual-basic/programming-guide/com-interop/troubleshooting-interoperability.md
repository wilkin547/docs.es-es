...
## <a name="overloaded-methods"></a>Métodos sobrecargados  
 Aunque puede crear varios métodos sobrecargados con [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)], no son compatibles con COM. Cuando una clase que contiene métodos sobrecargados se expone como un objeto COM, se generan nombres de método nuevos para los métodos sobrecargados.  
