...
## Option Infer  
 ...
 Si no especifica un valor para `Option Infer` en el código, el valor predeterminado del compilador es `Option Infer On`.  Para los proyectos actualizados de [!INCLUDE[vb_orcas_long](../../../../visual-basic/misc/includes/vb_orcas_long_md.md)] o versiones anteriores, el valor predeterminado del compilador es `Option Infer Off`.  
