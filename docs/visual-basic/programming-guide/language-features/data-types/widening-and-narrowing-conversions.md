...
Una consideración importante con la conversión de tipos es si el resultado de la conversión está dentro del intervalo del tipo de datos de destino.  
 ...
 Una *conversión de restricción* cambia un valor a un tipo de datos que no pueda mantener algunos de los valores posibles. Por ejemplo, un valor fraccionario se redondea cuando se convierte en un tipo entero y un tipo numérico al `Boolean` que se convierte se reduce `True` a o `False`.  
...
|[Nothing](../../../../visual-basic/language-reference/nothing.md)|Cualquier tipo de datos o tipo de objeto.|  
 ...
 <sup>2</sup> las conversiones de `Integer`, `UInteger`, `Long`, `ULong` `Single` o ao`Double` pueden dar lugar a una pérdida de precisión, pero nunca en pérdida de magnitud. `Decimal` En este sentido, no incurre en la pérdida de información.  
