...
El compilador de [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] lleva a cabo un proceso denominado `binding` cuando se asigna un objeto a una variable de objeto.  Un objeto es de *enlace en tiempo de compilación* cuando se asigna a una variable que se declara de un tipo de objeto específico.  Los objetos de enlace en tiempo de compilación permiten al compilador asignar memoria y realizar otras optimizaciones antes de que se ejecute la aplicación.  Por ejemplo, en el siguiente fragmento de código se declara que una variable es de tipo <xref:System.IO.FileStream>:  
