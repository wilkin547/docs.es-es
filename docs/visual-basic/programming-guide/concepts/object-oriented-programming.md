---
title: "Programación orientada a objetos (Visual Basic) | Documentos de Microsoft"
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
Visual Basic proporciona compatibilidad completa para la programación orientada a objetos, incluidos la encapsulación, herencia y polimorfismo.  
 ...
 *Polimorfismo* significa que puede tener múltiples clases que se pueden usar indistintamente, aunque cada clase implementa las mismas propiedades o métodos de maneras diferentes.  
  
 En esta sección se describen los conceptos siguientes:  
  
-   [Clases y objetos](#Classes)  
  
    -   [Miembros de clase](#Members)  
  
         [Los campos y propiedades](#Properties)  
    ...
    -   [Instancias de clases](#InstantiatingClasses)  
  
    -   [Clases compartidas y miembros](#Static)  
...
##  <a name="Classes"></a>Clases y objetos  
 Los términos *clase* y *objeto* se utilizan a veces indistintamente pero, en realidad, las clases describen el *tipo* de objetos, mientras que los objetos son utilizables *instancias* de clases. Por lo tanto, la acción de crear un objeto se denomina *creación de instancias de*. Con la analogía de plano, una clase es un plano y un objeto es un edificio construido a partir de ese plano.  
...
```  
 ...
 Visual Basic también proporciona una versión ligera de clases denominadas *estructuras* que son útiles cuando necesite crear una matriz grande de objetos y desea usar demasiada memoria para que.  
...
###  <a name="Members"></a>Miembros de clase  
 Cada clase puede tener distintos *miembros de la clase* que incluyan las propiedades que describen los datos de clase, métodos que definen el comportamiento de la clase y eventos que proporcionan comunicación entre distintos objetos y clases.  
  
####  <a name="Properties"></a>Los campos y propiedades  
...
```  
 ...
 Visual Basic permite crear un campo privado para almacenar el valor de propiedad o bien usar las denominadas propiedades de implementación automática que crean este campo en segundo plano automáticamente y proporcionan la lógica básica para los procedimientos de propiedad.  
...
```  
 ...
 Una clase puede tener varias implementaciones o *sobrecargas*, del mismo método que se diferencian en el número de parámetros o tipos de parámetro.  
...
####  <a name="Destructors"></a>Destructores  
 ...
 Para obtener más información acerca de los destructores y recopilación de elementos no utilizados en .NET Framework, vea [recolección](../../../standard/garbagecollection/index.md).  
  
####  <a name="Events"></a>Eventos  
 Cuando ocurre algo interesante, los eventos habilitan una clase u objeto para notificarlo a otras clases u objetos. La clase que envía (o genera) el evento se denomina la *publisher* y las clases que reciben (o controlarán) el evento se denominan *suscriptores*. Para obtener más información acerca de los eventos, cómo se generan y controlan, vea [eventos](http://msdn.microsoft.com/library/b6f65241-e0ad-4590-a99f-200ce741bb1f).  
...
-   Para especificar controladores de eventos de forma declarativa, use la [WithEvents](../../../visual-basic/language-reference/modifiers/withevents.md) instrucción y el [controla](../../../visual-basic/language-reference/statements/handles-clause.md) cláusula.  
...
####  <a name="NestedClasses"></a>Clases anidadas  
 Una clase definida dentro de otra clase se denomina *anidada*. De forma predeterminada, una clase anidada es privada.  
...
###  <a name="AccessModifiers"></a>Modificadores y niveles de acceso  
 Todas las clases y miembros de clase pueden especificar a qué nivel de acceso que proporcionan a otras clases mediante *los modificadores de acceso*.  
...
###  <a name="InstantiatingClasses"></a>Instancias de clases  
...
```  
 ...
 Módulos compartidos en Visual Basic han compartido a solo miembros y no se pueden crear instancias. Miembros compartidos también no pueden tener acceso a propiedades no compartidos, los campos o métodos  
...
##  <a name="Inheritance"></a>Herencia  
 La herencia permite crear una nueva clase que reutiliza, extiende y modifica el comportamiento que se define en otra clase. La clase cuyos miembros se heredan se denomina la *clase base*, y la clase que hereda esos miembros se denomina la *clase derivada*. Sin embargo, todas las clases en Visual Basic heredan implícitamente de la <xref:System.Object>clase que admite la jerarquía de clases .NET y proporciona servicios de bajo nivel a todas las clases.</xref:System.Object>  
  
> [!NOTE]
>  Visual Basic no admite herencia múltiple. Es decir, puede especificar sólo una clase base para una clase derivada.  
...
##  <a name="Delegates"></a>Delegados  
 Un *delegar* es un tipo que define una firma de método y puede proporcionar una referencia a cualquier método con una firma compatible. Puede invocar (o llamar) al método a través del delegado. Los delegados se utilizan para pasar métodos como argumentos a otros métodos.  
  
> [!NOTE]
>  Los controladores de eventos no son más que métodos que se invocan a través de delegados. Para obtener más información acerca de cómo utilizar delegados en el control de eventos, vea [eventos](http://msdn.microsoft.com/library/b6f65241-e0ad-4590-a99f-200ce741bb1f).  
