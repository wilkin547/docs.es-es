---
title: LINQ to XML seguridad (Visual Basic) | Documentos de Microsoft
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
## <a name="linq-to-xml-security-overview"></a>Información general de seguridad de LINQ to XML  
 ...
 Si debe procesar datos no es de confianza de orígenes desconocidos, Microsoft recomienda que utilice una instancia de la <xref:System.Xml.XmlReader>clase que se ha configurado para filtrar la denegación de servicio (DoS) XML.</xref:System.Xml.XmlReader>  
  
 Si ha configurado un <xref:System.Xml.XmlReader>para mitigar la denegación de ataques de servicio, puede usar ese lector para rellenar un árbol de LINQ to XML y seguir sacando provecho de las mejoras de productividad del programador de LINQ to XML.</xref:System.Xml.XmlReader> Muchas técnicas de mitigación del riesgo implican crear lectores configurados para mitigar el problema de seguridad y crear a continuación instancias de un árbol XML mediante el lector configurado.  
...
## <a name="mitigation-of-xml-xsd-xpath-and-xslt-attacks"></a>Reducción del riesgo de ataques de XML, XSD, XPath y XSLT  
 LINQ to XML se basa en <xref:System.Xml.XmlReader>y <xref:System.Xml.XmlWriter>.</xref:System.Xml.XmlWriter> </xref:System.Xml.XmlReader> LINQ to XML admite XSD y XPath mediante métodos de extensión en el <xref:System.Xml.Schema?displayProperty=fullName>y <xref:System.Xml.XPath?displayProperty=fullName>los espacios de nombres.</xref:System.Xml.XPath?displayProperty=fullName> </xref:System.Xml.Schema?displayProperty=fullName> Mediante el <xref:System.Xml.XmlReader>, <xref:System.Xml.XPath.XPathNavigator>, y <xref:System.Xml.XmlWriter>clases junto con LINQ to XML, puede invocar XSLT para transformar árboles XML.</xref:System.Xml.XmlWriter> </xref:System.Xml.XPath.XPathNavigator> </xref:System.Xml.XmlReader>  
  
 Si está trabajando en un entorno menos seguro, existen una serie de problemas de seguridad asociados con XML y el uso de las clases <xref:System.Xml?displayProperty=fullName>, <xref:System.Xml.Schema?displayProperty=fullName>, <xref:System.Xml.XPath?displayProperty=fullName>y <xref:System.Xml.Xsl?displayProperty=fullName>.</xref:System.Xml.Xsl?displayProperty=fullName> </xref:System.Xml.XPath?displayProperty=fullName> </xref:System.Xml.Schema?displayProperty=fullName> </xref:System.Xml?displayProperty=fullName> Algunos de los problemas son:  
...
-   No acepte componentes auxiliares, como <xref:System.Xml.NameTable>, <xref:System.Xml.XmlNamespaceManager>, y <xref:System.Xml.XmlResolver>objetos de ensamblados de confianza.</xref:System.Xml.XmlResolver> </xref:System.Xml.XmlNamespaceManager> </xref:System.Xml.NameTable>  
...
### <a name="do-not-call-codeaccesspermissionsassert-in-an-event-handler"></a>No llamar a CodeAccessPermissions.Assert en un controlador de eventos  
 ...
 Si el código en un ensamblado con más permisos llama <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName>en un controlador de eventos y, a continuación, el código XML árbol se pasa a un ensamblado malintencionado que tiene permisos restringidos, el ensamblado malintencionado puede hacer que se produzca un evento.</xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName> Como el evento ejecuta código que está en el ensamblado con más permisos, el ensamblado malintencionado funcionará con privilegios elevados.  
  
 Microsoft recomienda que nunca llaman a <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName>en un controlador de eventos.</xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName>  
...
### <a name="avoid-excessive-buffer-allocation"></a>Evitar la asignación excesiva de búfer  
 ...
 Si un usuario malintencionado envía o carga un documento XML de gran tamaño, puede hacer que LINQ to XML consuma demasiados recursos del sistema. Esto puede representar un ataque de denegación de servicio. Para evitar esto, puede establecer el <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName>propiedad y crear un lector que tenga un límite en el tamaño del documento que puede cargar.</xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> El lector se usa a continuación para crear el árbol XML.  
  
 Por ejemplo, si sabe que el tamaño máximo previsto de los documentos XML procedentes de un origen de confianza se ser inferior a 50 KB, establezca <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName>en 100.000.</xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> Esto no será un inconveniente para el procesamiento de sus documentos XML y a la vez contrarrestará amenazas de denegación de servicio en las que se podrían cargar documentos que consumirían grandes cantidades de memoria.  
  
### <a name="avoid-excess-entity-expansion"></a>Evitar la expansión excesiva de entidades  
 Uno de los ataques de denegación de servicio conocidos cuando se usa una DTD es un documento que provoca una expansión excesiva de entidades. Para evitar esto, puede establecer el <xref:System.Xml.XmlReaderSettings.MaxCharactersFromEntities%2A?displayProperty=fullName>propiedad y crear un lector que tenga un límite en el número de caracteres que resultan de la expansión de entidades.</xref:System.Xml.XmlReaderSettings.MaxCharactersFromEntities%2A?displayProperty=fullName> El lector se usa a continuación para crear el árbol XML.  
  
### <a name="limit-the-depth-of-the-xml-hierarchy"></a>Limitar la profundidad de la jerarquía XML  
 Un posible ataque de denegación de servicio consiste en enviar un documento que tiene una profundidad de jerarquía excesiva. Para evitarlo, puede encapsular un <xref:System.Xml.XmlReader>en su propia clase que cuenta la profundidad de elementos.</xref:System.Xml.XmlReader> Si la profundidad supera un nivel razonable predeterminado, puede finalizar el procesamiento del documento malintencionado.  
...
### <a name="protect-against-untrusted-xmlreader-or-xmlwriter-implementations"></a>Protegerse frente a implementaciones de XmlReader o XmlWriter que no son de confianza  
 Los administradores deben comprobar que cualquier proporcionadas de forma externa <xref:System.Xml.XmlReader>o <xref:System.Xml.XmlWriter>las implementaciones tienen nombres seguros y se han registrado en la configuración del equipo.</xref:System.Xml.XmlWriter> </xref:System.Xml.XmlReader> Esto evita que se cargue un código malintencionado que simula ser un lector o escritor.  
  
### <a name="periodically-free-objects-that-reference-xname"></a>Liberar periódicamente objetos que hacen referencia a XName  
 Para protegerse frente a cierto tipo de ataques, los programadores de aplicaciones deben liberar todos los objetos que hacen referencia a un <xref:System.Xml.Linq.XName>objeto en el dominio de aplicación de forma periódica.</xref:System.Xml.Linq.XName>  
  
### <a name="protect-against-random-xml-names"></a>Protegerse frente a nombres XML aleatorios  
 Deben tener en cuenta las aplicaciones que toman datos de orígenes de confianza utilizando una <xref:System.Xml.XmlReader>que se incluye en el código personalizado para inspeccionar la posibilidad de espacios de nombres y nombres XML aleatorios.</xref:System.Xml.XmlReader> Si se detectan nombres y espacios de nombres XML aleatorios, la aplicación puede finalizar el procesamiento del documento malintencionado.  
