...
- La palabra clave [End \<palabra clave > instrucción](../../../visual-basic/language-reference/statements/end-keyword-statement.md) , que finaliza la ejecución de un programa, está deshabilitada. El siguiente programa se compila y se ejecuta sin `-netcf` pero produce un error en tiempo de compilación con `-netcf`.
