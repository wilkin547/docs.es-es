---
description: 'Más información sobre: no se pueden recibir eventos del objeto porque el objeto ya está desencadenando eventos hasta el número máximo de receptores de eventos que admite'
title: No se pueden recibir eventos del objeto porque el objeto ya está desencadenando eventos hasta el número máximo de receptores de eventos que admite.
ms.date: 07/20/2015
f1_keywords:
- vbrID96
ms.assetid: e40b19a2-8100-43ff-a1a4-0ddee3cc7f9a
ms.openlocfilehash: f8f854f983dc666b04e77283ce3578008ad870f3
ms.sourcegitcommit: 10e719780594efc781b15295e499c66f316068b8
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 02/14/2021
ms.locfileid: "100456970"
---
# <a name="unable-to-sink-events-of-object-because-the-object-is-already-firing-events-to-the-maximum-number-of-event-receivers-it-supports"></a>No se pueden recibir eventos del objeto porque el objeto ya está desencadenando eventos hasta el número máximo de receptores de eventos que admite.

Se ha superado el número máximo de receptores de eventos admitidos por el objeto.  
  
### <a name="to-correct-the-error"></a>Para corregir el error  
  
- Reduzca el número de receptores de eventos.  
  
## <a name="see-also"></a>Consulte también

- [Cómo: Generar eventos controlados por un receptor COM](/previous-versions/dotnet/netframework-4.0/dd8bf0x3(v=vs.100))
- [Tutorial: Controlar eventos](../programming-guide/language-features/events/walkthrough-handling-events.md)
