...
Una expresión se evalúa a un tipo al que Common Language Runtime \(CLR\) no puede aplicar la conversión boxing.  
 ...
 Si no ha usado `CType` o `CObj` en la instrucción que contiene esta expresión, [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] ha intentado una conversión implícita que produce este error.  
