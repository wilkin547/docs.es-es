...
Una definición de propiedad aplica el atributo <xref:System.CLSCompliantAttribute> a su instrucción `Get` o `Set` .  
 ...
 Para una propiedad que sea compatible con la [independencia del lenguaje y componentes independientes del lenguaje](https://msdn.microsoft.com/library/12a7a7h3) (CLS), toda la propiedad debe marcarse como `<CLSCompliant(True)>`. Se debe aplicar <xref:System.CLSCompliantAttribute> a la [Property Statement](../../visual-basic/language-reference/statements/property-statement.md), no a la instrucción `Get` o `Set` .  
