...
Una definición de propiedad aplica el atributo <xref:System.CLSCompliantAttribute> a su instrucción `Get` o `Set` .  
 ...
 Para que una propiedad sea compatible con la [independencia del lenguaje y los componentes de Language-Independent](../../standard/language-independence-and-language-independent-components.md) (CLS), la propiedad completa debe marcarse como `<CLSCompliant(True)>` . Se debe aplicar <xref:System.CLSCompliantAttribute> a la [Property Statement](../language-reference/statements/property-statement.md), no a la instrucción `Get` o `Set` .  
