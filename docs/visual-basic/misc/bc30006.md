...
Es necesaria una referencia al módulo '\<`modulename`\>' que contiene la definición del evento '`eventname`'. Agregue una al proyecto.  
 ...
 El evento se define en un módulo al que no se hace referencia directamente en el proyecto. El compilador [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] requiere una referencia para evitar la ambigüedad en caso de que el evento esté definido en más de un módulo.  
