...
Hay varias instrucciones en C# que se basan en patrones definidos, como `foreach` y `using`. Por ejemplo, el [ `foreach` instrucción](../language-reference/keywords/foreach-in.md) se basa en la clase de colección que implementa el patrón "enumerable".
