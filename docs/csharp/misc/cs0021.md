...
En el C# ejemplo siguiente se llama al archivo. dll. Intenta tener acceso a la clase a través de un indexador, pero dado que no se declaró ningún miembro como el indexador predeterminado, se genera el error. Para solucionar el error, quite la marca de comentario de la línea `[DefaultMember("myItem")]`.
