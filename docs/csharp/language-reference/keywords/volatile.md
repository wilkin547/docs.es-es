...
La palabra clave volatile solo puede aplicarse a campos de `class` o `struct`. Las variables locales no se pueden declarar como `volatile`.
...
- Clase <xref:System.Threading.Interlocked>
