...
Una instrucción `if` identifica qué instrucción se debe ejecutar dependiendo del valor de una expresión booleana. En el ejemplo siguiente, la variable `bool` `result` se establece en `true` y, a continuación, se comprueba en la instrucción `if` . El resultado es `The variable is set to true.`
