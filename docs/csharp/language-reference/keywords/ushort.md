...
En el ejemplo siguiente, los enteros que equivalen a 65 034 que se representan como literales binarios, hexadecimales y decimales se convierten implícitamente de [int](int.md) a valores `ushort`.
...
Debe usarse una conversión al llamar a métodos sobrecargados. Por ejemplo, considere los siguientes métodos sobrecargados que usan parámetros `ushort` e [int](int.md):
...
Hay una conversión implícita predefinida de `ushort` a [int](int.md), [uint](uint.md), [long](long.md), [ulong](ulong.md), [float](float.md), [double](double.md) o [decimal](decimal.md).

Hay una conversión implícita predefinida de [byte](byte.md) o [char](char.md) a `ushort`. De lo contrario, se debe usar una conversión para realizar una conversión explícita. Por ejemplo, considere las dos variables de `ushort` siguientes, `x` y `y`:
