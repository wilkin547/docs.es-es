...
El atributo 'attribute' no es válido en este tipo de declaración. Solo es válido en declaraciones 'type'.  
 ...
 Al definir un atributo, se define a qué construcciones se puede aplicar especificando un valor de `AttributeTargets`. En el siguiente ejemplo, el atributo `MyAttribute` se puede aplicar solo a las interfaces, porque el atributo `AttributeUsage` especifica `AttributeTargets.Iterface`. Se genera el error porque el atributo se aplica a una clase (clase `A`).  
