---
title: "Cómo: Usar anotaciones para transformar árboles LINQ to XML en un estilo XSLT (C#) | Microsoft Docs"
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
```  
 ...
 Para cualquier nodo de texto dado puede haber cualquier número de elementos secundarios `<b>` y `<i>`. Este enfoque se extiende a otras situaciones diferentes, por ejemplo páginas que contienen diversos elementos secundarios, como párrafos normales, párrafos con viñetas o mapas de bits. Las celdas de una tabla pueden contener texto, listas desplegables o mapas de bits. Una de las características principal del XML centrado en documentos es que no se sabe qué elemento secundario puede tener cualquier elemento particular.  
...
-   Esto es parecido a la especificación de transformaciones en XSL. La consulta que selecciona un conjunto de nodos es análoga a la expresión XPath para una plantilla. El código para crear el nuevo <xref:System.Xml.Linq.XElement> que se guarda como anotación es análogo al constructor de secuencias en XSL y el elemento `ApplyTransforms` es análogo en funcionamiento al elemento `xsl:apply-templates` en XSL.  
...
## <a name="transforming-a-tree"></a>Transformar un árbol  
 El primer ejemplo cambia el nombre de todos los nodos `Paragraph` a `para`.  
