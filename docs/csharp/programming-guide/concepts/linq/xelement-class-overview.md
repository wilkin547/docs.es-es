---
title: "Información general de la clase XElement (C#) | Microsoft Docs"
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
La clase <xref:System.Xml.Linq.XElement> es una de las clases fundamentales en [!INCLUDE[sqltecxlinq](../../../../csharp/programming-guide/concepts/linq/includes/sqltecxlinq_md.md)]. Representa a un elemento XML. Puede utilizar esta clase para crear elementos; cambiar el contenido del elemento; agregar, modificar o eliminar elementos secundarios; agregar atributos a un elemento; o serializar el contenido de un elemento en forma de texto. También puede interoperar con otras clases de <xref:System.Xml?displayProperty=fullName>, como <xref:System.Xml.XmlReader>, <xref:System.Xml.XmlWriter> y <xref:System.Xml.Xsl.XslCompiledTransform>.  
  
## <a name="xelement-functionality"></a>Funcionalidad de XElement  
 En este tema se describe la funcionalidad que proporciona la clase <xref:System.Xml.Linq.XElement>.  
...
-   Puede analizar XML a partir de diferentes orígenes, incluido un <xref:System.IO.TextReader>, archivos de texto o una dirección web (URL). Para obtener más información, consulte [Parsing XML (C#)](../../../../csharp/programming-guide/concepts/linq/parsing-xml.md) (Analizar XML [C#]).  
  
-   Puede usar un <xref:System.Xml.XmlReader> para rellenar el árbol. Para obtener más información, consulte <xref:System.Xml.Linq.XNode.ReadFrom%2A>.  
  
-   Si dispone de un módulo que pueda escribir contenido en un <xref:System.Xml.XmlWriter>, puede usar el método <xref:System.Xml.Linq.XContainer.CreateWriter%2A> para crear un sistema de escritura, pasar este al módulo y, luego, usar el contenido que se haya escrito en el <xref:System.Xml.XmlWriter> para rellenar el árbol XML.  
...
```  
 ...
 Otra técnica usada con frecuencia para crear un árbol XML implica el uso de los resultados de una consulta [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq_md.md)] para rellenar el árbol XML, tal y como se muestra en el ejemplo siguiente:  
...
### <a name="serializing-xml-trees"></a>Serializar árboles XML  
 Puede serializar el árbol XML en un <xref:System.IO.File>, un <xref:System.IO.TextWriter> o un <xref:System.Xml.XmlWriter>.  
...
### <a name="querying-xml-trees"></a>Consultar árboles XML  
 Puede escribir consultas [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq_md.md)] que extraigan datos de un árbol XML.  
