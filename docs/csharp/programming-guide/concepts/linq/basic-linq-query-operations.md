...
## Obtener un origen de datos  
 En una consulta [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq_md.md)], el primer paso es especificar el origen de datos.  En C\#, como en la mayoría de los lenguajes de programación, se debe declarar una variable antes de poder utilizarla.  En una consulta [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq_md.md)], la cláusula `from` aparece en primer lugar para introducir el origen de datos \(`customers`\) y la *variable de rango* \(`cust`\).  
