---
title: Seguridad de LINQ to XML (C#) | Microsoft Docs
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
## <a name="linq-to-xml-security-overview"></a>Información general de seguridad de LINQ to XML  
 ...
 Si debe procesar datos que no son de confianza de orígenes desconocidos, Microsoft recomienda usar una instancia de la clase <xref:System.Xml.XmlReader> que se ha configurado para filtrar ataques por denegación de servicio (DoS) XML.  
  
 Si ha configurado <xref:System.Xml.XmlReader> para mitigar los ataques por denegación de servicio, puede usar ese lector para rellenar un árbol de LINQ to XML y seguir sacando provecho de las mejoras de productividad del programador de LINQ to XML. Muchas técnicas de mitigación del riesgo implican crear lectores configurados para mitigar el problema de seguridad y crear a continuación instancias de un árbol XML mediante el lector configurado.  
...
## <a name="mitigation-of-xml-xsd-xpath-and-xslt-attacks"></a>Reducción del riesgo de ataques de XML, XSD, XPath y XSLT  
 LINQ to XML se basa en <xref:System.Xml.XmlReader> y <xref:System.Xml.XmlWriter>. LINQ to XML admite XSD y XPath mediante métodos de extensión en los espacios de nombre <xref:System.Xml.Schema?displayProperty=fullName> y <xref:System.Xml.XPath?displayProperty=fullName>. Usando las clases <xref:System.Xml.XmlReader>, <xref:System.Xml.XPath.XPathNavigator> y <xref:System.Xml.XmlWriter> junto con LINQ to XML, puede invocar XSLT para transformar árboles XML.  
  
 Si está trabajando en un entorno menos seguro, existen varios problemas de seguridad asociados con XML y el uso de clases en <xref:System.Xml?displayProperty=fullName>, <xref:System.Xml.Schema?displayProperty=fullName>, <xref:System.Xml.XPath?displayProperty=fullName> y <xref:System.Xml.Xsl?displayProperty=fullName>. Algunos de los problemas son:  
...
-   No acepte componentes auxiliares, como objetos <xref:System.Xml.NameTable>, <xref:System.Xml.XmlNamespaceManager> y <xref:System.Xml.XmlResolver> de ensamblados que no sean de confianza.  
...
### <a name="do-not-call-codeaccesspermissionsassert-in-an-event-handler"></a>No llamar a CodeAccessPermissions.Assert en un controlador de eventos  
 ...
 Si el código de un ensamblado con más permisos llama a <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName> en un controlador de eventos y después el árbol XML se pasa a un ensamblado malintencionado que tiene permisos restringidos, el ensamblado malintencionado puede hacer que se genere un evento. Como el evento ejecuta código que está en el ensamblado con más permisos, el ensamblado malintencionado funcionará con privilegios elevados.  
  
 Microsoft recomienda que no llame nunca a <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName> en un controlador de eventos.  
...
### <a name="avoid-excessive-buffer-allocation"></a>Evitar la asignación excesiva de búfer  
 ...
 Si un usuario malintencionado envía o carga un documento XML de gran tamaño, puede hacer que LINQ to XML consuma demasiados recursos del sistema. Esto puede representar un ataque de denegación de servicio. Para evitarlo, puede establecer la propiedad <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> y crear un lector que presente un límite en el tamaño del documento que puede cargar. El lector se usa a continuación para crear el árbol XML.  
  
 Por ejemplo, si sabe que el tamaño máximo previsto de los documentos XML procedentes de un origen que no es de confianza será inferior a 50.000 bytes, establezca <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> en 100.000. Esto no será un inconveniente para el procesamiento de sus documentos XML y a la vez contrarrestará amenazas de denegación de servicio en las que se podrían cargar documentos que consumirían grandes cantidades de memoria.  
  
### <a name="avoid-excess-entity-expansion"></a>Evitar la expansión excesiva de entidades  
 Uno de los ataques de denegación de servicio conocidos cuando se usa una DTD es un documento que provoca una expansión excesiva de entidades. Para evitarlo, puede establecer la propiedad <xref:System.Xml.XmlReaderSettings.MaxCharactersFromEntities%2A?displayProperty=fullName> y crear un lector que presente un límite en el número de caracteres producidos por la expansión de entidades. El lector se usa a continuación para crear el árbol XML.  
...
### <a name="periodically-free-objects-that-reference-xname"></a>Liberar periódicamente objetos que hacen referencia a XName  
 Para protegerse frente a ciertos tipos de ataques, los programadores de aplicaciones deben liberar todos los objetos que hacen referencia a un objeto <xref:System.Xml.Linq.XName> en el dominio de la aplicación de forma periódica.  
