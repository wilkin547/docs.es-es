...
La instrucción [`throw`](../language-reference/throw.md) **genera** una excepción. De esta forma, se finaliza la ejecución del método actual, que se reanudará cuando se encuentre un bloque `catch` correspondiente. Se agregará un bloque `catch` para probar este código un poco más adelante.
