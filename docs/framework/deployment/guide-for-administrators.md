---
title: "Gu&#237;a de implementaci&#243;n de .NET Framework para administradores | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "guía del administrador, implementar .NET Framework"
  - "implementación [.NET Framework], guía del administrador"
...
>   
>  Para obtener información sobre soporte técnico para .NET Framework, vea la [política del ciclo de vida de soporte técnico de Microsoft .NET Framework](http://go.microsoft.com/fwlink/?LinkId=196607) en el sitio web de soporte de Microsoft.  
...
-   Las **colecciones** son grupos de recursos de Configuration Manager, tales como usuarios, grupos de usuarios o equipos, en los que se implementa .NET Framework.  Para obtener más información, vea [Colecciones en el Administrador de configuración](http://technet.microsoft.com/library/gg682169.aspx) en la biblioteca de documentación de Configuration Manager.  
  
-   Los **paquetes y programas** suelen representar aplicaciones de software que se van a instalar en un equipo cliente, aunque también pueden contener archivos individuales, actualizaciones o incluso comandos individuales.  Para obtener más información, vea [Paquetes y programas en Configuration Manager](http://technet.microsoft.com/library/gg699369.aspx) en la biblioteca de documentación de Configuration Manager.  
...
-   Las **implementaciones** indican los miembros correspondientes de la colección de destino especificada para instalar el paquete de software.  Para obtener más información, vea [Cómo implementar aplicaciones en Configuration Manager](http://technet.microsoft.com/library/gg682082.aspx) en la biblioteca de documentación de Configuration Manager.  
  
> [!IMPORTANT]
>  Los procedimientos de este tema contienen valores típicos para crear e implementar un paquete y un programa y puede que no cubran todos los valores posibles.  Para conocer otras opciones de implementación de Configuration Manager, vea la [Biblioteca de documentación de System Center 2012 Configuration Manager](http://technet.microsoft.com/library/gg682041.aspx).  
...
6.  En la página **Reglas de pertenencia**, elija **Agregar regla** y, a continuación, **Regla directa** para abrir el **Asistente para crear reglas de pertenencia directa**.  Elija **Siguiente**.  
  
7.  En la página **Buscar recursos**, en la lista **Clase de recurso**, elija **Recurso del sistema**.  En la lista **Nombre del atributo**, elija **Nombre**.  En el campo **Valor**, escriba `%` y, a continuación, elija **Siguiente**.  
  
8.  En la página **Seleccionar recursos**, active la casilla de cada equipo en el que desea implementar .NET Framework.  Elija **Siguiente** y, a continuación, finalice el asistente.  
...
9. En la página **Reglas de pertenencia** del **Asistente para crear recopilación de dispositivos**, elija **Siguiente** y finalice el asistente.  
 ...
 Para obtener más información sobre colecciones, vea el tema sobre [colecciones en Configuration Manager](http://technet.microsoft.com/library/bb693730.aspx) en la biblioteca de documentación de Configuration Manager.  
...
2.  En el área de trabajo **Biblioteca de software**, expanda **Administración de aplicaciones** y, a continuación, elija **Paquetes**.  
...
5.  Elija **Este paquete contiene archivos de origen** y, después, **Examinar** para seleccionar la carpeta local o de red que contiene los archivos de instalación de .NET Framework.  Una vez seleccionada la carpeta, elija **Aceptar** y, a continuación, **Siguiente**.  
...
7.  En la página **Programa** del **Asistente para crear paquetes y programas**, escriba la siguiente información:  
    ...
    2.  **Línea de comandos:** `dotNetFx45_Full_x86_x64.exe /q /norestart /ChainingPackage ADMINDEPLOYMENT` \(las opciones de la línea de comandos se describen en la tabla tras estos pasos\)  
...
8.  En la página **Requisitos**, elija **Siguiente** para aceptar los valores predeterminados y, a continuación, finalice el asistente.  
...
|**\/chainingpackage** *PackageName*|Especifica el nombre del paquete que realiza el encadenamiento.  Esta información se notifica, junto con otra información de sesión de instalación, en relación a aquellos usuarios que se han registrado en el [Programa para la mejora de la experiencia del usuario de Microsoft \(CEIP, por sus siglas en inglés\)](http://go.microsoft.com/fwlink/p/?LinkId=248244).  Si el nombre del paquete incluye espacios, utilice comillas dobles como delimitadores; por ejemplo: **\/chainingpackage "Chaining Product"**.|  
  
 Mediante estos pasos se crea un paquete denominado .NET Framework 4.5.  El programa implementa una instalación silenciosa de .NET Framework 4.5.  En una instalación silenciosa, los usuarios no interactúan con el proceso de instalación y la aplicación de encadenamiento tiene que capturar el código devuelto y controlar el reinicio; vea [Obtener información de progreso de un paquete de instalación](http://go.microsoft.com/fwlink/?LinkId=179606) en MSDN Library.  
...
6.  En la página **Destino del contenido** del asistente, elija **Agregar** y, a continuación, **Punto de distribución**.  
...
5.  En la página **General** del **Asistente para implementar software**, elija **Examinar** y, a continuación, seleccione la colección que creó anteriormente.  Elija **Siguiente**.  
...
7.  En la página **Configuración de implementación** del asistente, confirme que la **Acción** está establecida en **Instalar** y el **Propósito** en **Requerido**.  Esto garantiza que el paquete de software sea una instalación obligatoria en los equipos de destino.  Elija **Siguiente**.  
  
8.  En la página **Programación** del asistente, especifique cuándo desea que se instale .NET Framework.  Puede elegir **Nuevo** para asignar un tiempo de instalación o bien indicar al software que realice la instalación cuando el usuario inicie o cierre sesión o lo antes posible.  Elija **Siguiente**.  
  
9. En la página **Experiencia del usuario** del asistente, use los valores predeterminados y elija **Siguiente**.  
  
    > [!WARNING]
    >  El entorno de producción puede tener directivas que requieran selecciones distintas para la programación de distribución.  Para obtener información acerca de estas opciones, consulte la [Pestaña programación de las propiedades de nombre de anuncio](http://technet.microsoft.com/library/bb694016.aspx) en la biblioteca de TechNet.  
...
-   [Guía detallada de Windows Server 2008 para DNS en redes pequeñas](http://www.microsoft.com/download/details.aspx?id=17157)  
...
-   [Instalación de SQL Server 2008 \(vídeo de SQL Server\)](http://technet.microsoft.com/library/dd299415.aspx)  
...
-   [Códigos de error del Servicio de transferencia inteligente en segundo plano \(BITS, por sus siglas en inglés\)](http://msdn.microsoft.com/library/aa362823.aspx)  
  
-   [Códigos de error del moniker de dirección URL](http://msdn.microsoft.com/library/ms775145.aspx)  
  
-   [Códigos de error WinHttp](http://msdn.microsoft.com/library/aa383770.aspx)  
