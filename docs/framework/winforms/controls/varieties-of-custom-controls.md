...
## <a name="base-control-class"></a>Clase base de Control  
 La <xref:System.Windows.Forms.Control> clase es la clase base para los controles de Windows Forms. Proporciona la infraestructura necesaria para la presentación visual en aplicaciones de Windows Forms.  
  
 La <xref:System.Windows.Forms.Control> clase realiza las siguientes tareas para proporcionar la presentación visual en Windows Forms aplicaciones:  
...
- Contiene muchas propiedades específicas de la presentación <xref:System.Windows.Forms.Control.ForeColor%2A>visual, como <xref:System.Windows.Forms.Control.Height%2A>, <xref:System.Windows.Forms.Control.BackColor%2A>, y <xref:System.Windows.Forms.Control.Width%2A>.  
...
### <a name="composite-controls"></a>Controles compuestos  
 ...
 Un ejemplo de un control compuesto podría ser un control creado para mostrar los datos de dirección de los clientes de una base de datos. Este control podría incluir un <xref:System.Windows.Forms.DataGridView> control para mostrar los campos de base de <xref:System.Windows.Forms.BindingSource> datos, para controlar el enlace a un origen de <xref:System.Windows.Forms.BindingNavigator> datos y un control para desplazarse por los registros. Las propiedades de enlace de datos se podrían exponer de forma selectiva, y el control completo se podría empaquetar y reutilizar en distintas aplicaciones. Para obtener un ejemplo de este tipo de control compuesto, [consulte How to: Aplicar atributos en controles](how-to-apply-attributes-in-windows-forms-controls.md)de Windows Forms.  
...
### <a name="extended-controls"></a>Controles extendidos  
 ...
 Por ejemplo, puede crear un control derivado del <xref:System.Windows.Forms.Button> control que realice un seguimiento del número de veces que un usuario ha haga clic en él.  
  
 En algunos controles, también puede Agregar una apariencia personalizada a la interfaz gráfica de usuario del control invalidando el <xref:System.Windows.Forms.Control.OnPaint%2A> método de la clase base. Para un botón extendido que realice un seguimiento de los clics, <xref:System.Windows.Forms.Control.OnPaint%2A> puede invalidar el método para llamar <xref:System.Windows.Forms.Control.OnPaint%2A>a la implementación base de y, a continuación, dibujar el <xref:System.Windows.Forms.Button> recuento de clics en una esquina del área cliente del control.  
...
### <a name="custom-controls"></a>Controles personalizados  
 Otra forma de crear un control es crear uno prácticamente desde el principio heredando de <xref:System.Windows.Forms.Control>. La <xref:System.Windows.Forms.Control> clase proporciona toda la funcionalidad básica requerida por los controles, incluidos los eventos de control del mouse y del teclado, pero no la funcionalidad específica del control ni la interfaz gráfica.  
  
 La creación de un control heredando de <xref:System.Windows.Forms.Control> la clase requiere mucho más pensamiento y esfuerzo que heredar <xref:System.Windows.Forms.UserControl> de o un control de Windows Forms existente. Como puede realizar gran parte de la implementación, el control puede tener mayor flexibilidad que un control compuesto o ampliado, y puede personalizar el control exactamente para sus necesidades.  
  
 Para implementar un control personalizado, debe escribir código para el <xref:System.Windows.Forms.Control.OnPaint%2A> evento del control, así como cualquier código específico de la característica que necesite. También puede invalidar el <xref:System.Windows.Forms.Control.WndProc%2A> método y controlar los mensajes de Windows directamente. Esta es la manera más eficaz de crear un control, pero para utilizar esta técnica de forma eficaz, debe estar familiarizado con la API Win32 de Microsoft®.  
  
 Un ejemplo de un control personalizado es un control de reloj que reproduce la apariencia y el funcionamiento de un reloj analógico. La representación personalizada se invoca para hacer que las manecillas del reloj se muevan en <xref:System.Windows.Forms.Timer.Tick> respuesta a eventos de <xref:System.Windows.Forms.Timer> un componente interno. Para obtener más información, consulte [Cómo Desarrolle un control](how-to-develop-a-simple-windows-forms-control.md)de Windows Forms simple.  
...
### <a name="activex-controls"></a>Controles ActiveX  
 Aunque la infraestructura de Windows Forms se han optimizado para hospedar controles de Windows Forms, todavía puede utilizar los controles ActiveX. Visual Studio es compatible con esta tarea. Para obtener más información, vea [Cómo: Agregue controles ActiveX a Windows Forms](how-to-add-activex-controls-to-windows-forms.md).  
...
## <a name="custom-design-experience"></a>Experiencia de diseño personalizada  
 Si necesita implementar una experiencia personalizada en tiempo de diseño, puede crear su propio diseñador. En el caso de los controles compuestos, derive la clase <xref:System.Windows.Forms.Design.ParentControlDesigner> de diseñador <xref:System.Windows.Forms.Design.DocumentDesigner> personalizada de las clases o. En el caso de los controles extendidos y personalizados, derive la <xref:System.Windows.Forms.Design.ControlDesigner> clase de diseñador personalizada de la clase.  
  
 <xref:System.ComponentModel.DesignerAttribute> Utilice para asociar el control al diseñador. Para obtener más información, vea [extender la compatibilidad en tiempo de diseño](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/37899azc(v=vs.120)) y [cómo: Cree un control de Windows Forms que aproveche las características](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/307hck25(v=vs.120))en tiempo de diseño.  
...
- [Procedimientos: Crear un control de Windows Forms que aproveche las características en tiempo de diseño](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/307hck25(v=vs.120))
