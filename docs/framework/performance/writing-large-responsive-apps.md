---
title: "Escribir aplicaciones grandes de .NET Framework que respondan | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Por qué el rendimiento del nuevo compilador afecta a su aplicación  
 ...
 Si desea obtener más información sobre los nuevos compiladores, visite el [proyecto de código abierto de .NET Compiler Platform \(«Roslyn»\)](http://roslyn.codeplex.com/).  
...
### Hecho 1: No optimice prematuramente  
...
### Hecho 3: Las herramientas de calidad marcan la diferencia  
 Unas herramientas de calidad permiten profundizar rápidamente en los problemas de rendimiento más importantes \(CPU, memoria o disco\) y sirven para localizar el código que provoca esos cuellos de botella.  Microsoft suministra diversas herramientas de rendimiento como pueden ser el [generador de perfiles de Visual Studio](../Topic/Beginners%20Guide%20to%20Performance%20Profiling.md), la [herramienta de análisis de Windows Phone](http://msdn.microsoft.com/es-es/e67e3199-ea43-4d14-ab7e-f7f19266253f) y [PerfView](http://www.microsoft.com/download/details.aspx?id=28567).  
  
 PerfView es una herramienta gratuita muy potente que sirve para centrarse en los problemas con raíces profundas como, por ejemplo, E\/S de disco, eventos de GC y memoria.  Puede capturar eventos de [Seguimiento de eventos para Windows](../../../docs/framework/wcf/samples/etw-tracing.md) \(ETW, por sus siglas en inglés\) relacionados con el rendimiento y luego ver la información dividida por aplicación, proceso, pila y subproceso.  PerfView muestra la cantidad y el tipo de memoria que asigna la aplicación, así como las funciones o pilas de llamadas que contribuyen en determinada medida a las asignaciones de memoria.  Para obtener más información, vea los completos artículos de ayuda, las demos y los vídeos que se incluyen con la herramienta \(por ejemplo, los [tutoriales de PerfView](http://channel9.msdn.com/Series/PerfView-Tutorial) en Channel 9\).  
  
### Hecho 4: La clave está en las asignaciones  
 ...
 **Ejemplo 2: Conversión boxing de enumeración**  
...
```  
 ...
 Otra fuente común de conversión boxing en tipos de enumeración es el método <xref:System.Enum.HasFlag%28System.Enum%29?displayProperty=fullName>.  Es necesario realizar una conversión boxing del argumento pasado a <xref:System.Enum.HasFlag%28System.Enum%29>.  En la mayoría de los casos, el reemplazo de llamadas a <xref:System.Enum.HasFlag%28System.Enum%29?displayProperty=fullName> con una prueba bit a bit es más sencilla y sin asignaciones.  
 ...
 **Ejemplo 5: Lambdas, List\<T\> e IEnumerable\<T\>**  
 ...
 En este ejemplo se usa [LINQ y código de estilo funcional](http://blogs.msdn.com/b/charlie/archive/2007/01/26/anders-hejlsberg-on-linq-and-functional-programming.aspx) para buscar un símbolo en el modelo del compilador a partir de una cadena de nombre:  
...
```  
 ...
 En la primera línea, la [expresión lambda](../Topic/Lambda%20Expressions%20\(C%23%20Programming%20Guide\).md) `s => s.Name == name` [se cierra sobre](http://blogs.msdn.com/b/ericlippert/archive/2003/09/17/53028.aspx) la variable local `name`.  Esto significa que, además de asignar un objeto para el [delegado](../Topic/delegate%20\(C%23%20Reference\).md) que contiene `predicate`, el código asigna una clase estática para contener el entorno que captura el valor de `name`.  El compilador genera código como el siguiente:  
...
```  
 ...
 Ahora observe la llamada a `FirstOrDefault`.  Este método de extensión en el tipo <xref:System.Collections.Generic.IEnumerable%601?displayProperty=fullName> también crea una asignación.  Dado que `FirstOrDefault` toma un objeto <xref:System.Collections.Generic.IEnumerable%601> como primer argumento, se puede expandir la llamada al código siguiente \(un poco simplificado para el análisis\):  
...
### Almacenamiento en caché del método asincrónico  
 En el ejemplo siguiente se muestra un problema común que se produce al usar resultados de la caché en un método [async](../Topic/Asynchronous%20Programming%20with%20Async%20and%20Await%20\(C%23%20and%20Visual%20Basic\).md).  
 ...
 **Corrección para el ejemplo 6**  
 ...
 Para quitar la asignación <xref:System.Threading.Tasks.Task> completada, puede almacenar el objeto de tarea con el resultado completado:  
...
## Vea también  
 ...
 [Guía básica para la generación de perfiles de rendimiento](../Topic/Beginners%20Guide%20to%20Performance%20Profiling.md)   
 [Performance](../../../docs/framework/performance/index.md)   
 [Sugerencias para mejorar el rendimiento de .NET](http://msdn.microsoft.com/library/ms973839.aspx)   
 [Herramienta de análisis de rendimiento de Windows Phone](http://msdn.microsoft.com/magazine/hh781024.aspx)   
 [Encontrar cuellos de botella de aplicaciones con el generador de perfiles de Visual Studio](http://msdn.microsoft.com/magazine/cc337887.aspx)   
 [Tutoriales de PerfView en Channel 9](http://channel9.msdn.com/Serie,%20PerfView-Tutorial)   
 [Sugerencias para un rendimiento de alto nivel](http://curah.microsoft.com/4604/improving-your-net-apps-startup-performance)   
 [Proyecto de código abierto de .NET Compiler Platform \(«Roslyn»\)](http://roslyn.codeplex.com/)