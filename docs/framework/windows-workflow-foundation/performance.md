---
title: "Rendimiento de Windows Workflow Foundation | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Rendimiento de Windows Workflow Foundation
Dustin Metzgar  
...
## Información general sobre las mejoras de rendimiento de WF4  
...
### Almacenamiento y flujo de datos  
 En WF3, los datos asociados a una actividad se modelan mediante propiedades de dependencia implementadas por el tipo <xref:System.Windows.DependencyProperty>.El modelo de propiedades de dependencia se introdujo en [!INCLUDE[avalon1](../../../includes/avalon1-md.md)].En general, este modelo es muy flexible para admitir un enlace de datos fácil y otras características de interfaz de usuario.Sin embargo, el modelo requiere que las propiedades se definan como campos estáticos en la definición de flujo de trabajo.El establecimiento o la obtención de los valores de propiedad por parte del runtime de [!INCLUDE[wf1](../../../includes/wf1-md.md)] suponen una lógica de búsqueda compleja.  
...
## Comparaciones de rendimiento en el nivel de componente  
 Esta sección contiene datos sobre comparaciones directas entre actividades individuales en flujos de trabajo de WF3 y WF4.Áreas clave como la persistencia tienen un impacto más profundo en el rendimiento que los componentes de actividades individuales.Las mejoras de rendimiento en componentes individuales de WF4 son importantes porque los componentes son ahora bastante rápidos en comparación con la lógica de orquestación codificada manualmente.Un ejemplo de ello se trata en la sección siguiente: "Escenario de composición de servicio".  
  
### Configuración de entorno  
 ![Entorno de la prueba de rendimiento de flujo de trabajo](../../../docs/framework/windows-workflow-foundation//media/wfperfenvironment.gif "WFPerfEnvironment")  
...
### Flujo de trabajo vacío  
 Esta prueba usa un flujo de trabajo de secuencia sin actividades secundarias.  
...
### Replicador comparado con ParallelForEach  
 <xref:System.Workflow.Activities.ReplicatorActivity> en WF3 tiene modos de ejecución paralela y secuencial.En modo secuencial, el rendimiento de la actividad es similar a <xref:System.Workflow.Activities.WhileActivity>.<xref:System.Workflow.Activities.ReplicatorActivity> es muy útil para la ejecución paralela.El equivalente de WF4 es la actividad <xref:System.Activities.Statements.ParallelForEach>.  
 ...
 ![ReplicatorActivity de WF3 y ParallelForEach de WF4](../../../docs/framework/windows-workflow-foundation//media/replicatorandparallelforeach.gif "ReplicatorAndParallelForEach")  
...
### Resultados de pruebas de rendimiento  
 ![Resultados de pruebas de rendimiento](../../../docs/framework/windows-workflow-foundation//media/performancedata.gif "PerformanceData")  
  
 ![Gráfico de datos de la prueba de rendimiento](../../../docs/framework/windows-workflow-foundation//media/performancetestchart.gif "PerformanceTestChart")  
...
## Escenario de composición de servicio  
 Como se muestra en la sección anterior, “Comparaciones de rendimiento en el nivel de componente”, ha habido una gran reducción en la sobrecarga entre WF3 y WF4.Los servicios de flujo de trabajo de [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] pueden casi igualar ahora el rendimiento de los servicios de [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] codificados a mano, pero siguen teniendo todas las ventajas del runtime de [!INCLUDE[wf1](../../../includes/wf1-md.md)].Este escenario de prueba compara un servicio de [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] con un servicio de flujo de trabajo de [!INCLUDE[indigo2](../../../includes/indigo2-md.md)] en WF4.  
  
### Servicio de tienda en línea  
 Uno de los puntos fuertes de [!INCLUDE[wf2](../../../includes/wf2-md.md)] es la capacidad de crear procesos mediante el uso de varios servicios.En este ejemplo, hay un servicio de tienda en línea que orquesta dos llamadas de servicio para realizar un pedido.El primer paso es validar el pedido utilizando un servicio de validación de pedidos.El segundo paso es rellenar el pedido utilizando un servicio de almacén.  
...
### Rendimiento  
 ![Gráfico de rendimiento de servicio de tienda en línea](../../../docs/framework/windows-workflow-foundation//media/onlinestoreperfgraph.gif "OnlineStorePerfGraph")  
...
#### Configuración de prueba  
 ![Prueba de flujo de trabajo del rendimiento de la correlación](../../../docs/framework/windows-workflow-foundation//media/correlationthroughputworkflow.gif "CorrelationThroughputWorkflow")  
  
 El flujo de trabajo mostrado más arriba es el mismo que se utiliza en la sección "Persistencia" que figura más abajo.Para las pruebas de la correlación sin persistencia no hay ningún proveedor de persistencia instalado en el runtime.La correlación se produce en dos lugares: CreateOrder y CompleteOrder.  
...
### Rendimiento de un flujo de trabajo complejo  
...
### Configuración de prueba  
 ...
 ![Compara el número de actividades usadas en cada prueba](../../../docs/framework/windows-workflow-foundation//media/comparechart.gif "CompareChart")  
 ...
 ![Flujo de trabajo de WF3](../../../docs/framework/windows-workflow-foundation//media/wf3workflow.gif "WF3Workflow")  
 ...
 ![Secuencias anidadas](../../../docs/framework/windows-workflow-foundation//media/nestedsequencewf.gif "NestedSequenceWF")  
...
### Memoria  
 La sobrecarga de la memoria de Windows Workflow Foundation se mide en dos áreas clave: la complejidad del flujo de trabajo y el número de definiciones de flujo de trabajo.Las mediciones de memoria se tomaron en una estación de trabajo de 64 bits con Windows 7.Hay muchas maneras de obtener la medición del tamaño del espacio de trabajo: supervisar los contadores de rendimiento, sondear Environment.WorkingSet o utilizar una herramienta como VMMap disponible en [VMMap](http://technet.microsoft.com/sysinternals/dd535533.aspx).Se utilizó una combinación de métodos para obtener y comprobar los resultados de cada prueba.  
  
### Prueba de complejidad de flujo de trabajo  
 ...
 ![Gráfico de uso de la memoria](../../../docs/framework/windows-workflow-foundation//media/complexmemoryusage.gif "ComplexMemoryUsage")  
...
|Servicios de flujo de trabajo hospedados en IIS|446 MB|364 MB|  
 ...
 Para el hospedaje en consola en WF3 los flujos de trabajo se implementaron en código, en lugar de XOML.En WF4, la opción predeterminada es usar XAML.El XAML se almacena como un recurso incrustado en el ensamblado y se compila en tiempo de ejecución para proporcionar la implementación del flujo de trabajo.Hay alguna sobrecarga asociada a este proceso.Para realizar una comparación objetiva entre WF3 y WF4, se usaron flujos de trabajo codificados en lugar de XAML.A continuación se muestra un ejemplo de uno de los flujos de trabajo de WF4:  
...
### Configuración de prueba  
 ...
 ![Flujos de trabajo de persistencia](../../../docs/framework/windows-workflow-foundation//media/persistworkflow.gif "PersistWorkflow")  
...
### Resultados de pruebas  
 ![Costo del seguimiento del flujo de trabajo](../../../docs/framework/windows-workflow-foundation//media/workflowtracingcost.gif "WorkflowTracingCost")  
