...
Compensación en Windows Workflow Foundation (WF) es el mecanismo por el que previamente trabajo completado se pueden deshacer o compensar (siguiendo la lógica definida por la aplicación) cuando se produce un error posterior. Esta sección describe cómo usar la compensación en los flujos de trabajo.  
...
> [!NOTE]
>  Este tema trata la compensación en flujos de trabajo. Para obtener más información acerca de las transacciones en flujos de trabajo, consulte [transacciones](../../../docs/framework/windows-workflow-foundation/workflow-transactions.md) y <xref:System.Activities.Statements.TransactionScope>. Para obtener más información acerca de las transacciones, vea <xref:System.Transactions?displayProperty=nameWithType> y <xref:System.Transactions.Transaction?displayProperty=nameWithType>.  
...
**Se completó correctamente con el estado de flujo de trabajo: cancelado.**  Para obtener más información sobre la cancelación, consulte [cancelación](../../../docs/framework/windows-workflow-foundation/modeling-cancellation-behavior-in-workflows.md).  
