---
title: "Esquema de base de datos de persistencia | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Esquema de base de datos de persistencia
En este tema se describen las vistas públicas admitidas por el almacén de instancias de flujo de trabajo de SQL.  
  
## Vista Instances  
 La vista **Instances** contiene información general sobre todas las instancias de flujo de trabajo de la base de datos.  
...
|SuspensionExceptionName|Nvarchar\(450\)|Indica el tipo de excepción \(por ejemplo,InvalidOperationException\) que hizo que el flujo de trabajo se suspendiera.|  
...
|ExecutionStatus|Nvarchar\(450\)|Indica el estado de ejecución actual del flujo de trabajo.Entre los posibles estados se encuentran **Executing**, **Idle**, **Closed**.|  
...
|IsCompleted|Bit|Indica si la instancia de flujo de trabajo ha completado la ejecución. **Note:**  Si la propiedad **InstanceCompletionAction** está establecida en **DeleteAll**, las instancias se quitan de la vista al finalizar.|  
|EncodingOption|TinyInt|Describe la codificación usada para serializar las propiedades de datos.<br /><br /> -   0: sin codificación<br />-   1: GzipStream|  
...
|Compilación|BigInt|Número de compilación de la versión de flujo de trabajo.|  
...
|RelativeServicePath|Nvarchar\(max\)|Representa la ruta de acceso virtual relativa al sitio que señala al servicio de flujo de trabajo.\(por ejemplo,**\/app1\/PurchaseOrderService.svc**\).|  
...
2.  Cualquier intento de eliminación de una fila de ServiceDeployment a la que se hace referencia en entradas de la vista **Instances** tendrá como resultado una operación inefectiva.Solo puede eliminar las filas de ServiceDeployment con cero referencias.  
  
## Vista InstancePromotedProperties  
 La vista **InstancePromotedProperties** contiene información para todas las propiedades promovidas que especifica el usuario.Una propiedad promovida funciona como una propiedad de primera clase, que un usuario puede utilizar en consultas para recuperar instancias.Por ejemplo, un usuario puede agregar una promoción de PurchaseOrder que siempre almacena el costo de un pedido en la columna **Value1**.Esto permitiría a un usuario consultar todos los pedidos cuyo costo supera un determinado valor.  
...
|EncodingOption|TinyInt|Describe la codificación usada para serializar las propiedades binarias promovidas.<br /><br /> -   0: sin codificación<br />-   1: GZipStream|  
...
> [!NOTE]
>  Una vista indizada requiere más almacenamiento y agrega sobrecarga de procesamiento adicional.Vea [Mejorar el rendimiento con vistas indizadas de SQL Server 2008](http://go.microsoft.com/fwlink/?LinkId=179529) para obtener más información.