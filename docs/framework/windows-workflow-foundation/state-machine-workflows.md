...
## <a name="creating-and-configuring-states"></a>Crear y configurar estados  
 <xref:System.Activities.Statements.State> representa un estado en el que una máquina de estados puede estar. Para agregar un <xref:System.Activities.Statements.State> a un flujo de trabajo, arrastre el diseñador de actividad **State** desde la sección **máquina de Estados** del **cuadro** de herramientas <xref:System.Activities.Statements.StateMachine> y colóquelo en una actividad en la superficie de diseñador de flujo de trabajo de Windows.  
