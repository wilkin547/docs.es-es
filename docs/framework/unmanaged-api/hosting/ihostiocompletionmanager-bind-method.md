...
## <a name="parameters"></a>Parámetros  
 `hPort`  
 de Puerto de finalización de e/s al que `hHandle`se va a enlazar. Si el valor de `hPort` es null, `hHandle` se enlaza al puerto de finalización de e/s predeterminado.  
  
 `hHandle`  
 de Identificador del sistema operativo con el que `hPort`se va a enlazar.  
...
## <a name="remarks"></a>Comentarios  
 Se crea un puerto de finalización de e/s mediante una `CreateIoCompletionPort`llamada a. CLR llama `Bind` a para enlazar un identificador a ese puerto.  
