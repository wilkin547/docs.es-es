...
## <a name="parameters"></a>Parámetros  
 `ixCdTkn`  
 [en] El tipo de token codificado que se va a devolver.  
  
 `pcTokens`  
 [fuera] Un puntero a `ppTokens`la longitud de .  
  
 `ppTokens`  
 [fuera] Puntero a un puntero a una matriz que contiene la lista de tokens devueltos.  
  
 `ppName`  
 [fuera] Un puntero a un puntero al `ixCdTkn`nombre del token en .  
