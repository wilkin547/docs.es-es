...
Esta función procesa la consulta especificada en el `strQuery` parámetro y crea un enumerador a través del cual el autor de llamada puede tener acceso a los resultados de consulta. El enumerador es un puntero a un [IEnumWbemClassObject](/windows/desktop/api/wbemcli/nn-wbemcli-ienumwbemclassobject) interfaz; la consulta los resultados son instancias de objetos de clase disponibles a través de la [IWbemClassObject](/windows/desktop/api/wbemcli/nn-wbemcli-iwbemclassobject) interfaz.
