...
|<xref:System.Windows.Automation.AutomationElementIdentifiers.ClickablePointProperty>|Vea las notas.|Si el control de lista tiene un punto interactivo (un punto en el que puede hacer clic para que la lista reciba el foco), dicho punto se debe exponer a través de esta propiedad.<br /><br /> Si el valor de la propiedad `IsOffScreen` es true, se generará el <xref:System.Windows.Automation.NoClickablePointException>.|  
