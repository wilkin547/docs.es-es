...
|serializationFormat|Especifica el formateador que se usa para serializar objetos que se envían como parte de un mensaje de MSMQ. Los valores válidos son<br /><br /> Controles El formateador de ActiveX se usa al serializar objetos COM.<br />Binario  Serializa el objeto en un paquete binario.<br />ByteArray  Serializa el objeto a una matriz de bytes.<br />Misiones  Serializa el objeto a una secuencia.<br />Lenguaje  Serializa el objeto en un paquete XML. El valor predeterminado es XML.<br /><br /> Este atributo es del tipo <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>.|  
