...
|queueTransferProtocol|Especifica el transporte del canal de comunicación en cola que usa este enlace. Los valores válidos son<br /><br /> Nativos  Utilice el protocolo MSMQ nativo.<br />SRMP  Utilice el Protocolo de mensajería confiable de SOAP (SRMP).<br />SrmpSecure Utilice el transporte seguro del Protocolo de mensajería confiable de SOAP (SRMPS).<br /><br /> Este atributo es del tipo <xref:System.ServiceModel.QueueTransferProtocol>.<br /><br /> Puesto que MSMQ no admite el direccionamiento de Active Directory cuando se usa el protocolo de mensajería confiable de SOAP, no debe establecer este atributo `useActiveDirectory` en SRMP o `true`Srmps cuando se establece en.|  
