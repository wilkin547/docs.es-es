...
|queueTransferProtocol|Especifica el transporte del canal de comunicación en cola que usa este enlace. Los valores válidos son<br /><br /> -Nativo: Use el protocolo MSMQ nativo.<br />-Srmp: Use el protocolo para la mensajería confiable Soap (SRMP).<br />-SrmpSecure: Utilizar el transporte de Soap de mensajería del protocolo confiable segura (SRMPS).<br /><br /> Este atributo es del tipo <xref:System.ServiceModel.QueueTransferProtocol>.<br /><br /> Puesto que MSMQ no admite el direccionamiento de Active Directory cuando se utiliza SOAP Reliable Messaging Protocol, no debe establecer este atributo a Srmp o Srmps cuando `u``seActiveDirectory` está establecido en `true`.|  
