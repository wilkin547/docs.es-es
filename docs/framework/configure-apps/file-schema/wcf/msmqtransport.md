...
|queueTransferProtocol|Especifica el transporte del canal de comunicación en cola que usa este enlace. Los valores válidos son<br /><br /> -Nativo: Use el protocolo MSMQ nativo.<br />-Srmp: Use el protocolo para la mensajería confiable Soap (SRMP).<br />-SrmpSecure: Use el transporte Soap Reliable Messaging del protocolo Secure (SRMPS).<br /><br /> Este atributo es del tipo <xref:System.ServiceModel.QueueTransferProtocol>.<br /><br /> Puesto que MSMQ no admite el direccionamiento de Active Directory cuando se usa el protocolo de mensajería confiable de SOAP, no debe establecer este atributo a Srmp o Srmps cuando `useActiveDirectory` está establecido en `true`.|  
