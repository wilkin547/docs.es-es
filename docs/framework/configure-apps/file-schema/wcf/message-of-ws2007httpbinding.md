...
Define la configuración para la seguridad de nivel de mensaje del elemento [\<> ws2007HttpBinding](ws2007httpbinding.md) .  
...
|`UserName`|Permite al servicio exigir la autenticación del cliente mediante una credencial `UserName`. WCF no admite el envío de un resumen de contraseña ni la derivación de claves mediante contraseña y el uso de tales claves para la seguridad de los mensajes. Como tal, WCF exige que el transporte esté protegido al usar credenciales de `UserName`. Este modo de credencial produce o bien un intercambio interoperable o bien una negociación no interoperable basada en el atributo `negotiateServiceCredential`.|  
...
|-------------|-----------------|  
|[\<La >](security-of-ws2007httpbinding.md)|Define la configuración de seguridad para un [\<ws2007HttpBinding >](ws2007httpbinding.md).|  
