...
|`UserName`|Permite al servicio exigir la autenticación del cliente mediante una credencial `UserName`. WCF no admite el envío de un resumen de contraseña ni la derivación de claves mediante contraseña y el uso de tales claves para la seguridad de los mensajes. Como tal, WCF exige que el transporte sea seguro al usar `UserName` las credenciales. Este modo de credencial produce o bien un intercambio interoperable o bien una negociación no interoperable basada en el atributo `negotiateServiceCredential`.|  
...
|-------------|-----------------|  
|[\<security>](security-of-ws2007httpbinding.md)|Define la configuración de seguridad para una [ \<> ws2007HttpBinding](ws2007httpbinding.md).|  
