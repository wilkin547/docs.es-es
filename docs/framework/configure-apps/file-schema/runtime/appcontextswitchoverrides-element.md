...
|`Switch.System.Activities.`<br/>`UseMD5CryptoServiceProviderForWFDebugger`|Cuando se establece `false`en, permite la depuración de proyectos de flujo de trabajo basados en XAML con Visual Studio cuando se habilita FIPS. Sin él, se <xref:System.NullReferenceException> produce una excepción en las llamadas a los métodos del ensamblado System. Activities.|.NET Framework 4.7|
...
|`Switch.System.Globalization.EnforceJapaneseEraYearRanges`|Controla si se aplican las comprobaciones de intervalo de año para las eras del calendario japonés. `true`para aplicar las comprobaciones de intervalo `false` de año y para deshabilitarlas (comportamiento predeterminado). Para obtener más información, vea [trabajar con calendarios](../../../../standard/datetime/working-with-calendars.md).|.NET Framework 4.6|
...
|`Switch.System.IdentityModel.`<br/>`DisableMultipleDNSEntriesInSANCertificate`|Controla si el <xref:System.IdentityModel.Claims.X509CertificateClaimSet.FindClaims%2A?displayProperty=nameWithType> método intenta hacer coincidir el tipo de Claim solo con la última entrada DNS. Para más información, vea [Mitigación: Método](../../../migration-guide/mitigation-x509certificateclaimset-findclaims-method.md)X509CertificateClaimSet. FindClaims.|.NET Framework 4.6.1|  
...
|`Switch.System.IO.BlockLongPaths`|Controla si las rutas de `MAX_PATH` acceso de más de (260 <xref:System.IO.PathTooLongException>caracteres) producen una excepción. Para obtener más información, consulte [compatibilidad con rutas largas](../../../migration-guide/retargeting/4.6.1-4.6.2.md#long-path-support).|.NET Framework 4.6.2|  
|`Switch.System.IO.Compression.`<br/>`DoNotUseNativeZipLibraryForDecompression`|Controla si la clase usa rutinas de sistema operativo nativas para <xref:System.IO.Compression.DeflateStream> la descompresión. `false`para usar las API nativas; para usar la <xref:System.IO.Compression.DeflateStream> implementación de. `true`|.NET Framework 4.7.2|
|`Switch.System.IO.Compression.ZipFile.`<br/>`UseBackslash`|Usa la barra diagonal inversa (\\"") en lugar de la barra diagonal ("/") como separador de <xref:System.IO.Compression.ZipArchiveEntry.FullName%2A?displayProperty=nameWithType> la ruta de acceso en la propiedad. Para obtener más información, [consulte mitigación: Separador](../../../migration-guide/mitigation-ziparchiveentry-fullname-path-separator.md)de la ruta de acceso ZipArchiveEntry. FullName.|.NET Framework 4.6.1|  
...
|`Switch.System.IO.`<br/>`UseLegacyPathHandling`|Controla si se usa la normalización de la ruta de acceso heredada <xref:System.IO.Path.GetDirectoryName%2A?displayProperty=nameWithType> y <xref:System.IO.Path.GetPathRoot%2A?displayProperty=nameWithType> los métodos y admiten las rutas de acceso de URI. Para más información, vea [Mitigación: Normalización](../../../migration-guide/mitigation-path-normalization.md) y [mitigación de rutas de acceso: Comprobaciones](../../../migration-guide/mitigation-path-colon-checks.md)de la ruta de acceso.|.NET Framework 4.6.2|
...
|`Switch.System.Net.`<br/>`DontEnableSchUseStrongCrypto`|Controla si las <xref:System.Net.ServicePointManager?displayProperty=nameWithType> clases <xref:System.Net.Security.SslStream?displayProperty=nameWithType> y pueden usar el protocolo SSL 3,0. Para más información, vea [Mitigación: protocolos TLS](../../../migration-guide/mitigation-tls-protocols.md).|.NET Framework 4.6|
...
|`Switch.System.Runtime.InteropServices.`<br/>`DoNotMarshalOutByrefSafeArrayOnInvoke`|Controla si los parámetros ByRef SAFEARRAY en los eventos de interoperabilidad com vuelven`false`a calcular las referencias a código nativo () o si el`true`cálculo de referencias en código nativo está deshabilitado ().|.NET Framework 4.8|
|`Switch.System.Runtime.Serialization.`<br/>`DoNotUseECMAScriptV6EscapeControlCharacter` |Controla si el [DataContractJsonSerializer](xref:System.Runtime.Serialization.Json.DataContractJsonSerializer) serializa algunos caracteres de control basados en los estándares de ECMAScript V6 y V8. Para más información, vea [Mitigación: Serialización de caracteres de control con DataContractJsonSerializer](../../../migration-guide/mitigation-serialization-control-characters.md)| .NET Framework 4.7 |
...
|`Switch.System.ServiceModel.`<br/>`DisableExplicitConnectionCloseHeader`|Al utilizar el transporte http con un servicio autohospedado, si se establece este `true` valor en, WCF omitirá una aplicación `Connection: close` agregando el encabezado a los encabezados de respuesta de una solicitud. Establecer este valor en `false` permite agregar el `Connection: close` encabezado a los encabezados de respuesta, lo que da como resultado el cierre del socket de la solicitud después de enviar una respuesta.|.NET Framework 4.6|
...
|`Switch.System.ServiceProcess.`<br/>`DontThrowExceptionsOnStart`|Controla si las excepciones producidas en el inicio del servicio se propagan al llamador <xref:System.ServiceProcess.ServiceBase.Run%2A?displayProperty=nameWithType> del método.|.NET Framework 4.7.1|
|`Switch.System.Threading.UseNetCoreTimer`|Controla si <xref:System.Threading.Timer> las instancias de aprovechan las mejoras de rendimiento de los entornos a gran escala. Si `true`es, se habilitan las mejoras de `false` rendimiento; si es (el valor predeterminado), se deshabilitan.|.NET Framework 4.8|
|`Switch.System.Uri.`<br/>`DontEnableStrictRFC3986ReservedCharacterSets`|Determina si ciertos caracteres con codificación porcentual que a veces se descodificaban se codifican ahora de forma coherente. Si `true`es, se descodifican; en caso `false`contrario,.|.NET Framework 4.7.2|
...
|`Switch.System.Windows.Data.Binding.`<br/>`IListIndexerHidesCustomIndexer`|Controla si la`false``true` claseusaindexadoresdeIListpersonalizadosdeformaincorrecta()o<xref:System.Windows.Data.Binding?displayProperty=nameWithType> correctamente ().|.NET Framework 4.8|
|`Switch.System.Windows.DoNotScaleForDpiChanges`|Determina si los cambios de PPP se producen en cada sistema (un valor `false`de) o cada monitor (un valor de `true`).|.NET Framework 4.6.2|
|`Switch.System.Windows.`<br/>`DoNotUsePresentationDpiCapabilityTier2OrGreater`|Controla si se pueden deshabilitar ( <xref:System.Windows.Interop.HwndHost?displayProperty=nameWithType> `true`) o habilitar (`false`) las mejoras en el tamaño de los controles de cuando se ejecuta WPF en el modo de reconocimiento de cada monitor.|.NET Framework 4.8|
|`Switch.System.Windows.Forms.`<br/>`DomainUpDown.UseLegacyScrolling`|Determina si el desarrollador necesita controlar de forma especial <xref:System.Windows.Forms.DomainUpDown.UpButton?displayProperty=nameWithType> la acción cuando el texto del control está presente. `true`para controlar la <xref:System.Windows.Forms.DomainUpDown.UpButton> acción; para que <xref:System.Windows.Forms.DomainUpDown.UpButton?displayProperty=nameWithType> las <xref:System.Windows.Forms.DomainUpDown.DownButton?displayProperty=nameWithType> acciones y estén sincronizadas correctamente. `false`|.NET Framework 4.7.2|
|`Switch.System.Windows.Forms.`<br />`DontSupportReentrantFilterMessage`|Opta por el código que permite a una implementación <xref:System.Windows.Forms.IMessageFilter.PreFilterMessage%2A?displayProperty=nameWithType> personalizada filtrar mensajes de forma segura sin producir una excepción <xref:System.Windows.Forms.Application.FilterMessage%2A?displayProperty=nameWithType> cuando se llama al método. Para más información, vea [Mitigación: Implementaciones de IMessageFilter. PreFilterMessage](../../../migration-guide/mitigation-custom-imessagefilter-prefiltermessage-implementations.md)personalizadas.|.NET Framework 4.6.1|  
...
|`Switch.System.Windows.Forms.UseLegacyToolTipDisplay`|Controla si la compatibilidad con la invocación de`true`la información sobre herramientas`false`está deshabilitada () o habilitada (). Habilitar la compatibilidad con la invocación de la información sobre herramientas `Switch.UseLegacyAccessibilityFeatures`también requiere que `Switch.UseLegacyAccessibilityFeatures.3` las características de accesibilidad heredadas `false`definidas por, `Switch.UseLegacyAccessibilityFeatures.2`y estén deshabilitadas (establecidas en).|.NET Framework 4.8|
|`Switch.System.Windows.Input.Stylus.`<br/>`EnablePointerSupport`|Determina si una pila `WM_POINTER`de entrada táctil o de lápiz basada en opcional está habilitada en las aplicaciones WPF. Para más información, vea [Mitigación: Compatibilidad con el lápiz y el toque basados en puntero](../../../migration-guide/mitigation-pointer-based-touch-and-stylus-support.md)|.NET Framework 4.7|
|`Switch.System.Windows.Markup.`<br/>`DoNotUseSha256ForMarkupCompilerChecksumAlgorithm`|Determina si el algoritmo hash predeterminado utilizado para las sumas de comprobación`false`es SHA256 ()`true`o SHA1 ().<br>Debido a problemas de colisión con SHA1, Microsoft recomienda SHA256.|.NET Framework 4.7.2|
...
|`Switch.System.Workflow.Runtime.`<br/>`UseLegacyHashForWorkflowDefinitionDispenserCacheKey`|Controla si la suma de comprobación hash del tiempo de ejecución del flujo de`true`trabajo usa el algoritmo MD5 () para las definiciones de flujo de trabajo almacenadas en caché o si usa el algoritmo SHA256 introducido como valor predeterminado en .NET Framework 4,8.<br>Debido a problemas de colisión con MD5, Microsoft recomienda SHA256.|.NET Framework 4.8|
...
|`Switch.UseLegacyAccessibilityFeatures.2`|Controla si las características de accesibilidad disponibles en .NET Framework 4.7.2 están`false`habilitadas ()`true`o deshabilitadas (). Si `true` `Switch.UseLegacyAccessibilityFeatures` es ,`true` también debe habilitar .NET Framework 4.7.1 características de accesibilidad.|.NET Framework 4.7.2|
|`Switch.UseLegacyAccessibilityFeatures.3`|Controla si las características de accesibilidad introducidas en .NET Framework 4,8`false`están habilitadas (`true`) o deshabilitadas (). Si `true`, `Switch.UseLegacyAccessibilityFeatures` y también`Switch.UseLegacyAccessibilityFeatures.2` deben ser `true`.|.NET Framework 4.8|
|`Switch.UseLegacyToolTipDisplay`|Controla si se muestra información sobre herramientas cuando un usuario mantiene el cursor del mouse sobre un control de`true`WPF () o si se muestran en el foco del teclado y a través de la`false`tecla de método abreviado de teclado (, el comportamiento predeterminado). En el caso de las aplicaciones que se ejecutan en .NET Framework 4,8 pero que tienen como destino versiones anteriores del .NET Framework, la habilitación de la `Switch.UseLegacyAccessibilityFeatures.3` compatibilidad con el foco de teclado y la tecla de método abreviado requiere que `Switch.UseLegacyAccessibilityFeatures`, `Switch.UseLegacyAccessibilityFeatures.2`y se establezcan en `false`.|.NET Framework 4.8|
...
Puede configurar una aplicación de ASP.net para usar la configuración de compatibilidad agregando un [ \<elemento Add >](../appsettings/add-element-for-appsettings.md) a la [ \<sección > appSettings](../appsettings/index.md) del archivo Web. config. 

En el ejemplo siguiente se `<add>` usa el elemento para agregar dos valores `<appSettings>` a la sección de un archivo Web. config:
...
## <a name="example"></a>Ejemplo
 ...
 En el ejemplo siguiente se `AppContextSwitchOverrides` usa el elemento para definir un solo modificador `Switch.System.Globalization.NoAsyncCurrentCulture`de compatibilidad de aplicaciones,, que impide que la referencia cultural fluya entre subprocesos en llamadas asincrónicas a métodos.  
...
```  
 ...
 En el ejemplo siguiente se `AppContextSwitchOverrides` usa el elemento para definir dos modificadores `Switch.System.Globalization.NoAsyncCurrentCulture` de `Switch.System.IO.BlockLongPaths`compatibilidad de aplicaciones, y. Tenga en cuenta que un punto y coma separa los dos pares de nombre y valor.  
