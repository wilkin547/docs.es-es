...
- El único formato admitido para el atributo `href` es `file://`. Se admiten los archivos locales y los archivos UNC.

- No hay ninguna restricción en el número de configuraciones vinculadas por archivo de configuración.

- Todos los archivos de configuración vinculados se combinan para formar un archivo, de forma similar al comportamiento de la Directiva `#include`C++en C/.

- El elemento **\<linkedConfiguration >** solo se permite en los archivos de configuración de la aplicación; se omite en *Machine. config*.

- Las referencias circulares se detectan y finalizan. Es decir, si el **\<linkedConfiguration >** elementos de una serie de archivos de configuración forman un bucle, el bucle se detecta y se detiene.
