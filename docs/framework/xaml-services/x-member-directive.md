...
|`memberName`|Nombre del miembro de la propiedad que se está definiendo.|  
  
## Comentarios  
 En la implementación de los servicios XAML de .NET Framework,  `x:Member` no tiene un respaldo de tipos directo pero es compatible con la clase <xref:System.Windows.Markup.MemberDefinition>.  En un flujo de nodo XAML, un elemento `x:Member` se representa como un miembro llamado `Member`, del espacio de nombres XAML de lenguaje XAML.  El miembro `Member` contiene atributos tal y como declara el marcado.  
