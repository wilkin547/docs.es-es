...
> [!NOTE]
> Los valores predeterminados de las propiedades no se fuerzan. Es posible que se produzca un valor de propiedad igual al valor predeterminado si un valor de propiedad todavía tiene su valor predeterminado inicial o si <xref:System.Windows.DependencyObject.ClearValue%2A>se borran otros valores con.  
...
### <a name="using-coercevalue-to-cancel-value-changes"></a>Uso de CoerceValue para cancelar cambios de valor  
 El sistema de propiedades tratará <xref:System.Windows.CoerceValueCallback> cualquier que devuelva <xref:System.Windows.DependencyProperty.UnsetValue> el valor como un caso especial. Este caso especial significa que el sistema de propiedades debe rechazar el <xref:System.Windows.CoerceValueCallback> cambio de propiedad que dio como resultado la llamada, y que el sistema de propiedades debe notificar en su lugar el valor anterior que tenía la propiedad. Este mecanismo puede ser útil para comprobar que los cambios en una propiedad iniciados de manera asincrónica siguen siendo válidos para el estado actual del objeto y suprimirlos si no lo son. Otro escenario posible es la posibilidad de suprimir de manera selectiva un valor según el componente de determinación del valor propiedad que sea responsable del valor comunicado. Para ello, puede usar el <xref:System.Windows.DependencyProperty> pasado en la devolución de llamada y el identificador de propiedad como entrada para <xref:System.Windows.DependencyPropertyHelper.GetValueSource%2A>y, a continuación, procesar. <xref:System.Windows.ValueSource>  
