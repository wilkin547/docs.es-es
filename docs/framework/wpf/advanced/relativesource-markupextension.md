...
`{RelativeSource Self}` es útil para escenarios en los que una propiedad de una instancia debe depender del valor de otra propiedad de la misma instancia, y no existe ninguna relación de propiedad de dependencia general (como la coerción) entre esas dos propiedades. Aunque es poco frecuente que existan dos propiedades en un objeto de modo que los valores sean literalmente idénticos (y tengan tipos idénticos), también puede aplicar un `Converter` parámetro a un enlace que tenga `{RelativeSource Self}`y usar el convertidor para realizar la conversión entre el origen y el destino. distintos. Otro escenario para `{RelativeSource Self}` es como parte de un <xref:System.Windows.MultiDataTrigger>.
