...
## <a name="xaml-syntax-in-brief"></a>Resumen de sintaxis XAML  
 ...
 Gran parte del material de las secciones siguientes será básico para usted si está familiarizado con el lenguaje XML. Esta es una consecuencia de uno de los principios de diseño básicos de XAML.  El lenguaje XAML define conceptos propios, pero estos conceptos funcionan dentro del formulario de lenguaje y marcado XML.  
...
### <a name="collection-syntax"></a>Sintaxis de colección  
 ...
 El ejemplo siguiente muestra la sintaxis de colección para establecer valores de la <xref:System.Windows.Media.GradientBrush.GradientStops%2A> propiedad:  
...
### <a name="content-properties-and-collection-syntax-combined"></a>Sintaxis de las propiedades y colección contenido combinado  
...
-   **Elemento de propiedad StackPanel.Children omitido:** <xref:System.Windows.Controls.StackPanel> deriva <xref:System.Windows.Controls.Panel>. <xref:System.Windows.Controls.Panel> define <xref:System.Windows.Controls.Panel.Children%2A?displayProperty=nameWithType> como su XAML propiedad de contenido.  
  
-   **Elemento de objeto UIElementCollection omitido:** El <xref:System.Windows.Controls.Panel.Children%2A?displayProperty=nameWithType> propiedad toma el tipo <xref:System.Windows.Controls.UIElementCollection>, que implementa <xref:System.Collections.IList>. Se puede omitir una etiqueta de elemento de la colección, basándose en las reglas XAML para procesar colecciones como <xref:System.Collections.IList>. (En este caso, <xref:System.Windows.Controls.UIElementCollection> realmente no pueden crearse instancias porque no expone un constructor predeterminado y, por eso la <xref:System.Windows.Controls.UIElementCollection> elemento de objeto se muestra marcado como comentario).  
...
## <a name="markup-extensions"></a>Extensiones de marcado  
 ...
 Por ejemplo, el siguiente marcado establece el valor de la <xref:System.Windows.FrameworkElement.Style%2A> propiedad mediante la sintaxis de atributo. El <xref:System.Windows.FrameworkElement.Style%2A> propiedad toma una instancia de la <xref:System.Windows.Style> (clase), que no se pueden crear instancias mediante una cadena de sintaxis de atributo de forma predeterminada. Pero en este caso, el atributo hace referencia a una extensión de marcado determinada, [StaticResource](staticresource-markup-extension.md). Cuando se procesa dicha extensión de marcado, devuelve una referencia a un estilo del que ya se han creado instancias como un recurso con clave en un diccionario de recursos.  
  
 [!code-xaml[FEResourceSH_snip#XAMLOvwShortResources](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources)]  
[!code-xaml[FEResourceSH_snip#XAMLOvwShortResources2](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources2)]  
[!code-xaml[FEResourceSH_snip#XAMLOvwShortResources3](~/samples/snippets/csharp/VS_Snippets_Wpf/FEResourceSH_snip/CS/page1.xaml#xamlovwshortresources3)]  
  
 Para obtener una lista de referencias de todas las extensiones de marcado para XAML implementadas específicamente en WPF, vea [Extensiones XAML de WPF](wpf-xaml-extensions.md). Para obtener una lista de referencia de las extensiones de marcado definidas por System.Xaml y están más disponibles para las implementaciones de XAML de .NET Framework, vea [XAML Namespace (x:) Características del lenguaje](../../xaml-services/xaml-namespace-x-language-features.md). Para obtener más información sobre los conceptos de la extensión de marcado, vea [Extensiones de marcado y XAML de WPF](markup-extensions-and-wpf-xaml.md).  
  
<a name="type_converters"></a>   
## <a name="type-converters"></a>Convertidores de tipos  
 En la sección [Resumen de sintaxis de XAML](xaml-overview-wpf.md#xaml_syntax_in_brief), se ha mencionado que el valor de atributo debe poder establecerlo una cadena. El control nativo básico de cómo se convierten cadenas en otros tipos de objetos o valores primitivos se basa en el <xref:System.String> propio tipo, procesamiento además a código nativo para ciertos tipos como <xref:System.DateTime> o <xref:System.Uri>. Pero muchos tipos o miembros de [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] de esos tipos extienden el comportamiento básico del procesamiento de atributos de cadena, de tal forma que se pueden especificar instancias de tipos de objeto más complejos como cadenas y atributos.  
  
 El <xref:System.Windows.Thickness> estructura es un ejemplo de un tipo que tiene una conversión de tipos habilitada para los usos de XAML. <xref:System.Windows.Thickness> indica las medidas dentro de un rectángulo anidado y se usa como el valor para las propiedades como <xref:System.Windows.FrameworkElement.Margin%2A>. Al colocar un convertidor de tipos en <xref:System.Windows.Thickness>, todas las propiedades que usan un <xref:System.Windows.Thickness> son más fáciles de especificar en XAML porque se pueden especificar como atributos. En el ejemplo siguiente se usa una sintaxis de conversión y el atributo de tipo para proporcionar un valor para un <xref:System.Windows.FrameworkElement.Margin%2A>:  
  
 [!code-xaml[XAMLOvwSupport#MarginTCE](~/samples/snippets/csharp/VS_Snippets_Wpf/XAMLOvwSupport/CSharp/page7.xaml#margintce)]  
  
 El ejemplo de sintaxis de atributo anterior es equivalente al siguiente ejemplo de sintaxis más detallado, donde el <xref:System.Windows.FrameworkElement.Margin%2A> se establece mediante la sintaxis de elemento de propiedad que contiene un <xref:System.Windows.Thickness> elemento de objeto. Propiedades de clave de los cuatro <xref:System.Windows.Thickness> se establecen como atributos en la nueva instancia:  
...
## <a name="xaml-root-elements-and-xaml-namespaces"></a>Los elementos raíz XAML y espacios de nombres XAML  
 Para que un archivo XAML pueda ser tanto un archivo [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] con un formato correcto como un archivo XAML válido, solo debe tener un elemento raíz. Para los escenarios típicos de WPF, usa un elemento raíz que tiene un significado destacado en el modelo de aplicación de WPF (por ejemplo, <xref:System.Windows.Window> o <xref:System.Windows.Controls.Page> para una página, <xref:System.Windows.ResourceDictionary> para un diccionario externo, o <xref:System.Windows.Application> para la definición de aplicación). El ejemplo siguiente muestra el elemento raíz de un archivo XAML típico para un [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] página con el elemento raíz de <xref:System.Windows.Controls.Page>.  
...
[!code-xaml[XAMLOvwSupport#RootOnly2](~/samples/snippets/csharp/VS_Snippets_Wpf/XAMLOvwSupport/CSharp/page2.xaml#rootonly2)]  
 ...
 Observe que los atributos `xmlns` solo son estrictamente necesarios en el elemento raíz de cada archivo XAML. `xmlns` las definiciones se aplicarán a todos los elementos descendientes del elemento raíz (este comportamiento nuevo es coherente con la especificación XML 1.0 para `xmlns`.) `xmlns` atributos también se permiten en otros elementos bajo la raíz y se aplicará a cualquier elemento descendiente del elemento de definición. En cambio, una definición o redefinición frecuente de los espacios de nombres XAML puede dar lugar a un estilo de marcado XAML que resulte difícil de leer.  
...
-   [x:Key](../../xaml-services/x-key-directive.md): Establece una clave única para cada recurso en un <xref:System.Windows.ResourceDictionary> (o conceptos de diccionario similares en otros marcos). `x:Key` probablemente el 90% de la `x:` usos que verá en el marcado de una aplicación WPF típica.  
  
-   [x: Class](../../xaml-services/x-class-directive.md): Especifica el [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] espacio de nombres y el nombre de la clase que proporciona código subyacente para una página XAML. Debe disponer de esta clase para admitir el código subyacente por el modelo de programación WPF; por esto casi siempre verá `x:` asignado, aunque no haya ningún recurso.  
  
-   [x: Name](../../xaml-services/x-name-directive.md): Especifica un nombre de objeto de tiempo de ejecución para la instancia que existe en el código de tiempo de ejecución después de procesa un elemento de objeto. En general, usará frecuentemente una propiedad equivalente definida por WPF para [x:Name](../../xaml-services/x-name-directive.md). Tales propiedades se asignan específicamente a una propiedad de respaldo de CLR y así son más útiles para la programación de aplicaciones, donde frecuentemente se usa código del tiempo de ejecución para encontrar los elementos con nombre de XAML inicializado. La más común de esas propiedades es <xref:System.Windows.FrameworkElement.Name%2A?displayProperty=nameWithType>. Todavía podría usar [x: Name](../../xaml-services/x-name-directive.md) cuando el marco WPF equivalente-nivel <xref:System.Windows.FrameworkElement.Name%2A> propiedad no se admite en un tipo determinado. Esto sucede en ciertos escenarios de animación.  
  
-   [x:Static](../../xaml-services/x-static-markup-extension.md): Habilita una referencia que devuelve un valor estático que en caso contrario, no es una propiedad compatible con XAML.  
  
-   [x:Type](../../xaml-services/x-type-markup-extension.md): Construye un <xref:System.Type> referencia basada en un nombre de tipo. Esto se utiliza para especificar atributos que toman <xref:System.Type>, tales como <xref:System.Windows.Style.TargetType%2A?displayProperty=nameWithType>, aunque con frecuencia la propiedad tiene la cadena nativa-a-<xref:System.Type> conversión de manera que el [x: Type](../../xaml-services/x-type-markup-extension.md) es el uso de la extensión de marcado opcional.  
  
 Hay construcciones de programación adicionales en el prefijo o espacio de nombres XAML `x:` que no son tan habituales. Para obtener más información, consulte [XAML Namespace (x:) Características del lenguaje](../../xaml-services/xaml-namespace-x-language-features.md).  
...
## <a name="attached-properties-and-attached-events"></a>Las propiedades adjuntas y eventos adjuntos  
 ...
 El ejemplo siguiente ilustra la <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> propiedad adjunta. El <xref:System.Windows.Controls.DockPanel> clase define los descriptores de acceso <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> y, por tanto, tiene la propiedad adjunta. El <xref:System.Windows.Controls.DockPanel> clase también incluye la lógica que recorre en iteración sus elementos secundarios y específicamente comprueba si cada elemento tiene un valor de conjunto de <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType>. Si se encuentra dicho valor, este se usa durante el diseño para colocar los elementos secundarios. El uso de la <xref:System.Windows.Controls.DockPanel.Dock%2A?displayProperty=nameWithType> propiedad adjunta y esta función de posición es en realidad el escenario de motivación para la <xref:System.Windows.Controls.DockPanel> clase.  
...
## <a name="whats-next"></a>Pasos adicionales  
 ...
 Si aún no lo ha hecho, intente realizar los ejercicios del tutorial [Tutorial: Mi primera aplicación de escritorio de WPF](../getting-started/walkthrough-my-first-wpf-desktop-application.md). Crear la aplicación centrada en el marcado descrita en el tutorial le ayudará a reforzar muchos de los conceptos descritos en este tema.  
  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] usa un modelo de aplicación determinado que se basa en el <xref:System.Windows.Application> clase. Para obtener información detallada, vea [Información general sobre la administración de aplicaciones](../app-development/application-management-overview.md).  
