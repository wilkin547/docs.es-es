...
Algunas propiedades de estilo y plantilla, <xref:System.Windows.Setter.Property%2A?displayProperty=nameWithType> como toman un nombre de propiedad completo que se parece superficialmente a. <xref:System.Windows.PropertyPath> Pero esto <xref:System.Windows.PropertyPath>no es cierto, sino que es un propietario calificado. uso del formato de cadena de *propiedad* que está habilitado por [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] el procesador de WPF en combinación con el <xref:System.Windows.DependencyProperty>convertidor de tipos de.
...
Tenga en cuenta que el [!INCLUDE[TLA#tla_xml](../../../../includes/tlasharptla-xml-md.md)] enlace de datos <xref:System.Windows.PropertyPath>a no usa, porque no <xref:System.Windows.Data.Binding.Path%2A> utiliza en <xref:System.Windows.Data.Binding>. En su lugar, use <xref:System.Windows.Data.Binding.XPath%2A> y especifique la sintaxis de XPath válida [!INCLUDE[TLA#tla_xmldom](../../../../includes/tlasharptla-xmldom-md.md)] en el de los datos. <xref:System.Windows.Data.Binding.XPath%2A>también se especifica como una cadena, pero no se documenta aquí. vea [enlazar a datos XML mediante XmlDataProvider y consultas XPath](../data/how-to-bind-to-xml-data-using-an-xmldataprovider-and-xpath-queries.md).
...
Los paréntesis indican que esta propiedad de <xref:System.Windows.PropertyPath> se debe construir usando una calificación parcial. Puede usar un espacio de nombres XML para buscar el tipo con la asignación adecuada. Busca en los tipos a [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] los que un procesador tiene acceso, <xref:System.Windows.Markup.XmlnsDefinitionAttribute> a través de las declaraciones de cada ensamblado. `ownerType` La mayoría de las aplicaciones tienen el espacio de nombres XML predeterminado asignado al espacio de nombres [!INCLUDE[TLA#tla_wpfxmlnsv1](../../../../includes/tlasharptla-wpfxmlnsv1-md.md)], de modo que, normalmente, solo es necesario para los tipos personalizados o para tipos que queden fuera del espacio de nombres.  `propertyName` debe resolverse para ser el nombre de una propiedad que exista en `ownerType`. Esta sintaxis se usa, normalmente, para uno de los siguientes casos:
...
> [!NOTE]
> Esta sintaxis se parece ligeramente a [!INCLUDE[TLA2#tla_xpath](../../../../includes/tla2sharptla-xpath-md.md)]. Una expresión [!INCLUDE[TLA2#tla_xpath](../../../../includes/tla2sharptla-xpath-md.md)] true para enlazar a [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] un origen de datos no <xref:System.Windows.Data.Binding.Path%2A> se utiliza como valor y debe utilizarse en <xref:System.Windows.Data.Binding.XPath%2A> su lugar para la propiedad mutuamente excluyente.
...
`propertyName`debe ser una propiedad que sea un <xref:System.Windows.Freezable> tipo de valor o un primitivo, que existe en el tipo especificado. <xref:System.Windows.Media.Animation.Storyboard.TargetName%2A>

`propertyName2` debe ser el nombre de una propiedad de dependencia que exista en el objeto que es el valor de `propertyName`. En otras palabras, `propertyName2` debe existir como una propiedad de dependencia en el tipo que `propertyName` <xref:System.Windows.DependencyProperty.PropertyType%2A>es.
...
Los paréntesis indican que esta propiedad de <xref:System.Windows.PropertyPath> se debe construir usando una calificación parcial. Puede usar un espacio de nombres XML para buscar el tipo. Busca en los tipos a [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] los que un procesador tiene acceso, <xref:System.Windows.Markup.XmlnsDefinitionAttribute> a través de las declaraciones de cada ensamblado. `ownerType` La mayoría de las aplicaciones tienen el espacio de nombres XML predeterminado asignado al espacio de nombres [!INCLUDE[TLA#tla_wpfxmlnsv1](../../../../includes/tlasharptla-wpfxmlnsv1-md.md)], de modo que, normalmente, solo es necesario para los tipos personalizados o para tipos que queden fuera del espacio de nombres. `propertyName` debe resolverse para ser el nombre de una propiedad que exista en `ownerType`. La propiedad especificada como `propertyName` debe <xref:System.Windows.DependencyProperty>ser. (Todas las propiedades adjuntas [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] se implementan como propiedades de dependencia, por lo que este problema solo afecta a las propiedades adjuntas personalizadas).
