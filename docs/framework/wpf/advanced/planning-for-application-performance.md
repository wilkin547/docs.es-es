...
## <a name="make-performance-tuning-an-iterative-process"></a>Realizar un proceso iterativo de optimización del rendimiento  
 Debe saber el costo relativo de cada característica que se va a usar. Por ejemplo, el uso de reflexión en Microsoft .NET Framework suele ser rendimiento intensivo en cuanto a los recursos informáticos, por lo que desea usarlo con prudencia. Esto no significa para evitar el uso de reflexión, sólo que debe tener cuidado equilibrar los requisitos de rendimiento de la aplicación con las exigencias de rendimiento de las características que usa.  
