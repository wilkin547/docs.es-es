...
## <a name="extension-classes"></a>\*Clases de extensión  
 Para el lenguaje XAML general y las extensiones de marcado específicas de WPF, el comportamiento de cada extensión de marcado se identifica en un procesador XAML a través de una `*Extension` clase que deriva de <xref:System.Windows.Markup.MarkupExtension>, y proporciona una implementación del <xref:System.Windows.Markup.StaticExtension.ProvideValue%2A> método. Este método de cada extensión proporciona el objeto que se devuelve cuando se evalúa la extensión de marcado. El objeto devuelto se suele evaluar según los distintos token de cadena que se pasan a la extensión de marcado.  
  
 Por ejemplo, <xref:System.Windows.StaticResourceExtension> la clase proporciona la implementación <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A> de superficie de búsqueda de recursos real para que su implementación devuelva el `x:Key`objeto que se solicita, con la entrada de esa implementación concreta que es una cadena que se usa para buscar el recurso por su . Gran parte de este detalle de implementación no es importante si usa una extensión de marcado existente.  
 ...
 El patrón de nomenclatura `*Extension` es por comodidad y coherencia. No es necesario para que un procesador XAML identifique esa clase como compatible para una extensión de marcado. Siempre que el código base incluya System.Xaml y use implementaciones de servicios XAML de .NET Framework, <xref:System.Windows.Markup.MarkupExtension> todo lo que es necesario reconocer como una extensión de marcado XAML es derivar de y admitir una sintaxis de construcción. WPFWPF define las clases que `*Extension` habilitan la <xref:System.Windows.Data.Binding>extensión de marcado que no siguen el patrón de nomenclatura, por ejemplo. Normalmente, el motivo es que la clase admite escenarios más allá de la compatibilidad de extensión de marcado pura. En el <xref:System.Windows.Data.Binding>caso de , esa clase admite el acceso en tiempo de ejecución a métodos y propiedades del objeto para escenarios que no tienen nada que ver con XAML.  
...
- Si hay un resultado paralelo entre el comportamiento del constructor y el comportamiento de la configuración de propiedades en una extensión de marcado, no importa qué comportamiento use. Es más común usar los pares de*valores* de *propiedad*`=`para las extensiones de marcado que tienen más de una propiedad configurable, aunque solo sea porque hace que el marcado sea más intencional y es menos probable que transponga accidentalmente parámetros de constructor. (Cuando se especifican pares propiedad-valor, esas propiedades pueden estar en cualquier orden.) Además, no hay ninguna garantía de que una extensión de marcado proporciona un parámetro constructor que establece cada una de sus propiedades configurables. Por ejemplo, <xref:System.Windows.Data.Binding> es una extensión de marcado, con muchas propiedades que <xref:System.Windows.Data.Binding> se pueden establecer a través de la extensión en forma de*valor* de *propiedad,*`=`pero solo admite dos constructores: un constructor sin parámetros y uno que establece una ruta de acceso inicial.  
...
## <a name="escape-sequences-and-markup-extensions"></a>Secuencias de escape y extensiones de marcado  
 El control de atributos en un procesador XAML usa las llaves como indicadores de una secuencia de extensión de marcado. También es posible generar un valor de atributo de caracteres literales de llave si es necesario, escribiendo una secuencia de escape con un par de llaves vacío seguido de la llave de cierre literal. Consulte [ {} Secuencia de escape - Extensión de marcado](../../../desktop-wpf/xaml-services/escape-sequence-markup-extension.md).  
