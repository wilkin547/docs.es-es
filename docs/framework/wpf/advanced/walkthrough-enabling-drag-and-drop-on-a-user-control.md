...
## <a name="creating-the-application-project"></a>Creación del proyecto de la aplicación  
 En esta sección, aprenderá a crear la infraestructura de aplicación, que incluye una página principal con dos paneles y un <xref:System.Windows.Controls.TextBox>.  
  
### <a name="to-create-the-project"></a>Para crear el proyecto  
  
1.  Cree un proyecto de aplicación de WPF en Visual Basic o Visual C# denominado `DragDropExample`. Para obtener más información, vea [Cómo: Crear un nuevo proyecto de aplicación de WPF](http://msdn.microsoft.com/library/1f6aea7a-33e1-4d3f-8555-1daa42e95d82).  
  
2.  Abra MainWindow.xaml.  
  
3.  Agregue el siguiente marcado entre la apertura y cierre <xref:System.Windows.Controls.Grid> etiquetas.  
...
4.  Agregue el marcado siguiente a la raíz <xref:System.Windows.Controls.Grid> para crear un control de usuario simple que tiene un círculo azul como su interfaz de usuario.  
...
2.  Agregue el siguiente código XAML para la apertura <xref:System.Windows.Window> etiqueta que se va a crear una referencia al espacio de nombres XML para la aplicación actual.  
  
    ```  
    xmlns:local="clr-namespace:DragDropExample"  
    ```  
  
3.  En la primera <xref:System.Windows.Controls.StackPanel>, agregue el siguiente código XAML para crear dos instancias del control de usuario de círculo en el primer panel.  
...
## <a name="implementing-drag-source-events-in-the-user-control"></a>Implementar eventos del origen de arrastre en el control de usuario  
 En esta sección, invalidará el <xref:System.Windows.UIElement.OnMouseMove%2A> método y: iniciar la operación de arrastrar y colocar.  
  
 Si se ha iniciado un arrastre (se presiona un botón del mouse y se mueve el mouse), se empaqueta los datos que se va a transferirse a un <xref:System.Windows.DataObject>. En este caso, el control de círculo empaquetará tres elementos de datos: una representación de cadena de su color de relleno, una representación doble de su altura y una copia de sí mismo.  
...
2.  Agregue el siguiente <xref:System.Windows.UIElement.OnMouseMove%2A> invalidación para proporcionar control de clases para el <xref:System.Windows.UIElement.MouseMove> eventos.  
    ...
    -   Llama el método estático <xref:System.Windows.DragDrop.DoDragDrop%2A?displayProperty=nameWithType> método para iniciar la operación de arrastrar y colocar. Pasar los tres parámetros siguientes para el <xref:System.Windows.DragDrop.DoDragDrop%2A> método:  
...
4.  Haga clic en uno de los controles de círculo y arrastre sobre los paneles, del círculo y el <xref:System.Windows.Controls.TextBox>. Al arrastrar el puntero sobre el <xref:System.Windows.Controls.TextBox>, el cursor cambia para indicar un movimiento.  
...
6.  Arrastre y coloque un círculo en el <xref:System.Windows.Controls.TextBox>. La representación de cadena del color de relleno del círculo se anexa a la <xref:System.Windows.Controls.TextBox>.  
 ...
 De forma predeterminada, el cursor cambiará durante una operación de arrastrar y colocar para indicar el efecto que tendrá la colocación de los datos. Puede personalizar los mensajes de estado al usuario controlando el <xref:System.Windows.UIElement.GiveFeedback> eventos y la configuración de un cursor diferente.  
...
4.  Arrastre uno del círculo controles sobre los paneles, del círculo y el <xref:System.Windows.Controls.TextBox>. Observe que los cursores son ahora los cursores personalizados que ha especificado en el <xref:System.Windows.UIElement.OnGiveFeedback%2A> invalidar.  
...
2.  En la apertura <xref:System.Windows.Controls.UserControl> etiqueta, agregue el <xref:System.Windows.UIElement.AllowDrop%2A> propiedad y establézcalo en `true`.  
  
     [!code-xaml[DragDropWalkthrough#UCTagXAML](../../../../samples/snippets/csharp/VS_Snippets_Wpf/DragDropWalkthrough/CS/Circle.xaml#uctagxaml)]  
  
 El <xref:System.Windows.UIElement.OnDrop%2A> se llama al método cuando el <xref:System.Windows.UIElement.AllowDrop%2A> propiedad está establecida en `true` y datos desde el origen de arrastre se colocan en el control de usuario de círculo. En este método, procesará los datos que se han colocado y aplicará los datos al círculo.  
...
2.  Agregue el siguiente <xref:System.Windows.UIElement.OnDrop%2A> invalidación para proporcionar control de clases para el <xref:System.Windows.UIElement.Drop> eventos.  
    ...
    -   Usa el <xref:System.Windows.DataObject.GetDataPresent%2A> método para comprobar si los datos arrastrados contienen un objeto string.  
...
9. Arrastre desde el control de círculo verde y coloque en el control de círculo azul. El círculo cambia de azul a verde. Tenga en cuenta que el cursor que se muestra depende de si el <xref:System.Windows.Controls.TextBox> o el círculo es el origen de arrastre.  
 ...
 Establecer el <xref:System.Windows.UIElement.AllowDrop%2A> propiedad `true` y procesar los datos perdidos es todo lo que es necesario para habilitar un elemento que se va a ser un destino de colocación. Sin embargo, para proporcionar una mejor experiencia de usuario, también debe controlar la <xref:System.Windows.UIElement.DragEnter>, <xref:System.Windows.UIElement.DragLeave>, y <xref:System.Windows.UIElement.DragOver> eventos. En estos eventos, puede realizar comprobaciones y proporcionar repuesta adicional al usuario antes de que se coloquen los datos.  
  
 Cuando los datos se arrastran sobre el control de usuario de círculo, el control debe notificarle al origen de arrastre si puede procesar los datos que se están arrastrando. Si el control no sabe cómo procesar los datos, debe rechazar la operación de colocar. Para ello, controlará la <xref:System.Windows.UIElement.DragOver> eventos y establezca la <xref:System.Windows.DragEventArgs.Effects%2A> propiedad.  
...
2.  Agregue el siguiente <xref:System.Windows.UIElement.OnDragOver%2A> invalidación para proporcionar control de clases para el <xref:System.Windows.UIElement.DragOver> eventos.  
    ...
    -   Si el control de usuario pueda procesar los datos, Establece la <xref:System.Windows.DragEventArgs.Effects%2A> propiedad <xref:System.Windows.DragDropEffects.Copy> o <xref:System.Windows.DragDropEffects.Move>.  
...
5.  Arrastre el texto a un control de círculo. Observe que ahora el cursor cambia para indicar que no se permite la colocación porque `gre` no es un color válido.  
 ...
 Puede mejorar la experiencia del usuario si aplica una vista previa de la operación de colocar. Para el control de usuario de círculo, deberá reemplazar el <xref:System.Windows.UIElement.OnDragEnter%2A> y <xref:System.Windows.UIElement.OnDragLeave%2A> métodos. Cuando se arrastran los datos sobre el control, el fondo actual <xref:System.Windows.Shapes.Shape.Fill%2A> se guarda en una variable de marcador de posición. La cadena se convierte en un pincel, a continuación y se aplica a la <xref:System.Windows.Shapes.Ellipse> que proporciona el círculo de la interfaz de usuario. Si los datos se arrastran fuera del círculo pero no se coloca el original <xref:System.Windows.Shapes.Shape.Fill%2A> valor se vuelve a aplicar al círculo.  
...
2.  En la clase de círculo, declare una privada <xref:System.Windows.Media.Brush> variable denominada `_previousFill` e inicialícela en `null`.  
...
3.  Agregue el siguiente <xref:System.Windows.UIElement.OnDragEnter%2A> invalidación para proporcionar control de clases para el <xref:System.Windows.UIElement.DragEnter> eventos.  
    ...
    -   Realiza las mismas comprobaciones que se realizan en el <xref:System.Windows.UIElement.OnDrop%2A> método para determinar si los datos se pueden convertir a un <xref:System.Windows.Media.Brush>.  
...
4.  Agregue el siguiente <xref:System.Windows.UIElement.OnDragLeave%2A> invalidación para proporcionar control de clases para el <xref:System.Windows.UIElement.DragLeave> eventos.  
    ...
    -   Se aplica el <xref:System.Windows.Media.Brush> guardados en el `_previousFill` variable a la <xref:System.Windows.Shapes.Shape.Fill%2A> de la <xref:System.Windows.Shapes.Ellipse> que proporciona la interfaz de usuario del control de usuario de círculo.  
...
4.  Agregue el código siguiente para el <xref:System.Windows.UIElement.DragOver> controlador de eventos.  
     ...
     Esto <xref:System.Windows.UIElement.DragOver> controlador de eventos realiza las tareas siguientes:  
  
    -   Comprueba que los datos arrastrados contienen los datos de "Object" que se empaquetan en la <xref:System.Windows.DataObject> el control de usuario de círculo y se pasa en la llamada a <xref:System.Windows.DragDrop.DoDragDrop%2A>.  
  
    -   Si los datos "Object" están presentes, comprueba si se ha presionado la tecla Ctrl.  
  
    -   Si se presiona la tecla CTRL, Establece la <xref:System.Windows.DragEventArgs.Effects%2A> propiedad <xref:System.Windows.DragDropEffects.Copy>. En caso contrario, Establece la <xref:System.Windows.DragEventArgs.Effects%2A> propiedad <xref:System.Windows.DragDropEffects.Move>.  
...
5.  Agregue el código siguiente para el <xref:System.Windows.UIElement.Drop> controlador de eventos.  
     ...
     Esto <xref:System.Windows.UIElement.Drop> controlador de eventos realiza las tareas siguientes:  
  
    -   Comprueba si el <xref:System.Windows.UIElement.Drop> ya se controló el evento. Por ejemplo, si se quita un círculo en otro círculo que controla el <xref:System.Windows.UIElement.Drop> eventos, no desea que el panel que contiene el círculo para controlarla también.  
  
    -   Si el <xref:System.Windows.UIElement.Drop> eventos no se controla, comprueba si se presionó la tecla CTRL.  
  
    -   Si se presiona la tecla CTRL cuando el <xref:System.Windows.UIElement.Drop> ocurre, realiza una copia del círculo de control y agregarlo a la <xref:System.Windows.Controls.Panel.Children%2A> colección de la <xref:System.Windows.Controls.StackPanel>.  
  
    -   Si no se presiona la tecla CTRL, mueve el círculo de la <xref:System.Windows.Controls.Panel.Children%2A> colección de su panel primario para el <xref:System.Windows.Controls.Panel.Children%2A> colección del panel que se ha quitado en.  
  
    -   Conjuntos de la <xref:System.Windows.DragEventArgs.Effects%2A> propiedad que se va a notificar el <xref:System.Windows.DragDrop.DoDragDrop%2A> método si se realizó una operación de mover o copiar.  
...
10. Arrastre un control círculo del panel en el que se encuentra al otro panel y colóquelo mientras presiona la tecla Ctrl. El círculo se copia y la copia se agrega a la <xref:System.Windows.Controls.Panel.Children%2A> colección del panel de destino.  
