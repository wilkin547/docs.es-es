---
title: "Interoperabilidad con transacciones de Enterprise Services y COM+  | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Interoperabilidad con transacciones de Enterprise Services y COM+ 
El espacio de nombres <xref:System.Transactions> admite la interoperabilidad entre los objetos de transacción creados utilizando este espacio de nombres y las transacciones creadas a través de COM\+.  
 ...
 De forma predeterminada, cuando su código de aplicación comprueba la propiedad <xref:System.Transactions.Transaction.Current%2A> estática, <xref:System.Transactions> intenta buscar una transacción que es de lo contrario actual o un objeto <xref:System.Transactions.TransactionScope> que los dictados que <xref:System.Transactions.Transaction.Current%2A> es **null**.Si no encuentra ninguno de estos, <xref:System.Transactions> consulta una transacción en el contexto de COM\+.Tenga en cuenta que aunque <xref:System.Transactions> puede buscar una transacción del contexto de COM\+, todavía favorece transacciones que son nativas a <xref:System.Transactions>.  
  
## Niveles de la interoperabilidad  
 La enumeración <xref:System.Transactions.EnterpriseServicesInteropOption> define los niveles siguientes de interoperabilidad. <xref:System.Transactions.EnterpriseServicesInteropOption>, <xref:System.Transactions.EnterpriseServicesInteropOption> y <xref:System.Transactions.EnterpriseServicesInteropOption>.  
...
1.  <xref:System.Transactions.Transaction.Current%2A> se comprueba para ver si hay una transacción.Comprueba los resultados en:  
    ...
    -   Si la enumeración <xref:System.Transactions.EnterpriseServicesInteropOption> está establecida en <xref:System.Transactions.EnterpriseServicesInteropOption>, la transacción de COM\+ \(Transacción <xref:System.EnterpriseServices> \) tiene precedencia sobre la transacción <xref:System.Transactions> en almacenamiento local de subprocesos administrado.  
  
         Si el valor se ha establecido en <xref:System.Transactions.EnterpriseServicesInteropOption>, la transacción <xref:System.Transactions> toma precedentes en el almacenamiento local de subproceso.  
  
         Si el valor es <xref:System.Transactions.EnterpriseServicesInteropOption>, hay solo una transacción y es una transacción de COM\+.  
...
3.  Si se va a crear una nueva transacción, los valores siguientes de resultado <xref:System.Transactions.EnterpriseServicesInteropOption> en:  
    ...
    -   <xref:System.Transactions.EnterpriseServicesInteropOption>: se crea una transacción asociada a un contexto de COM\+.  
  
    -   <xref:System.Transactions.EnterpriseServicesInteropOption>: se crea una transacción <xref:System.Transactions>.  
  
    -   <xref:System.Transactions.EnterpriseServicesInteropOption>: si hay un contexto de COM\+, se crea una transacción y se asocia al contexto.  
...
|Contexto ES|Ninguno|Automático|Lleno|  
|-----------------|-------------|----------------|-----------|  
...
|Contexto ES|None|Automático|Lleno|  
|-----------------|----------|----------------|-----------|  
