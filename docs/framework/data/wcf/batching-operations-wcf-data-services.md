...
ms.locfileid: "70780504"
---
# <a name="batching-operations-wcf-data-services"></a>Procesar por lotes operaciones (Servicios de datos de WCF)
Admite [!INCLUDE[ssODataFull](../../../../includes/ssodatafull-md.md)] el procesamiento por lotes de las solicitudes [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)]a un servicio basado en. Para obtener más información, [consulte OData: Procesamiento](https://go.microsoft.com/fwlink/?LinkId=186075)por lotes. En [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)], cada operación que <xref:System.Data.Services.Client.DataServiceContext>usa, como ejecutar una consulta o guardar cambios, da como resultado que se envíe una solicitud independiente al servicio de datos. Para mantener un número razonable de conjuntos de operaciones, puede definir los lotes operacionales explícitamente. Esto garantiza que todas las operaciones del lote se envían al servicio de datos en una única solicitud HTTP, permite que el servidor Procese las operaciones de forma atómica y reduce el número de viajes de ida y vuelta al servicio de datos.  
...
## <a name="batching-the-savechanges-operation"></a>Procesamiento por lotes de la operación SaveChanges  
 Cuando se llama al <xref:System.Data.Services.Client.DataServiceContext.SaveChanges%2A> método, todos los cambios de los que el contexto realiza un seguimiento se traducen en operaciones basadas en REST que [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)] se envían como solicitudes al servicio. De forma predeterminada, estos cambios no se envían en un mensaje de solicitud único. Para requerir que todos los cambios se envíen en una única solicitud, debe llamar <xref:System.Data.Services.Client.DataServiceContext.SaveChanges%28System.Data.Services.Client.SaveChangesOptions%29> al método e incluir un valor <xref:System.Data.Services.Client.SaveChangesOptions.Batch> de en <xref:System.Data.Services.Client.SaveChangesOptions> la enumeración que se proporciona al método.  
