...
ms.locfileid: "79174659"
---
# <a name="securing-wcf-data-services"></a>Proteger WCF Data Services
En este tema se describen las consideraciones de seguridad que son específicas para desarrollar, implementar y ejecutar SERVICIOS de datos WCF y aplicaciones que tienen acceso a servicios que admiten el Protocolo de datos abiertos (OData). También debe seguir las recomendaciones para crear aplicaciones seguras de .NET Framework.  
  
Al planear cómo proteger un servicio OData basado en WCF Data Services, debe abordar tanto la autenticación, el proceso de detección y comprobación de la identidad de una entidad de seguridad, y la autorización, el proceso de determinar si una entidad de seguridad autenticada es acceso a los recursos solicitados. También debe plantearse si va a cifrar el mensaje mediante SSL.  
  
## <a name="authenticating-client-requests"></a>Autenticar las solicitudes de clientes  
Servicios de datos de WCFWCF Data Services no implementa ningún tipo de autenticación propia, sino que se basa en las disposiciones de autenticación del host del servicio de datos. Esto significa que el servicio supone que cualquier solicitud que recibe ya ha sido autenticada por el host de red y que el host ha identificado correctamente el principio de la solicitud correctamente a través de las interfaces proporcionadas por WCF Data Services. Estas opciones y enfoques de autenticación se detallan en la [serie Multipart OData y Authentication](https://devblogs.microsoft.com/odata/?s=%22OData+and+authentication%22).  
...
```  
 ...
 Para obtener más información, vea [Cómo: Especificar credenciales](specify-client-creds-for-a-data-service-request-wcf.md)de cliente para una solicitud de servicio de datos .  
  
 Cuando el servicio de datos necesite credenciales de inicio de sesión que no se puedan especificar mediante el uso de un objeto <xref:System.Net.NetworkCredential>, como un token basado en notificaciones o una cookie, debe establecer manualmente los encabezados en la solicitud HTTP, que suelen ser los encabezados `Authorization` y `Cookie`. Para obtener más información acerca de este tipo de escenario de autenticación, vea la entrada de blog [OData y Autenticación – Parte 3 – Enlaces ClientSide](https://devblogs.microsoft.com/odata/odata-and-authentication-part-3-clientside-hooks/). Para obtener un ejemplo de cómo establecer encabezados HTTP en un mensaje de solicitud, vea [Cómo: Establecer encabezados en la solicitud](how-to-set-headers-in-the-client-request-wcf-data-services.md)de cliente .  
  
## <a name="impersonation"></a>Suplantación  
 Por lo general, el servicio de datos accede a los recursos necesarios, como los archivos del servidor o de la base de datos, mediante el uso de las credenciales del proceso de trabajo que hospeda el servicio de datos. Cuando se usa la suplantación, ASP.NET aplicaciones se pueden ejecutar con la identidad de Windows (cuenta de usuario) del usuario que realiza la solicitud. La suplantación se suele usar en aplicaciones que confían en IIS para autenticar al usuario y las credenciales de esta entidad de seguridad se usan para obtener acceso a los recursos necesarios. Para obtener más información, consulte [ASP.NET suplantación](https://docs.microsoft.com/previous-versions/aspnet/xh507fc5(v=vs.100)).  
...
### <a name="ignored-message-headers-and-cookies"></a>Cookies y encabezados del mensaje ignorados  
 ...
 Las cookies se pueden utilizar como parte de un esquema de autenticación, como con ASP.NET autenticación de formularios. Sin embargo, las cookies HTTP establecidas en una solicitud entrante son ignoradas por WCF DataServices. El host de un servicio de datos puede procesar la cookie, pero el tiempo de ejecución de Servicios de datos de WCFWCF Data Services nunca analiza ni devuelve cookies. La biblioteca de cliente de Servicios de datos de WCFWCF Data Services tampoco procesa las cookies enviadas en la respuesta.  
  
### <a name="custom-hosting-requirements"></a>Requisitos personalizados de hospedaje  
 De forma predeterminada, Servicios de datos de WCFWCF Data Services se crea como una aplicación ASP.NET hospedada en IIS. De esta forma, el servicio de datos puede sacar provecho de los comportamientos seguros de esta plataforma. Puede definir Servicios de datos de WCFWCF Data Services hospedados por un host personalizado. Para obtener más información, consulte [Hosting the Data Service](hosting-the-data-service-wcf-data-services.md). Los componentes y la plataforma que hospedan un servicio de datos deben asegurar los siguientes comportamientos de seguridad para impedir ataques contra el servicio de datos:  
...
- Limite el tamaño de los encabezados HTTP y sus valores y proporcione acceso de Servicios de datos de WCFWCF Data Services a los datos de encabezado.  
