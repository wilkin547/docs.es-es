...
El siguiente diagrama muestra un modelo conceptual con cuatro tipos de entidad: `Book`, `FictionBook`, `Publisher`, y `Author`. El tipo de entidad `FictionBook` es un tipo derivado, que hereda del tipo de entidad `Book`. El tipo `FictionBook` hereda las propiedades `ISBN (Key)`, `Title` y `Revision`, y define una propiedad adicional denominada `Genre`.  
