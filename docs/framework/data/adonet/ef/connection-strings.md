...
`DataDirectory`y el operador ~ se utilizan en <xref:System.Data.EntityClient.EntityConnection.ConnectionString%2A> como parte de las `Metadata` palabras clave `Provider Connection String` y. El elemento <xref:System.Data.EntityClient.EntityConnection> reenvía `DataDirectory` y el operador ~ a <xref:System.Data.Metadata.Edm.MetadataWorkspace> y al proveedor de almacenamiento, respectivamente.
