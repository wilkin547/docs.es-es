...
- Utilice su propio código para eliminar primero los objetos secundarios que impiden que se elimine el objeto primario.  
 ...
 Para obtener más información, consulte [Cómo Eliminar filas de la base](how-to-delete-rows-from-the-database.md)de datos.  
...
## <a name="groupby-invalidoperationexception"></a>GroupBy InvalidOperationException  
 Esta excepción se produce cuando un valor de columna es nulo en una consulta <xref:System.Linq.Enumerable.GroupBy%2A> que agrupa mediante una expresión `boolean`, como `group x by (Phone==@phone)`. Dado que la expresión es `boolean`, se deduce `boolean`que la clave es, no `nullable` `boolean`. Cuando la comparación traducida genera un valor null, se realiza un intento de `nullable` asignar un `boolean` `boolean` a un y se produce la excepción.  
