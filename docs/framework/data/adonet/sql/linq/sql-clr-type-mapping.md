...
- [Asignación de tipos predeterminada](#DefaultTypeMapping)  
...
- [Asignación de varios](#MiscMapping)  
...
### <a name="custom-type-mapping"></a>Asignación de tipos personalizada  
...
- Comentarios  
 ...
 Aunque <xref:System.Xml.Linq.XElement> puede asignar y SQL Server <xref:System.Xml.Linq.XDocument> tal y como se muestra en la matriz de comportamiento de la asignación de <xref:System.Data.Linq.DataContext.CreateDatabase%2A?displayProperty=nameWithType> tipos en tiempo de [ejecución](#BehaviorMatrix), el método no tiene ninguna asignación de tipo SQL Server predeterminada para estos tipos.  
  
### <a name="custom-types"></a>Tipos personalizados  
 Si `Parse()` una clase implementa y `TEXT` `XML``CHAR` `NTEXT` `NCHAR` `NVARCHAR` `VARCHAR`, puede asignar el objeto a cualquier tipo de texto de SQL (,,,,,,). `ToString()` El objeto se almacena en la base de datos enviando el valor devuelto por `ToString()` a la columna de base de datos asignada. El objeto se reconstruye invocando a `Parse()` en la cadena devuelta por la base de datos.  
