...
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]traduce [!INCLUDE[vbteclinq](../../../../../../includes/vbteclinq-md.md)] las consultas a SQL para su ejecución en una base de datos. Los resultados son `IEnumerable` fuertemente tipados. Dado que se trata de objetos CLR (Common Language Runtime) normales, se puede usar el enlace de datos de objeto normal para mostrar los resultados. Por otro lado, las operaciones de cambio (inserciones, actualizaciones y eliminaciones) requieren pasos adicionales.

## <a name="operation"></a>Operación

El enlace implícito a controles de Windows Forms se logra mediante la implementación de <xref:System.ComponentModel.IListSource>. Los orígenes de <xref:System.Data.Linq.Table%601> datos`Table<T>` genéricos `Table(Of T)` ( C# en o en Visual Basic `DataQuery` ) y genéricos se <xref:System.ComponentModel.IListSource>han actualizado para implementar. Los motores de enlace de datos de la interfaz de usuario (Windows Forms y Windows Presentation Foundation) comprueban si su origen de datos implementa <xref:System.ComponentModel.IListSource>. Por consiguiente, al escribir una redirección de una consulta en un origen de datos de un control, [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] se llama implícitamente a la generación de la colección, como en el ejemplo siguiente:
...
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]<xref:System.ComponentModel.IListSource> implementa en dos ubicaciones:

- El origen de datos es <xref:System.Data.Linq.Table%601>: [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] examina la tabla para rellenar `DataBindingList` una colección que mantiene una referencia en la tabla.
...
Al usar System. Windows. Forms. BindingSource y enlazar una >\<de la carpa de EntitySet a System. Windows. Forms. BindingSource. DataSource, debe llamar\<a la > de la carpa de EntitySet. Getnewbindinglist con fin actualizar BindingSource. List.

Si usa System. Windows. Forms. BindingSource y establece la propiedad BindingSource. DataMember y establece BindingSource. DataSource en una clase que tiene una propiedad denominada en BindingSource. DataMember que expone el > de la carpa de EntitySet\<, usted no tiene que llamar a\<la > de la carpa de EntitySet. Getnewbindinglist con fin para actualizar BindingSource. List, pero se pierde la capacidad de ordenación.

## <a name="caching"></a>Almacenamiento en memoria caché

[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]las consultas <xref:System.ComponentModel.IListSource.GetList%2A>implementan. Cuando la clase BindingSource de Windows Forms se encuentra con esta interfaz, llama a GetList() tres veces para la misma conexión. Para evitar esta situación, [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] implementa una caché por instancia para almacenar y siempre devuelve la misma colección generada.
...
- De forma predeterminada `image`, `varbinary`los tipos `timestamp` de base de datos, y se asignan a una matriz de bytes. Dado que `ToString()` no se admite en este escenario, estos objetos no se pueden mostrar.

- Un miembro de clase asignado a una clave principal tiene un establecedor [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] , pero no admite el cambio de identidad de objeto. Por consiguiente, la clave principal/única que se usa en la asignación no se puede actualizar en la base de datos. Un cambio en la cuadrícula produce una excepción al llamar a <xref:System.Data.Linq.DataContext.SubmitChanges%2A>.
