---
title: "Instancias de usuario de SQL Server Express | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Instancias de usuario de SQL Server Express
...
## Funcionalidades de las instancias de usuario  
...
## Habilitar instancias de usuario  
 Para generar instancias de usuario, debe estar ejecutándose una instancia primaria de SQL Server Express.  Las instancias de usuario se habilitan de forma predeterminada cuando se instala SQL Server Express y el administrador del sistema pueden habilitarlas o deshabilitarlas de forma explícita ejecutando en la instancia primaria el procedimiento almacenado del sistema **sp\_configure**.  
...
## Conectarse a una instancia de usuario  
 Las palabras clave `User Instance` y `AttachDBFilename` <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A> permiten que una <xref:System.Data.SqlClient.SqlConnection> se conecte a una instancia de usuario.  Las instancias de usuario también funcionan con las propiedades <xref:System.Data.SqlClient.SqlConnectionStringBuilder> `UserInstance` y `AttachDBFilename`.  
...
### Utilizar la cadena de sustitución &#124;DataDirectory&#124;  
 ...
 La ubicación física que indica `DataDirectory` depende del tipo de aplicación.  En este ejemplo, el archivo Northwind.mdf que se va a adjuntar se encuentra en la carpeta \\app\_data de la aplicación.  
...
```  
 ...
 Si se usa `DataDirectory`, la ruta del archivo resultante no puede estar más alta en la estructura de directorios que el directorio que señala la cadena de sustitución.  Por ejemplo, si el `DataDirectory` totalmente expandido es C:\\AppDirectory\\app\_data, la cadena de conexión de ejemplo que se muestra arriba funcionará porque está por debajo de c:\\AppDirectory.  Sin embargo, si se intenta especificar `DataDirectory` como `|DataDirectory|\..\data`, se producirá un error porque \\data no es un subdirectorio de \\AppDirectory.  
...
## Cómo funcionan las instancias de usuario  
 La primera vez que se genera una instancia de usuario para cada usuario, las bases de datos del sistema **maestra** y **msdb** se copian de la carpeta de datos de plantilla \(Template Data\) a una ruta de acceso en el directorio del repositorio de datos de aplicación local del usuario para que las utilice exclusivamente la instancia de usuario.  La ruta de acceso suele ser `C:\Documents and Settings\<UserName>\Local Settings\Application Data\Microsoft\Microsoft SQL Server Data\SQLEXPRESS`.  Cuando se inicia una instancia, también se copian en este directorio el archivo **tempdb** y los archivos de registro y de seguimiento.  Se genera un nombre para la instancia cuya exclusividad está garantizada para cada usuario.  
...
## Escenarios de instancias de usuario  
...
-   Implementación ClickOnce  Si .NET Framework 2.0, o posterior, y SQL Server Express ya están instalados en el equipo de destino, los usuarios que no sean administradores pueden instalar y utilizar el paquete de instalación que se descargó como resultado de una acción ClickOnce.  Un administrador debe instalar SQL Server Express si forma parte de la instalación.  Para obtener más información, consulta [ClickOnce Deployment for Windows Forms Applications](http://msdn.microsoft.com/es-es/34d8c770-48f2-460c-8d67-4ea5684511df).  
