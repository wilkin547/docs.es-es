---
title: "Extremos de servicio y direccionamiento de la cola | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Extremos de servicio y direccionamiento de la cola
En este tema se aborda cómo los clientes direccionan servicios que leen de las colas y cómo los extremos de servicio se asignan a las colas.A modo de recordatorio, la ilustración siguiente muestra la implementación clásica de aplicaciones puestas en la cola [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)].  
...
## Direccionamiento MSMQ  
 ...
 Los nombres de las rutas de acceso están asignados a "FormatNames" para determinar aspectos adicionales de la dirección, incluso enrutar y el protocolo de transferencia del Administrador de cola.El Administrador de la cola admite dos protocolos de transferencia: protocolo MSMQ nativo y SOAP Reliable Messaging Protocol \(SRMP\).  
  
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] la ruta de MSMQ y los nombres de formato, vea [Acerca de las colas de mensajes](http://go.microsoft.com/fwlink/?LinkId=94837).  
  
## NetMsmqBinding y direccionamiento del servicio  
 ...
 El direccionamiento de una cola en [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] está basado en el modelo siguiente:  
...
-   \<*queue\-name*\> es el nombre de la cola.El nombre de la cola también puede hacer referencia a una subcola.Así, \<*queue\-name*\> \= \<*name\-of\-queue*\>\[;*sub\-queue\-name*\].  
 ...
 Un extremo que lee de una cola debe especificar la dirección de la cola utilizando el mismo esquema especificada previamente al abrir ServiceHost.Para obtener ejemplos, vea [Enlace de red MSMQ](../../../../docs/framework/wcf/samples/net-msmq-binding.md) y [Message Queuing Integration Binding Samples](http://msdn.microsoft.com/es-es/997d11cb-f2c5-4ba0-9209-92843d4d0e1a).  
  
### Varios contratos en una cola  
...
### Asignar el URI net.msmq a nombres de formato de Message Queuing  
...
|Net.msmq:\/\/\<machine\-name\>\/private\/abc|True|Native|PUBLIC\=some\-guid \(el GUID de la cola\)|  
  
### Leer los mensajes de la cola de mensajes no enviados o la cola de mensajes dudosos  
 ...
 Un servicio de [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] comprueba que todos los mensajes que recibe se direccionaron a la cola específica en la que está escuchando.Si la cola de destino del mensaje no coincide con la cola donde se encuentra, el servicio no procesa el mensaje.Se trata de una cuestión que los servicios que escuchan a una cola de mensajes no enviados deben abordar porque cualquier mensaje en la cola de mensajes no enviados debía ser entregado a otra parte.Para leer los mensajes de una cola de mensajes no enviados o de una cola de mensajes dudosos, debe utilizarse `ServiceBehavior` con el parámetro <xref:System.ServiceModel.AddressFilterMode>.Vea un ejemplo en [Colas con problemas de entrega](../../../../docs/framework/wcf/samples/dead-letter-queues.md).  
  
## MsmqIntegrationBinding y direccionamiento del servicio  
 ...
 El nombre de formato de MSMQ es la forma especificada por MSMQ en [Acerca de Message Queuing](http://go.microsoft.com/fwlink/?LinkId=94837).  
