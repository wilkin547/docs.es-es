...
---
# Error handling
## Error de control en Windows Communication Foundation  
 Cuando un servicio detecta una excepción o error inesperados, hay varias maneras de diseñar una solución desde el control de excepciones.  Cuando no hay una única solución "correcta" o "recomendable" de control de errores, hay varias rutas de acceso válidas a considerar.  Se recomienda normalmente al implementar una solución híbrida en la que combine varios enfoques de la lista, dependiendo de la complejidad de la implementación de WCF, el tipo y la frecuencia de excepciones, las controladas vs.  naturaleza no controlada de excepciones, y cualquier traza, inicio de sesión o requisitos de la directiva.  
  
 Estas soluciones se explican en más profundidad en el resto de esta sección.  
  
### Biblioteca de Microsoft Enterprise  
 ...
 Para obtener más información, consulte [Microsoft Enterprise Library](http://msdn.microsoft.com/en-us/library/ff632023.aspx).  
  
### Tratar excepciones esperadas  
 La línea de conducta correspondiente es detectar excepciones esperadas en cada operación o punto de extensibilidad relevante, decidir si se puede recuperar y devolver el error personalizado apropiado en un FaultExceptionT\<T\>  
  
### Tratar excepciones no esperadas con un IErrorHandler  
 Para tratar excepciones no esperadas, la línea de conducta recomendada es “enlazar" un IErrorHandler.  Los controladores de errores solo capturan excepciones en el nivel de tiempo de ejecución de WCF \(capa "modelo de servicio”\), no en la capa de canal.  La única forma de enlazar un IErrorHandler en el nivel de canal es crear un canal personalizado, que no se recomienda en la mayoría de los escenarios.  
  
 Una “excepción inesperada” no suele ser ni una excepción irrecuperable ni una excepción de procesamiento; es, en cambio, una excepción inesperada de usuario.  Una excepción irrecuperable \(como una excepción porque se ha agotado la memoria\), un tipo de excepción que suele controlar el [Controlador de excepciones de modelo de servicio](http://msdn.microsoft.com/library/system.servicemodel.dispatcher.exceptionhandler.aspx) de forma automática, por lo general, no se puede controlar sin contratiempos y la única razón por la que se controla una excepción de este tipo posiblemente sea para realizar inicios de sesión adicionales o para devolver una excepción estándar al cliente.  Aparece una excepción de procesamiento en el procesamiento de mensajes, \(por ejemplo, en la serialización, el codificador o el nivel formateador, normalmente no se pueden controlar en un IErrorHandler, pues suele ser demasiado antiguo o demasiado atrasado para que intervenga el controlador de errores en el momento en que estas excepciones aparecen.  De forma similar, las excepciones de transporte no se pueden controlar en un IErrorHandler.  
...
### Tratar excepciones esperadas fuera de WCF  
 A menudo, excepciones de configuración, excepciones de la cadena de conexión a bases de datos y otras excepciones similares pueden aparecer en el contexto de una aplicación de WCF, pero no son excepciones iniciadas por consultas del modelo de servicio o servicio web en sí.  Estas excepciones son las excepciones “normales” externas al servicio web, y deben ser controladas al igual que otras excepciones externas en el entorno.  
  
### Traza de excepciones  
 El seguimiento es el único lugar “catchall” donde se podrían ver todas las excepciones.  Para obtener más información acerca de excepciones de seguimiento e inicio de sesión, vea Seguimiento e Inicio de sesión.  
  
### Errores de la plantilla del URI al usar WebGetAttribute y WebInvokeAttribute  
...
## Vea también  
 [Control de errores de WCF básico](http://msdn.microsoft.com/library/gg281715.aspx)