...
1. Cree un archivo *Web. config* para configurar un extremo para `CalculatorService` que use <xref:System.ServiceModel.WSHttpBinding> con sesión confiable habilitada y entrega ordenada de los mensajes requeridos.
...
1. *SvcUtil. exe* también genera la configuración para el cliente que usa la <xref:System.ServiceModel.WSHttpBinding> clase. Asigne un nombre al archivo de configuración *app. config* al usar Visual Studio.
