...
El Windows Communication Foundation (WCF) se puede considerar como una infraestructura de mensajería. Las operaciones de servicio pueden recibir mensajes, procesarlos y enviarles mensajes. Los mensajes se describen mediante contratos de operaciones. Por ejemplo, considere el siguiente contrato:  
...
```  
 ...
 Para obtener más información, consulta [using Message Contracts](using-message-contracts.md).  
...
## <a name="describing-messages-by-using-streams"></a>Descripción de mensajes mediante el uso de secuencias  
 ...
 En lugar de invocar el serializador, WCF recupera datos de una secuencia y los coloca directamente en un mensaje saliente, o recupera datos de un mensaje entrante y los coloca directamente en un flujo. En el siguiente ejemplo se muestra la forma de utilizar secuencias.  
...
```  
 ...
 Para obtener más información, consulte [datos y streaming de gran tamaño](large-data-and-streaming.md).  
...
```  
 ...
 Este es un escenario avanzado, que se describe en detalle en [uso de la clase de mensaje](using-the-message-class.md).  
...
```  
 ...
 Estos errores adicionales se pueden generar mediante la generación de una <xref:System.ServiceModel.FaultException%601> del tipo de contrato de datos adecuado. Para obtener más información, vea [controlar excepciones y errores](../extending/handling-exceptions-and-faults.md).  
...
### <a name="serialization-behaviors"></a>Comportamientos de serialización  
 ...
 Sin embargo, `DataContractSerializerOperationBehavior` tiene `MaxItemsInObjectGraph`, `IgnoreExtensionDataObject`, y las propiedades `DataContractSurrogate` que puede utilizar para personalizar el proceso de serialización. Las primeras dos propiedades tienen el mismo significado que se mencionó en la sección anterior. Puede utilizar la propiedad `DataContractSurrogate` para habilitar suplentes del contrato de datos, que son un mecanismo eficaz para personalizar y extender el proceso de serialización. Para obtener más información, vea [suplentes del contrato de datos](../extending/data-contract-surrogates.md).  
...
El siguiente es el código equivalente en el servicio, en el caso de autohospedaje:
...
3. Antes de abrir el host de servicio o crear un canal de cliente, elimine el comportamiento <xref:System.ServiceModel.Description.DataContractSerializerOperationBehavior> existente y conecte la clase derivada personalizada que cree en los pasos anteriores.  
 ...
 Para obtener más información sobre los conceptos de serialización avanzada, vea [serialización y deserialización](serialization-and-deserialization.md).  
