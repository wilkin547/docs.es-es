...
- Configure un servicio para utilizar un enlace proporcionado por el sistema que admite seguridad de mensaje.  
     ...
     A excepción del enlace de [ \<> basicHttpBinding](../../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md) , cuando los enlaces proporcionados por el sistema se configuran para usar la seguridad de mensajes, WCF usa automáticamente sesiones seguras. La siguiente tabla detalla los enlaces proporcionados por el sistema que admiten seguridad de mensaje y si la seguridad de mensaje es el mecanismo de seguridad predeterminado.  
  
    |Enlace proporcionado por el sistema|Elemento de configuración|Seguridad de mensaje activada de forma predeterminada|  
    |------------------------------|---------------------------|------------------------------------|  
    |<xref:System.ServiceModel.BasicHttpBinding>|[\<basicHttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)|Sin|  
    ...
    |<xref:System.ServiceModel.NetTcpBinding>|[\<netTcpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/nettcpbinding.md)|Sin|  
    |<xref:System.ServiceModel.NetMsmqBinding>|[\<netMsmqBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/netmsmqbinding.md)|Sin|  
  
     En el ejemplo de código siguiente se usa la `wsHttpBinding_Calculator` [ \<](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md)configuración para especificar un enlace denominado que usa wsHttpBinding >, seguridad de mensajes y sesiones seguras.  
    ...
    ```  
     ...
     En el ejemplo de código siguiente se especifica que se utiliza [ \<wsHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md), la seguridad de mensajes y las sesiones `secureCalculator` seguras para proteger el servicio.  
  
     [!code-csharp[c_CreateSecureSession#1](../../../../samples/snippets/csharp/VS_Snippets_CFX/c_createsecuresession/cs/secureservice.cs#1)]
     [!code-vb[c_CreateSecureSession#1](../../../../samples/snippets/visualbasic/VS_Snippets_CFX/c_createsecuresession/vb/secureservice.vb#1)]  
  
    > [!NOTE]
    > Las sesiones seguras pueden desactivarse para el [ \<> wsHttpBinding](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md) estableciendo el `establishSecurityContext` atributo en `false`. Para los otros enlaces proporcionados por el sistema, las sesiones seguras pueden desactivarse solo mediante la creación un enlace personalizado.  
...
- Cree un enlace personalizado que especifica que una sesión segura protege los mensajes SOAP.  
     ...
     Para obtener más información acerca de cómo crear un enlace [personalizado, consulte Cómo: Personalizar un enlace](../../../../docs/framework/wcf/extending/how-to-customize-a-system-provided-binding.md)proporcionado por el sistema.  
