...
En este tema se explica cómo hacer que un servicio de Windows Communication Foundation (WCF) sea reconocible. Se basa en el ejemplo de [host propio](https://docs.microsoft.com/dotnet/framework/wcf/samples/self-host) .  
...
11. Dentro del método `InvokeCalculatorService`, cree una instancia de la clase `CalculatorServiceClient`. Esta clase se define mediante el ejemplo [de host propio](https://docs.microsoft.com/dotnet/framework/wcf/samples/self-host) . Se ha generado mediante Svcutil.exe.  
