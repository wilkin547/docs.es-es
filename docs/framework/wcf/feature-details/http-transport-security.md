---
title: "Seguridad de transporte HTTP | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
### Certificados e infraestructura de clave pública \(PKI\)  
 ...
 Cada certificado tiene dos claves, una clave privada y una clave pública y los dos son conocidos como un *par de la clave de intercambio*.  En resumen, la clave privada solo es conocida por el propietario del certificado mientras que la clave pública es legible por el certificado.  La clave se puede utilizar para cifrar o descifrar un resumen, hash u otra clave, pero solo como operaciones contrarias.  Por ejemplo, si el cliente cifra con clave pública, solo el sitio puede descifrar el mensaje mediante la clave privada.  De igual forma, si el sitio cifra la clave privada, el cliente puede descifrar con la clave pública.  Esto proporciona garantía al cliente de que los mensajes solo se intercambian con el poseedor de la clave privada, porque solo los mensajes cifrados con la clave privada se pueden descifrar con la clave pública.  Se garantiza que el sitio está intercambiando los mensajes con un cliente que se ha cifrado utilizando la clave pública.  Este intercambio solo es seguro para un protocolo de enlace inicial, motivo por el cual se hace mucho más para crear la clave simétrica real.  No obstante, todas las comunicaciones dependen del servicio que tiene un certificado SSL válido.  
  
## Implementar SSL con WCF  
...
### Utilizar IIS para seguridad de transporte  
  
#### IIS 7.0  
 Para preparar [!INCLUDE[iisver](../../../../includes/iisver-md.md)] como un host seguro \(mediante SSL\), vea [Configurar la capa de sockets seguros en IIS 7](http://go.microsoft.com/fwlink/?LinkId=88600).  
  
 Para configurar certificados para su uso con [!INCLUDE[iisver](../../../../includes/iisver-md.md)], vea [Configurar certificados de servidor en IIS 7](http://go.microsoft.com/fwlink/?LinkID=88595).  
  
#### IIS 6,0  
 Para preparar [!INCLUDE[iis601](../../../../includes/iis601-md.md)] como un host seguro \(mediante SSL\), vea [Configurar la capa de sockets seguros](http://go.microsoft.com/fwlink/?LinkId=88601).  
  
 Para configurar certificados para su uso con [!INCLUDE[iis601](../../../../includes/iis601-md.md)], vea [Certificates\_IIS\_SP1\_Ops](http://go.microsoft.com/fwlink/?LinkId=88602).  
  
### Utilizar HttpCfg para SSL  
 Si va a crear una aplicación [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] autohospedada, descargue la herramienta HttpCfg.exe, disponible en el sitio de [Herramientas de soporte técnico del Service Pack 2 de Windows XP](http://go.microsoft.com/fwlink/?LinkId=29002).  
  
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] utilizar la herramienta HttpCfg.exe para configurar un puerto con un certificado X.509, consulte [Cómo: Configurar un puerto con un certificado SSL](../../../../docs/framework/wcf/feature-details/how-to-configure-a-port-with-an-ssl-certificate.md).  
