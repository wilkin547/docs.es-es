...
`xref:System.ServiceModel.Activities.SendReply.ValueType`
 ...
 El resultado final de inferencia del contrato es una descripción del servicio mediante las mismas estructuras de datos como servicios WCF y contratos de operación. A continuación, esta información se utiliza para exponer WSDL para el servicio de flujo de trabajo.  
...
## <a name="hosting-a-workflow-service"></a>Hospedar un servicio de flujo de trabajo  
 Al igual que los servicios WCF, servicios de flujo de trabajo deben estar hospedados. Servicios de WCF usan el <xref:System.ServiceModel.ServiceHost> clase para hospedar servicios y flujo de trabajo usan los servicios <xref:System.ServiceModel.Activities.WorkflowServiceHost> para hospedar los servicios. Al igual que los servicios WCF, servicios de flujo de trabajo pueden hospedarse en una variedad de formas, por ejemplo:  
