---
title: "Datos de gran tama&#241;o y secuencias | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] es una infraestructura de comunicaciones basada en XML.Debido a que los datos XML normalmente se codifican en el formato de texto estándar definido en [XML 1.0 specification](http://go.microsoft.com/fwlink/?LinkId=94838), los arquitectos y programadores de sistemas conectados normalmente se preocupan por la superficie \(o tamaño\) de conexión que requieren los mensajes enviados a través de la red, y la codificación XML, basada en texto, plantea desafíos especiales para la transferencia eficaz de los datos binarios.  
...
### Codificar datos: Texto yBinario  
...
### Contenido binario  
 ...
 Para evitar esta sobrecarga de codificación, la norma del Mecanismo de optimización de transmisión del mensaje \(MTOM\) permite exteriorizar elementos de datos de gran tamaño contenidos en un mensaje y llevarlos con el mensaje como datos binarios sin ninguna codificación especial.Con MTOM, los mensajes se intercambian en un modo similar a los mensajes de correo electrónico de Protocolo simple de transferencia de correo \(SMTP\) con datos adjuntos o contenido incrustado \(imágenes y otro contenido incrustado\); los mensajes de MTOM se empaquetan como secuencias MIME multiparte\/relacionado y la parte de la raíz es el mensaje SOAP real.  
...
-   No están disponibles en su totalidad cuando se inicia la transferencia.  
 ...
 Al enviar grandes cantidades de datos necesitará establecer el valor maxAllowedContentLength de IIS \(para obtener más información vea [Configurar límites de solicitud de IIS](http://go.microsoft.com/fwlink/?LinkId=253165)\) y el valor de enlace maxReceivedMessageSize \(por ejemplo <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> o <xref:System.ServiceModel.NetTcpBinding.MaxReceivedMessageSize%2A>\).La propiedad maxAllowedContentLength tiene como valor predeterminado 28,6 M y la propiedad maxReceivedMessageSize 64KB.  
  
## Codificaciones  
 Una *codificación* define un conjunto de reglas sobre cómo presentar los mensajes en la conexión.Un *codificador* implementa esta codificación y es responsable, en el lado del remitente, de convertir un <xref:System.ServiceModel.Channels.Message> mensaje en memoria en una secuencia de bytes o búfer de bytes que se puede enviar a través de la red.En el lado del receptor, el codificador convierte una secuencia de bytes en un mensaje en memoria.  
  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] incluye tres codificadores y le permite escribir y complementar sus propios codificadores, si es necesario.  
...
```  
 ...
 Dado que el codificador MTOM siempre emite un mensaje MIME\/de varias partes codificado con MTOM sin tener en cuenta si los datos binarios acaban exteriorizándose, generalmente se debería habilitar MTOM solo para los extremos que intercambian los mensajes con más de 1 KB de datos binarios.Asimismo, los contratos de servicios diseñados para el uso con extremos habilitados por MTOM deben, cuando sea posible, ser restringidos a especificar tales operaciones de transferencia de datos.La funcionalidad de control relacionada debería residir en un contrato independiente.Esta norma "solo MTOM" se aplica solamente a los mensajes enviados a través de un extremo habilitado por MTOM; el codificador MTOM puede descodificar y analizar también los mensajes de entrada no MTOM.  
  
 El uso del codificador MTOM se ajusta a las demás características de [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].Tenga en cuenta que tal vez no sea posible respetar esta regla en todos los casos, por ejemplo, cuando se requiere la compatibilidad de la sesión.  
  
### Modelo de programación  
 Independientemente de cuál de los tres codificadores integrados utiliza en su aplicación, la experiencia de programación es idéntica con respecto a la transferencia de datos binarios.La diferencia está en cómo [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] administra los datos basados en sus tipos de datos.  
...
> [!NOTE]
>  No debería estar utilizando tipos derivados <xref:System.IO.Stream?displayProperty=fullName> dentro de los contratos de datos.Los datos de la secuencia se deberían comunicar utilizando el modelo de secuencias, explicado en la sección "Transmisión por secuencias de datos".  
  
## Transmisión por secuencias de datos  
 Cuando tiene una gran cantidad de datos para transferir, el modo de transferencia de secuencias en [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] es una alternativa factible al comportamiento predeterminado de almacenar en búfer y procesar los mensajes en memoria en su totalidad.  
...
### Restricciones  
 No puede utilizar un número significativo de características de [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] cuando están habilitadas las secuencias:  
...
#### Transmisión por secuencias y sesiones  
...
### Habilitar la transmisión por secuencias  
 Puede habilitar la transmisión por secuencias de las siguientes maneras:  
  
-   Envíe y acepte las solicitudes en modo de transmisión y acepte y devuelva las respuestas en modo almacenado en búfer \(<xref:System.ServiceModel.TransferMode>\).  
  
-   Envíe y acepte las solicitudes en modo de almacenamiento en búfer y acepte y devuelva las respuestas en modo de transmisión \(<xref:System.ServiceModel.TransferMode>\).  
  
-   Envíe y reciba solicitudes y respuestas en modo de transmisión en ambas direcciones.\(<xref:System.ServiceModel.TransferMode>\).  
  
 Puede deshabilitar la transmisión por secuencias estableciendo el modo de transferencia en <xref:System.ServiceModel.TransferMode>, que es la configuración predeterminada en todos los enlaces.El código siguiente muestra cómo establecer el modo de transferencia en la configuración.  
...
```  
 ...
 `Echo` en el ejemplo anterior recibe y devuelve una secuencia y se debería utilizar por consiguiente en un enlace con <xref:System.ServiceModel.TransferMode>.Para `RequestInfo` de la operación, <xref:System.ServiceModel.TransferMode> se adapta mejor, porque solo devuelve <xref:System.IO.Stream>.La operación unidireccional se adapta mejor para <xref:System.ServiceModel.TransferMode>.  
...
```  
 ...
 Las transferencias con secuencias finalizan y el mensaje se cierra cuando la secuencia alcanza el final del archivo \(EOF\).Cuando se envía un mensaje \(devolviendo un valor o invocando una operación\), puede pasar <xref:System.IO.FileStream> y la infraestructura [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] extrae como consecuencia todos los datos de esa secuencia hasta que la secuencia se haya leído totalmente y haya alcanzado EOF.Para transferir datos transmitidos al origen cuando no existe clase derivada <xref:System.IO.Stream> previamente generada, construya esta clase, superponga la clase sobre su origen de secuencia y utilícelo como el argumento o valor devuelto.  
  
 Cuando recibe un mensaje, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] construye una secuencia sobre el contenido del cuerpo del mensaje codificado por Base64 \(o la parte respectiva MIME si se utiliza MTOM\) y la secuencia alcanza EOF cuando se ha leído el contenido.  
...
### Consideraciones de seguridad específicas para datos de gran tamaño  
 Todos los enlaces le permiten restringir el tamaño de los mensajes entrantes para evitar los ataques por denegación de servicio.Por ejemplo, <xref:System.ServiceModel.BasicHttpBinding> expone una propiedad <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> que limita el tamaño del mensaje entrante y así también limita la cantidad de memoria máxima a la que se obtiene acceso al procesar el mensaje.Esta unidad se establece en bytes con un valor predeterminado de 65.536 bytes.  
  
 Una amenaza de seguridad específica del escenario de transmisión por flujos de datos grandes provoca una denegación de servicio haciendo que se almacenen en búfer los datos cuando el receptor espera que se transmitan.Por ejemplo, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] siempre almacena en búfer los encabezados SOAP de un mensaje y, de este modo, un atacante puede construir un mensaje malintencionado grande compuesto totalmente de encabezados para forzar que los datos se almacenen en búfer.Cuando está habilitada la transmisión por secuencias, `MaxReceivedMessageSize` puede estar establecido en un valor sumamente grande, porque el receptor nunca espera que el mensaje completo esté almacenado en búfer en memoria a la vez.Si se fuerza [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] a que almacene en búfer el mensaje, se produce un desbordamiento de la memoria.  
 ...
 También es importante entender que esta cuota solo limita el almacenamiento en búfer realizado por [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] y no puede protegerle contra ningún almacenamiento en búfer que usted haga en su propio servicio o implementación de cliente.[!INCLUDE[crabout](../../../../includes/crabout-md.md)] las consideraciones de seguridad adicionales , vea [Consideraciones de seguridad para datos](../../../../docs/framework/wcf/feature-details/security-considerations-for-data.md).  
  
> [!NOTE]
>  La decisión de utilizar transferencias almacenadas en búfer o transmitidas es una decisión local del extremo.Para los transportes HTTP, el modo de transferencia no se propaga a través de una conexión o a los servidores proxy y otros intermediarios.Establecer el modo de transferencia no se refleja en la descripción de la interfaz de servicio.Después de generar un cliente [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] a un servicio, debe editar el archivo de configuración de los servicios pensados para ser utilizado con transferencias transmitidas para establecer el modo.Para TCP y transportes de la canalización con nombre, el modo de transferencia se propaga como una aserción de directiva.  
