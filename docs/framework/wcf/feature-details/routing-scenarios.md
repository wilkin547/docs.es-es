---
title: "Escenarios de enrutamiento | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
### Control de versiones del servicio  
 ...
 Utilizando el servicio de enrutamiento, puede exponer un extremo para recibir mensajes de aplicaciones cliente y, a continuación, enrutar cada mensaje a la versión de servicio correcta en función del contenido del mensaje.  La implementación más básica implica agregar un encabezado personalizado al mensaje que indica la versión del servicio con el que se procesará el mensaje.  El servicio de enrutamiento puede utilizar XPathMessageFilter para inspeccionar cada mensaje con el fin de detectar la presencia del encabezado personalizado y enrutar el mensaje al extremo de destino adecuado.  
  
 Para consultar los pasos utilizados para crear una configuración del control de versiones del servicio, vea [Cómo: Control de versiones del servicio](../../../../docs/framework/wcf/feature-details/how-to-service-versioning.md).  Para obtener un ejemplo sobre el uso de XPathMessageFilter para enrutar mensajes en función de un encabezado personalizado, vea el ejemplo [Filtros avanzados](../../../../docs/framework/wcf/samples/advanced-filters.md).  
  
### Particionar datos de servicio  
 ...
 Para consultar los pasos utilizados para crear una configuración de partición de datos de servicio, vea [Cómo: Particionar datos de servicio](../../../../docs/framework/wcf/feature-details/how-to-service-data-partitioning.md).  Para obtener un ejemplo del uso de filtros para crear particiones de datos en función de la dirección URL y los encabezados personalizados, vea el ejemplo [Filtros avanzados](../../../../docs/framework/wcf/samples/advanced-filters.md).  
...
### Enrutamiento dinámico  
 ...
 Para obtener información sobre los pasos utilizados para implementar el enrutamiento dinámico, vea [Cómo: Actualización dinámica](../../../../docs/framework/wcf/feature-details/how-to-dynamic-update.md).  Para obtener un ejemplo de uso del enrutamiento dinámico, vea el ejemplo [Reconfiguración dinámica](../../../../docs/framework/wcf/samples/dynamic-reconfiguration.md).  
...
-   Varios filtros deben devolver **true** al evaluar el mensaje.  
 ...
 Si se cumplen estas condiciones, cada extremo de destino asociado a un filtro que devuelva un valor verdadero recibirá una copia del mensaje.  
...
### Puente protocolar  
 ...
 .  Para obtener un ejemplo sobre el uso del servicio de enrutamiento para traducir los mensajes entre los protocolos, vea el ejemplo [Tratamiento de errores y puentes](../../../../docs/framework/wcf/samples/bridging-and-error-handling.md).  
...
### Control de errores  
 ...
 El servicio de enrutamiento intenta solucionar este escenario proporcionando capacidades de control de errores sólidas para los mensajes que se encuentran con errores de la red o errores relacionados con la comunicación.  Al crear una lista de posibles extremos de destino y asociar esta lista a cada filtro de mensajes, elimina el punto de concentración de errores que se produce cuando solo hay un destino posible.  En caso de error, el servicio de enrutamiento intentará entregar el mensaje al extremo siguiente de la lista hasta que se entregue el mensaje, se produzca un error no relacionado con la comunicación o se hayan agotado todos los extremos.  
  
 Para consultar los pasos utilizados para configurar el control de errores, vea [Cómo: Control de errores](../../../../docs/framework/wcf/feature-details/how-to-error-handling.md).  Para obtener un ejemplo de implementación del control de errores, vea los ejemplos [Tratamiento de errores y puentes](../../../../docs/framework/wcf/samples/bridging-and-error-handling.md) y [Control de errores avanzado](../../../../docs/framework/wcf/samples/advanced-error-handling.md).  
