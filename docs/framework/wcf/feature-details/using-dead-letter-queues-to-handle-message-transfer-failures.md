---
title: "Utilizaci&#243;n de las colas de mensajes no enviados para administrar los errores en la transferencia de mensajes | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Para leer mensajes de una cola de mensajes no enviados personalizada, el URI debe tener el formato: net.msmq://localhost/private/*custom-dlq-name*> donde *custom-dlq-name* es el nombre de la cola de mensajes no enviados personalizada.  
  
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]cómo direccionar las colas, consulte [extremos de servicio y direccionamiento de colas](../../../../docs/framework/wcf/feature-details/service-endpoints-and-queue-addressing.md).  
  
 La pila [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] en el receptor compara las direcciones de las que el servicio está realizando escuchas con la dirección del mensaje. Si las direcciones coinciden, se envía el mensaje; si no, no se envía el mensaje. Esto puede producir problemas al leer de la cola de mensajes no enviados, porque los mensajes en la cola de mensajes no enviados normalmente se direccionan al servicio y no al servicio de la cola de mensajes no enviados. Por consiguiente, el servicio que lee de la cola de mensajes no enviados debe instalar un filtro de direcciones `ServiceBehavior` que indique a la pila que haga coincidir todos los mensajes de la cola independientemente del destinatario. En concreto, debe agregar una `ServiceBehavior` con el <xref:System.ServiceModel.AddressFilterMode> parámetro al servicio de lectura de mensajes de la cola de mensajes no enviados.  
