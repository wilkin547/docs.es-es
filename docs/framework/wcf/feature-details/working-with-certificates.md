...
## <a name="chain-trust-and-certificate-authorities"></a>Confianza de cadena y entidades de certificación  
 ...
 Los certificados digitales se usan para autenticar una entidad confiando en esta jerarquía, que también se denomina *cadena de confianza*. Puede ver la cadena de cualquier certificado mediante el complemento MMC si hace doble clic en cualquier certificado y, después, hace clic en la pestaña **Ruta de acceso de certificado**. Para obtener más información sobre cómo importar cadenas de certificados para una entidad de certificación, consulte [Cómo: Especifique la cadena de certificados de entidad de certificado utilizada para comprobar las firmas](specify-the-certificate-authority-chain-verify-signatures-wcf.md).  
...
## <a name="using-the-powershell-new-selfsignedcertificate-cmdlet-to-build-a-certificate-chain"></a>Mediante el Cmdlet de PowerShell New-SelfSignedCertificate para generar una cadena de certificados  
 El cmdlet New-SelfSignedCertificate de PowerShell crea certificados X.509 y pares de claves pública/clave privados. Puede guardar la clave privada en el disco y, a continuación, usarla para emitir y firmar nuevos certificados, simulando así una jerarquía de certificados encadenados. El cmdlet está pensado para su uso únicamente como una ayuda al desarrollar servicios y nunca debe utilizarse para crear certificados para la implementación real. Al desarrollar un servicio WCF, siga estos pasos para crear una cadena de confianza con el cmdlet New-SelfSignedCertificate.  
...
4. Para obtener instrucciones detalladas, consulte [Cómo: Crear certificados temporales para su uso durante el desarrollo](how-to-create-temporary-certificates-for-use-during-development.md).  
