...
-   **Personal**. Este almacén se utiliza para los certificados asociados a un usuario de un equipo. Normalmente este almacén se usa para los certificados emitidos por uno de los certificados de la entidad de certificación situado en el almacén de las Entidades emisoras de certificados raíz de confianza. O bien, una aplicación puede confiar en un certificado que se encuentre aquí y que sea de emisión propia.  
 ...
 Para obtener más información acerca de los almacenes de certificados, consulte [almacenes de certificados](http://go.microsoft.com/fwlink/?LinkId=88912).  
...
## <a name="chain-trust-and-certificate-authorities"></a>Confianza de cadena y entidades de certificación  
 ...
 Los certificados digitales se utilizan para autenticar una entidad al depender de esta jerarquía, también se denomina un *cadena de confianza*. Puede ver la cadena de cualquier certificado mediante el complemento MMC haciendo doble clic en cualquier certificado, a continuación, haga clic en el **ruta de acceso del certificado** ficha. Para obtener más información sobre la importación de cadenas de certificados para una entidad de certificación, consulte [Cómo: especificar el certificado de entidad certificado cadena usa para comprobar firmas](../../../../docs/framework/wcf/feature-details/specify-the-certificate-authority-chain-verify-signatures-wcf.md).  
...
4.  Para obtener instrucciones detalladas, consulte [Cómo: crear certificados temporales para el uso durante el desarrollo](../../../../docs/framework/wcf/feature-details/how-to-create-temporary-certificates-for-use-during-development.md).  
...
### <a name="service-certificates"></a>Certificados de servicio  
 Los certificados de servicio tienen la tarea principal de autenticar el servidor a los clientes. Una de las comprobaciones iniciales cuando un cliente autentica un servidor es comparar el valor de la **asunto** campo para el identificador uniforme de recursos (URI) utilizado para ponerse en contacto con el servicio: debe coincidir con el DNS de ambos. Por ejemplo, si el URI del servicio es "http://www.contoso.com/endpoint/" la **asunto** campo también debe contener el valor "www.contoso.com".  
  
 Observe que el campo puede contener varios valores, cada uno prefijado con una inicialización que indique el valor. Muy comúnmente, la inicialización es "CN" para designar un nombre común, por ejemplo, "CN = www.contoso.com". También es posible que el **asunto** campo esté en blanco, en cuyo caso el **nombre alternativo del sujeto** campo puede contener el **nombre DNS** valor.  
  
 Tenga en cuenta el valor de la **propósitos planteados** campo del certificado debe incluir un valor apropiado, como "Autenticación de servidor" o "Autenticación de cliente".  
...
### <a name="certificate-validity"></a>Validez del certificado  
 Cada certificado es válido solo durante un determinado período de tiempo, llamado el *período de validez*. El período de validez se define mediante la **válido desde** y **válido hasta** campos de un certificado X.509. Durante la autenticación, el certificado se comprueba para determinar si el certificado todavía está dentro del período de validez.  
...
|<xref:System.ServiceModel.Security.X509CertificateInitiatorServiceCredential.SetCertificate%2A>|  
 ...
 El método `SetCertificate` funciona designando una ubicación del almacén y almacén, un tipo de "búsqueda” (parámetro`x509FindType`) que especifica un campo del certificado, y un valor para encontrar en el campo. Por ejemplo, el código siguiente crea un <xref:System.ServiceModel.ServiceHost> instancia y establece el certificado de servicio usado para autenticar el servicio a los clientes con la `SetCertificate` método.  
