---
title: "Informaci&#243;n general de Detecci&#243;n de WCF | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Detección y seguridad de nivel de mensaje  
 Al usar la seguridad de nivel de mensaje, es necesario especificar una <xref:System.ServiceModel.EndpointIdentity> en el extremo de detección de servicio y una <xref:System.ServiceModel.EndpointIdentity> coincidente en el extremo de detección de cliente.[!INCLUDE[crabout](../../../../includes/crabout-md.md)] la seguridad del nivel de mensaje, vea [Seguridad de los mensajes](../../../../docs/framework/wcf/feature-details/message-security-in-wcf.md).  
  
## Servicios de hospedaje web y detección  
...
2.  Usar un proxy de detección para comunicarse en nombre del servicio  
 ...
 Windows Server AppFabric tiene una característica de inicio automático que permite que un servicio se inicie antes de recibir ningún mensaje.Con este inicio automático establecido, un servicio hospedado por IIS\/WAS se puede configurar como detectable.[!INCLUDE[crabout](../../../../includes/crabout-md.md)] la característica de inicio automático, vea [Característica de inicio automático](http://go.microsoft.com/fwlink/?LinkId=205545).Además de activar la característica de inicio automático, debe configurar el servicio para la detección.[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Cómo: Agregar detectabilidad mediante programación a un cliente y un servicio de WCF](../../../../docs/framework/wcf/feature-details/how-to-programmatically-add-discoverability-to-a-wcf-service-and-client.md)[Configurar la detección en un archivo de configuración](../../../../docs/framework/wcf/feature-details/configuring-discovery-in-a-configuration-file.md).  
  
 Se puede usar un proxy de detección para comunicarse en nombre del servicio WCF cuando el servicio no se está ejecutando.El proxy puede escuchar para sondear o resolver los mensajes, y responder al cliente.El cliente puede entonces enviar los mensajes directamente al servicio.Cuando el cliente envíe un mensaje al servicio, se creará una instancia del mismo para responderlo.[!INCLUDE[crabout](../../../../includes/crabout-md.md)] cómo implementar un proxy de detección, vea [Implementar un proxy de detección](../../../../docs/framework/wcf/feature-details/implementing-a-discovery-proxy.md).