...
## <a name="basic-architecture"></a>Arquitectura Básica  
 ...
 Por lo tanto, WCF incluye un marco de servicio que proporciona un modelo de programación fácil de usar que puede usar para construir y recibir `Message` objetos. El marco del servicio asigna servicios a los tipos de .NET Framework a través de la noción de contratos de servicio y envía mensajes a las operaciones de usuario que son simplemente un .NET Framework, los métodos marcados con el <xref:System.ServiceModel.OperationContractAttribute> atributo (para obtener más información, consulte [ Diseño de contratos de servicio](../../../../docs/framework/wcf/designing-service-contracts.md)). Estos métodos pueden tener parámetros y valores devueltos. En el lado del servicio, el marco de trabajo de servicio convierte las instancias <xref:System.ServiceModel.Channels.Message> de entrada en parámetros y convierte los valores devueltos en instancias <xref:System.ServiceModel.Channels.Message> de salida. En el lado de cliente, hace lo contrario. Por ejemplo, considere la operación `FindAirfare` descrita abajo.  
...
## <a name="message-properties"></a>Message (propiedades)  
 Un mensaje puede contener propiedades. Un *propiedad* es cualquier objeto de .NET Framework que está asociado con un nombre de cadena. Se tiene acceso a las propiedades a través de la propiedad `Properties` en `Message`.  
...
### <a name="3-message-contracts"></a>3. Contratos de mensajes  
 WCF proporciona un modelo de programación declarativo para describir mensajes, denominados *contratos de mensaje*. Este modelo se describe en detalle en [Using Message Contracts](../../../../docs/framework/wcf/feature-details/using-message-contracts.md). En esencia, el mensaje completo se representa mediante un único tipo de .NET Framework que usa atributos como la <xref:System.ServiceModel.MessageBodyMemberAttribute> y <xref:System.ServiceModel.MessageHeaderAttribute> para describir qué partes de la clase de contrato de mensaje se deben asignar a qué parte del mensaje.  
