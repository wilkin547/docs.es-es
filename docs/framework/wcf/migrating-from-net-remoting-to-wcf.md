---
title: "Migraci&#243;n de .NET Remoting a WCF | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
En este artículo se describe el procedimiento para migrar una aplicación que usa .NET Remoting para que use Windows Communication Foundation \(WCF\).  Se comparan conceptos similares entre estos productos y se describe cómo llevar a cabo varios escenarios comunes de comunicación remota en WCF.  
 ...
 .NET Remoting es un producto heredado que solo se admite para la compatibilidad con versiones anteriores.  No es seguro en entornos de confianza mixta porque no puede mantener los niveles de confianza independientes entre el cliente y el servidor.  Por ejemplo, nunca debe exponer un extremo de .NET Remoting a Internet o a los clientes que no son de confianza.  Recomendamos que las aplicaciones de Remoting existentes se migren a tecnologías más recientes y seguras.  Si el diseño de la aplicación solo usa HTTP y es RESTful, recomendamos ASP.NET Web API.  Para obtener más información, consulte ASP.NET Web API.  Si la aplicación se basa en SOAP o necesita protocolos que no sean HTTP, como TCP, recomendamos WCF.  
  
-   [Comparación de .NET Remoting con WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Compare_Top)  
  
    -   [Comparación de la implementación del servidor](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Server_Comp)  
  
    -   [Comparación de la implementación del cliente](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Client_Comp)  
  
    -   [Uso de la serialización](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Serialization_Usage)  
  
    -   [Capacidades de control de excepciones](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Exception_Handling)  
  
    -   [Consideraciones de seguridad](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Security_Considerations)  
  
-   [Migrar a WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migrating_Top)  
  
    -   [¿Por qué migrar de Remoting a WCF?](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Why_Migrate)  
  
    -   [Recomendaciones de migración](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Recommendations)  
  
    -   [Escenarios de migración](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Scenarios)  
  
-   [Resumen](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Summary)  
...
### Comparación de la implementación del servidor  
  
#### Crear un servidor en .NET Remoting  
...
#### Crear un cliente en .NET Remoting  
...
```  
 ...
 Este ejemplo muestra la programación en el nivel de canal porque es más similar al ejemplo de Remoting.  También está disponible el enfoque **Agregar referencia de servicio** de Visual Studio que genera código para simplificar la programación del cliente.  Para obtener más información, vea los temas siguientes:  
  
-   [Programación a nivel de canal de cliente](../../../docs/framework/wcf/extending/client-channel-level-programming.md)  
...
-   *Por valor*: los valores del objeto se serializan en los límites del nivel y se crea una nueva instancia del objeto en el otro nivel.  Las llamadas a los métodos o las propiedades de la nueva instancia solo se ejecutan localmente y no afectan al objeto o al nivel original.  
  
-   *Por referencia*: se serializa una "referencia de objeto" especial en los límites del nivel.  Cuando un nivel interactúa con los métodos o las propiedades de ese objeto, se comunica de nuevo al objeto original del nivel original.  Los objetos por referencia pueden fluir en cualquier dirección: de servidor a cliente o de cliente a servidor.  
...
-   [Serialización en Windows Communication Foundation](http://msdn.microsoft.com/magazine/cc163569.aspx)  
  
<a name="Exception_Handling"></a>   
### Capacidades de control de excepciones  
...
-   [Guía de seguridad de WCF](http://wcfsecurity.codeplex.com/)  
...
-   **.NET Remoting es un producto heredado.** Tal y como se describe en [.NET Remoting](http://msdn.microsoft.com/library/vstudio/72x4h507\(v=vs.100\).aspx), este se considera un producto heredado y no se recomienda para las nuevas implementaciones.  Se recomienda WCF o ASP.NET Web API para aplicaciones nuevas y existentes.  
...
-   **Configurar y hospedar el servicio.** Una vez creado el contrato de servicio, el siguiente paso es configurar un enlace para exponer el servicio a un extremo.  Para obtener más información, consulte [Extremos: direcciones, enlaces y contratos](../../../docs/framework/wcf/feature-details/endpoints-addresses-bindings-and-contracts.md).  
...
5.  A continuación, se puede iniciar el servicio WCF con el siguiente código:  
    ...
    ```  
     ...
     Cuando se inicia este ServiceHost, usa el archivo web.config para establecer el contrato, el enlace y el extremo adecuados.  Para obtener más información sobre los archivos de configuración, consulte [Configuración de servicios mediante archivos de configuración](../../../docs/framework/wcf/configuring-services-using-configuration-files.md).  Este estilo de inicio del servidor se conoce como autohospedaje.  Para obtener más información sobre otras opciones de hospedaje de servicios WCF, consulte [Servicios de hospedaje](../../../docs/framework/wcf/hosting-services.md).  
  
6.  El archivo app.config del proyecto de cliente debe declarar la información de enlace coincidente para el extremo del servicio.  La forma más sencilla de hacerlo en Visual Studio es usar **Agregar referencia del servicio**, que actualizará automáticamente el archivo app.config.  Además, estos cambios se pueden agregar manualmente.  
    ...
    ```  
     ...
     Para obtener más información sobre el uso de **Agregar referencia de servicio**, consulte [How to: Add, Update, or Remove a Service Reference](../Topic/How%20to:%20Add,%20Update,%20or%20Remove%20a%20Service%20Reference.md).  
...
1.  Primero necesitamos definir un contrato de servicio WCF que se corresponda con el propio objeto con sesión.  
    ...
    > [!TIP]
    >  Tenga en cuenta que el objeto con sesión está marcado con \[ServiceContract\], lo que hace que sea una interfaz de servicio WCF normal.  El hecho de establecer la propiedad SessionMode indica que será un servicio con sesión.  En WCF, una sesión es un modo de asociar varios mensajes enviados entre dos extremos.  Esto significa que cuando un cliente obtiene una conexión a este servicio, se establecerá una sesión entre el cliente y el servidor.  El cliente usará una única instancia del objeto del lado servidor para todas las interacciones dentro de esta sesión única.  
...
3.  Ahora necesitamos un modo de obtener una instancia de este objeto con sesión.  Para ello, creamos otra interfaz de servicio WCF que devuelva un objeto EndpointAddress10.  Se trata de un formato serializable de un extremo que el cliente puede usar para crear el objeto con sesión.  
...
4.  Debemos modificar el archivo de configuración del servidor mediante las siguientes dos cosas que se muestran en el siguiente ejemplo:  
    ...
    1.  Declare una sección \<client\> que describa el extremo para el objeto con sesión.  Esto es necesario porque el servidor también actúa como un cliente en esta situación.  
  
    2.  Declare los extremos para el objeto de generador y el objeto con sesión.  Esto es necesario para permitir que el cliente se comunique con los extremos del servicio para adquirir EndpointAddress10 y para crear el canal con sesión.  
...
5.  Configuramos el cliente declarando estos mismos extremos en el archivo app.config de su proyecto.  
...
#### Escenario 3: el cliente envía al servidor una instancia por valor  
