...
>   
>  Además, el `/r` y `/ct` conmutadores para hacer referencia a tipos son para generar contratos de datos. Estos modificadores no funcionan al utilizar XmlSerializer.  
...
|/ namespace:\<cadena, cadena >|Especifica una asignación de un targetNamespace WSDL o esquema XML a un espacio de nombres CLR. Utilizando '\*' para targetNamespace asignan todos los targetNamespaces sin una asignación explícita a ese espacio de nombres CLR.<br /><br /> Para asegurarse de que el nombre de contrato de mensaje no produce una colisión con el nombre de la operación, debería calificar la referencia del tipo con `::` o asegurarse de que los nombres son únicos.<br /><br /> Valor predeterminado: Se deriva del espacio de nombres de destino del documento de esquema para contratos de datos. El espacio de nombres predeterminado se utiliza para todos los otros tipos generados.<br /><br /> Forma abreviada: `/n` **Nota:** al generar tipos para utilizarlos con XmlSerializer, se admite solo una asignación de espacio de nombres único. Todos los tipos generados será en el espacio de nombres predeterminado o el espacio de nombres especificado por ' *'.|  
...
|/serializer:Auto|Seleccionar automáticamente el serializador. Esto intenta utilizar el serializador de contratos de datos y utiliza XmlSerializer si eso no funciona.<br /><br /> Forma abreviada: `/ser`|  
...
|/UseSerializerForFaults|Especifica que la <!--zz <xref:System.Xml.XmlSerializer> --> `xref:System.Xml.XmlSerializer ` debe usarse para leer y escribir los errores, en lugar del predeterminado <xref:System.Runtime.Serialization.DataContractSerializer>.|  
