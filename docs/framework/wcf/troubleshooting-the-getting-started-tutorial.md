...
**Al intentar ejecutar la aplicación de servicio: HTTP no pudo registrar la dirección URL `http://+:8000/ServiceModelSamples/Service/`.** 
...
**Compilar la aplicación cliente: 'CalculatorClient' no contiene una definición para '\<nombre de método >' y no hay ningún método de extensión '\<nombre de método >' acepte un primer argumento de tipo 'CalculatorClient' se encontró (¿ Falta el uso de una directiva o una referencia de ensamblado?)**  
...
**Ejecuta el cliente: excepción no controlada: System.ServiceModel.EndpointNotFoundException: no se pudo conectar a `http://localhost:8000/ServiceModelSamples/Service/CalculatorService`. Código de error TCP 10061: se estableció ninguna conexión porque el equipo de destino rechazó.**
