...
En este tema se muestra cómo habilitar la seguridad de transporte en un servicio de Windows Communication Foundation (WCF) que reside en un dominio de Windows y al que llaman los clientes del mismo dominio. Para obtener más información acerca de este escenario, vea [seguridad de transporte con autenticación de Windows](../../../docs/framework/wcf/feature-details/transport-security-with-windows-authentication.md). Para obtener una aplicación de ejemplo, vea el ejemplo [wsHttpBinding](../../../docs/framework/wcf/samples/wshttpbinding.md) .
...
Este es el segundo procedimiento, que muestra cómo usar el enlace en un servicio autohospedado. Para obtener más información acerca de los servicios de hospedaje, vea [servicios de hospedaje](../../../docs/framework/wcf/hosting-services.md).
...
2. Cree una variable <xref:System.Type>`contractType` con nombre y asígnele el tipo de la interfaz (`ICalculator`). Al utilizar Visual Basic, utilice el `GetType` operador; cuando use C#, utilice la `typeof` palabra clave.
...
Este procedimiento muestra cómo generar un proxy que se comunica con el servicio. El proxy se genera con la [herramienta de utilidad de metadatos de ServiceModel (SvcUtil. exe),](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) que usa los metadatos del servicio para crear el proxy.
...
Si aún no tiene un servicio definido, vea [diseñar e implementar servicios](../../../docs/framework/wcf/designing-and-implementing-services.md)y [configurar servicios](../../../docs/framework/wcf/configuring-services.md).
...
1. Agregue un [ \<elemento wsHttpBinding >](../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md) a la [ \<sección de enlaces >](../../../docs/framework/configure-apps/file-schema/wcf/bindings.md) elemento del archivo de configuración.

2. Agregue un elemento`binding`< > al elemento <`WSHttpBinding`> y establezca el `configurationName` atributo en un valor adecuado para su aplicación.

3. Agregue un elemento`security`< > y establezca el `mode` atributo en Message.

4. Agregue un elemento`message`< > y establezca el `clientCredentialType` atributo en Windows.

5. En el archivo de configuración del servicio, reemplace la sección `<bindings>` con el código siguiente. Si aún no tiene un archivo de configuración de servicio, consulte [uso de enlaces para configurar servicios y clientes](../../../docs/framework/wcf/using-bindings-to-configure-services-and-clients.md).
...
1. Use la herramienta SvcUtil.exe para generar el código proxy y el archivo de configuración a partir de los metadatos del servicio. Para obtener más información, vea [Cómo: Cree un cliente](../../../docs/framework/wcf/how-to-create-a-wcf-client.md).

2. Reemplace la [ \<sección de enlaces >](../../../docs/framework/configure-apps/file-schema/wcf/bindings.md) del archivo de configuración generado por el código de configuración de la sección anterior.
