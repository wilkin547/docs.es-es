---
title: 'Punto de conexión: duración de las llamadas'
ms.date: 03/30/2017
ms.assetid: 1aebe748-ad05-4ade-819c-d480379ccfb9
ms.openlocfilehash: 93472287e73393e00921a0ae27959980b9751c30
ms.sourcegitcommit: 27a15a55019f6b5f2733961738babe94aec0def3
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/15/2020
ms.locfileid: "90545773"
---
# <a name="endpoint-calls-duration"></a>Punto de conexión: duración de las llamadas
La duración media de las llamadas a este punto de conexión.  La duración media se calcula aplicando esta ecuación: (N1-N0)/(D1-D0).  
  
## <a name="see-also"></a>Vea también

- [PERF_COUNTER_TYPE](/previous-versions/windows/it-pro/windows-server-2003/cc740048(v=ws.10))
