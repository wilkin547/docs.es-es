---
title: "Riesgos de seguridad relativos al registro de mensajes | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Registro PII conocido activado: se emite este evento cuando el registro de PII conocido está habilitado.Esto sucede cuando el atributo `enableLoggingKnownPii`  en el elemento `machineSettings` del archivo Machine.config se establece en `true`, y el atributo `logKnownPii` del elemento `source` en el archivo App.config o Web.config se establece en `true`.  
  
-   Registro de PII conocido no permitido: se emite este evento cuando no se permite el registro de PII conocido.Esto sucede cuando el atributo `logKnownPii` del elemento `source` en el archivo App.config o Web.config se establece en `true`, pero el atributo `enableLoggingKnownPii`  en el elemento `machineSettings` del archivo Machine.config se establece en `false`.No se produce ninguna excepción.  
  
 Estos eventos se pueden ver en la herramienta Visor de eventos que viene con Windows.Para obtener más información sobre este tema, vea [Registro de eventos](../../../../docs/framework/wcf/diagnostics/event-logging/index.md).  
