...
2. Abra un proyecto de servicio, seleccione **Build** > **Publish \<Project name >** en el menú principal, o haga clic con el botón derecho en el proyecto en **Explorador de soluciones** y haga clic en **Publish (publicar**).
...
4. Después de hacer clic en **publicar** en la ventana principal, Visual Studio implementa la aplicación en la ubicación de destino especificada y copia los archivos Web. config,. SVC y de ensamblado en el directorio de destino. . El nombre de. SVC será "ProjectName. ServiceName. SVC". Una vez que el servicio se publique correctamente, puede encontrar un enlace en la ventana de salida de Visual Studio, que tiene un aspecto similar a "conectándose a `http://localhost/WebApplicationFolderName...`". Puede presionar CTRL y hacer clic el vínculo para abrir una página del explorador dentro de Visual Studio y ver la estructura de directorios del servicio.
...
## <a name="files-generated-for-publishing"></a>Archivos generados para publicación
 Antes de que una biblioteca de servicios WCF pueda hospedarse en Web, la herramienta genera los siguientes archivos: archivos de ensamblado, archivo Web. config y archivo. SVC. Todos los archivos se copian en la ubicación de destino. A continuación se publica el servicio.
...
### <a name="webconfig-file"></a>Archivo Web.config
 ...
 El archivo Web. config generado incluye secciones web que son útiles para el hospedaje web y el contenido de App. config para la biblioteca de servicios WCF con los siguientes cambios:
