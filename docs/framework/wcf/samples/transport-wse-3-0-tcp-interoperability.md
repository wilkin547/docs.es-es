...
5.  Agregue un elemento de enlace que agrega el transporte personalizado a una pila de canales. [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Agregando un elemento de enlace].  
...
## <a name="creating-iduplexsessionchannel"></a>Creación de IDuplexSessionChannel  
 ...
 Una vez que <xref:System.ServiceModel.Channels.Message> se codifica en bytes, se debe transmitir en la conexión. Esto requiere un sistema para definir los límites del mensaje. WSE 3.0 usa una versión de [DIME](http://go.microsoft.com/fwlink/?LinkId=94999) como su protocolo de tramas. `WriteData` encapsula la lógica con tramas para ajustar un byte[] en un conjunto de registros de DIME.  
...
-   `ClientWseTcpDuplexSessionChannel`agrega lógica a la base de `WseTcpDuplexSessionChannel` para conectarse a un servidor TCP en `channel.Open` tiempo. Primero el nombre de host se resuelve como una dirección IP, como se muestra en el código siguiente.  
...
1.  Para ejecutar este ejemplo, debe tener WSE 3.0 y el ejemplo `TcpSyncStockService` de WSE instalados. Puede descargar [WSE 3.0 de MSDN](http://go.microsoft.com/fwlink/?LinkId=95000).  
