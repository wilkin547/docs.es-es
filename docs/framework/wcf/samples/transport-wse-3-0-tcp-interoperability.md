---
title: "Transporte: interoperabilidad de WSE 3.0 TCP | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Transporte: interoperabilidad de WSE 3.0 TCP
...
5.  Agregue un elemento de enlace que agrega el transporte personalizado a una pila de canales.[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Agregar un elemento de enlace](#AddingABindingElement).  
  
## Creación de IDuplexSessionChannel  
 El primer paso a la hora de escribir el transporte de interoperabilidad TCP de WSE 3.0 es crear una implementación de <xref:System.ServiceModel.Channels.IDuplexSessionChannel> encima de un <xref:System.Net.Sockets.Socket>.`WseTcpDuplexSessionChannel` se deriva de <xref:System.ServiceModel.Channels.ChannelBase>.La lógica para enviar un mensaje consta de dos elementos principales: \(1\) codificar el mensaje en bytes y \(2\) hacer tramas de esos bytes y enviarlos en la conexión.  
 ...
 Una vez que <xref:System.ServiceModel.Channels.Message> se codifica en bytes, se debe transmitir en la conexión.Esto requiere un sistema para definir los límites del mensaje.WSE 3.0 usa una versión de [DIME](http://go.microsoft.com/fwlink/?LinkId=94999) como protocolo de trama.`WriteData` encapsula la lógica con tramas para ajustar un byte\[\] en un conjunto de registros de DIME.  
...
-   `ClientWseTcpDuplexSessionChannel` agrega lógica al `WseTcpDuplexSessionChannel`base para conectarse a un servidor TCP en el momento de `channel.Open`.Primero el nombre de host se resuelve como una dirección IP, como se muestra en el código siguiente.  
...
## La consola de prueba de WSE TCP  
 ...
 Está compuesto de dos pruebas: una configura un cliente con tipo usando el código generado a partir de WSDL de WSE 3.0.La segunda prueba utiliza [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] como el cliente y el servidor enviando mensajes directamente encima de las API de canal.  
...
1.  Para ejecutar este ejemplo, debe tener WSE 3.0 y el ejemplo `TcpSyncStockService` de WSE instalados.Puede descargar [WSE 3.0 desde MSDN](http://go.microsoft.com/fwlink/?LinkId=95000).  
...
1.  Una vez instalado el ejemplo de `TcpSyncStockService`, haga lo siguiente:  
    ...
    3.  Abra StockService.cs en el proyecto StockService y marque como comentario el atributo \[Policy\] en la clase `StockService`.Esto deshabilita la seguridad del ejemplo.Aunque [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] puede interoperar con los extremos seguros de WSE 3.0, se deshabilita la seguridad para mantener este ejemplo centrado en el transporte de TCP personalizado.  
