...
1. Asegúrese de que ASP.NET está configurado. Para obtener más información sobre cómo configurar ASP.NET, consulte [instrucciones de hospedaje de Internet Information Services](internet-information-service-hosting-instructions.md).
...
4. Ejecute los comandos siguientes: Para obtener más información sobre por qué se deben ejecutar estos comandos, vea el [servicio hospedado de IIS produce un error](/previous-versions/dotnet/netframework-3.5/ms752252(v=vs.90)).
    ...
    > [!WARNING]
    > La ejecución del comando `aspnet_regiis –i –enable` hará que el grupo de aplicaciones predeterminado se ejecute con .NET Framework 4, lo que puede producir problemas de incompatibilidad con otras aplicaciones del mismo equipo.

5. Siga las [instrucciones de Firewall](firewall-instructions.md) para habilitar los puertos que se usan en los ejemplos.
...
7. Si los ejemplos no están instalados, instálelos desde [Windows Communication Foundation (WCF) y los ejemplos de Windows Workflow Foundation (WF) para .NET Framework 4](https://www.microsoft.com/download/details.aspx?id=21459).
...
9. Ejecute el archivo por lotes **Setupvroot.bat** . Hay que seguir estos pasos:
    ...
    Si prefiere configurar estos directorios manualmente, consulte las [instrucciones de configuración del directorio virtual](virtual-directory-setup-instructions.md). Para revertir todos los cambios efectuados en este paso, ejecute cleanupvroot.bat después de que termine de utilizar los ejemplos.
...
11. Cree un directorio C:\logs en el equipo; en algunos ejemplos podría esperarse que exista. Asegúrese de que la cuenta adecuada tiene concedido acceso de escritura en esta carpeta. En Windows 7, Windows Vista y Windows Server 2008 R2, esta cuenta es **servicio de red**. En Windows Server 2008, la cuenta es NT Authority\Network Service. En Windows XP y Windows Server 2003, la cuenta es ASPNET.

12. Ejecute el archivo Setupcerttool.bat. Este archivo se encuentra en la  \<InstallPath> carpeta \ WF_WCF_Samples \wcf\setup\  Este script realizará las siguientes tareas:
...
14. Algunos ejemplos requieren Message Queuing. Vea [instalar Message Queue Server (MSMQ)](installing-message-queuing-msmq.md) para obtener instrucciones de instalación.
