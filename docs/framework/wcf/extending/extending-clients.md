...
- Modelo de datos personalizado. Es posible que un usuario desee tener un modelo de datos o de serialización distinto de los que se admiten de forma <xref:System.Runtime.Serialization.DataContractSerializer?displayProperty=nameWithType>predeterminada <xref:System.Xml.Serialization.XmlSerializer?displayProperty=nameWithType>en WCF <xref:System.ServiceModel.Channels.Message?displayProperty=nameWithType> (es decir, los objetos, y). Esto se puede hacer implementando las interfaces del formateador de mensajes. Para obtener más información, vea <xref:System.ServiceModel.Dispatcher.IClientMessageFormatter?displayProperty=nameWithType> y la propiedad <xref:System.ServiceModel.Dispatcher.ClientOperation.Formatter%2A?displayProperty=nameWithType>.  
  
- Validación personalizada de parámetros. Un usuario puede querer exigir que los parámetros con tipo sean válidos (por oposición a XML). Esto puede hacerse mediante las interfaces del inspector de parámetros. Como ejemplo, vea [Cómo: Inspeccione o modifique](how-to-inspect-or-modify-parameters.md) los parámetros o la [validación del cliente](../samples/client-validation.md).  
...
- <xref:System.ServiceModel.Dispatcher.ClientRuntime.CallbackDispatchRuntime%2A>  
 ...
 Para obtener ejemplos que muestran la interceptación en un cliente de [WCF, consulte Cómo: Inspeccionar o modificar mensajes en el](how-to-inspect-or-modify-messages-on-the-client.md)cliente.  
...
- <xref:System.ServiceModel.Dispatcher.ClientOperation.BeginMethod%2A> Utilice y <xref:System.ServiceModel.Dispatcher.ClientOperation.EndMethod%2A> para especificar qué métodos de cliente WCF están asociados a una operación asincrónica.  
...
- Use la propiedad <xref:System.ServiceModel.Dispatcher.ClientOperation.SyncMethod%2A> para controlar qué método se asigna a la operación.  
 ...
 Para obtener ejemplos que muestran la interceptación en un cliente de [WCF, consulte Cómo: Inspeccionar o modificar](how-to-inspect-or-modify-parameters.md)parámetros.  
...
- [Procedimientos: Inspeccionar o modificar parámetros](how-to-inspect-or-modify-parameters.md)
