...
|------------------------|-----------------|  
|`winmdmodule`|Especifica el módulo \(.winmdobj\) que se va a exportar.  Solo se permite un módulo.  Para crear este módulo, utilice la opción del compilador `/target` con el destino de `winmdobj`.  Vea [\/target:winmdobj \(Crear un archivo intermedio para Windows en tiempo de ejecución\)](../Topic/-target:winmdobj%20\(C%23%20Compiler%20Options\).md) o [\/target](../Topic/-target%20\(Visual%20Basic\).md) en MSDN Library.|  
...
|`/nowarn:` `warning`|Suprime el número de advertencias especificadas.  Para *warning*, proporcione solo la parte numérica del código de error, sin ceros iniciales.|  
...
## Vea también  
 ...
 [Build, Deployment, and Configuration Tools \(.NET Framework\)](http://msdn.microsoft.com/es-es/b8c921be-6012-4181-b8d4-ab15813fc9a7)