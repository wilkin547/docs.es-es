...
|Opción|Descripción|  
|------------|-----------------|  
|**\/asmversion:** *númeroVersión*|Especifica el número de versión del ensamblado que se genera.  Especifique *númeroVersión* en el formato *principal.secundaria.compilación.revisión*.|  
...
|**\/delaysign**|Indica a la herramienta Tlbimp.exe que firme el ensamblado resultante con un nombre seguro mediante la firma retardada.  Esta opción se debe especificar con la opción **\/keycontainer:**, **\/keyfile:** o la opción **\/publickey:**.  Para obtener más información sobre el proceso de firma retardada, vea [Retrasar la firma de un ensamblado](../../../docs/framework/app-domains/delay-sign-assembly.md).|  
...
|**\/namespace:** *espacioNombres*|Especifica el espacio de nombres en que se genera el ensamblado.|  
...
|**\/publickey:** *nombreArchivo*|Especifica el archivo que contiene la clave pública que se va a utilizar para firmar el ensamblado resultante.  Si se especifica la opción **\/keyfile:** o la opción **\/keycontainer:** en vez de especificar **\/publickey:**, Tlbimp.exe genera la clave pública a partir del par de claves pública y privada proporcionado con **\/keyfile:** o **\/keycontainer:**.  La opción **\/publickey:** admite los escenarios de clave de prueba y de firma retardada.  El archivo tiene el formato generado por Sn.exe.  Para obtener más información, vea la opción **\-p** de Sn.exe en la [herramienta Nombre seguro \(Sn.exe\)](../../../docs/framework/tools/sn-exe-strong-name-tool.md).|  
...
|**\/transform:** *nombreTransformación*|Transforma los metadatos de la forma especificada por el parámetro *nombreTransformación*.<br /><br /> Especifique **dispret** para que el parámetro *nombreTransformación* transforme los parámetros \[out, retval\] de los métodos de las interfaces de solo de envío \(interfaces dispinterface\) en valores devueltos.<br /><br /> Para obtener más información sobre esta opción, vea los ejemplos que se muestran más adelante en este tema.|  
...
## Vea también  
 ...
 [Attributes for Importing Type Libraries into Interop Assemblies](http://msdn.microsoft.com/es-es/81e587b8-393f-43e1-9add-c4b05e65cbfd)   
