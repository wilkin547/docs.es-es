...
-   Un archivo de recursos de referencia cultural se puede guardar como otro archivo de recursos de referencia cultural.  Por ejemplo, se puede guardar un archivo de recursos de inglés \(EE. UU.\) como un archivo de recursos de polaco.  Normalmente, el nuevo archivo se editaría más adelante para admitir la nueva referencia cultural.  
 ...
 Consulte también [Organización jerárquica de recursos para la localización](http://msdn.microsoft.com/library/756hydy4\(v=vs.110\)) u [Organización jerárquica de recursos para la localización](http://msdn.microsoft.com/library/756hydy4\(v=vs.120\)).  
  
 Winres.exe no puede convertir un archivo .resx en el archivo .resources correspondiente; en su lugar, utilice la herramienta Resgen.exe.  Para obtener más información acerca de Resgen.exe, consulte [Resgen.exe \(Resource File Generator\)](../../../docs/framework/tools/resgen-exe-resource-file-generator.md).  
...
## Comparación entre Winres.exe y Visual Studio  
...
## Uso de Winres.exe  
 Para adaptar mediante Winres.exe, primero se debe desarrollar una aplicación con un diseñador visual, como el diseñador de formularios de Visual Studio.  Una vez completado el desarrollo, establezca el atributo <xref:System.ComponentModel.LocalizableAttribute> del formulario \(propiedad **Localizable** en el editor de propiedades\) en `true` y, a continuación, transfiera el archivo .resx de la referencia cultural predeterminada a un localizador de terceros.  El archivo .resx contiene información adicional que Winres.exe usa para crear una nueva versión en tiempo de diseño del formulario original.  
...
>  Winres.exe no se puede utilizar para editar el archivo de recursos predeterminado.  Winres.exe interpreta todas las propiedades cambiadas como propiedades adaptadas y las guarda en el archivo de recursos de la referencia cultural de destino.  
 ...
 Las versiones finales de los archivos de recursos de referencia cultural se pueden utilizar para crear versiones localizadas de la aplicación.  Para obtener más información, consulte [Recursos de aplicaciones de escritorio](../../../docs/framework/resources/index.md).  
...
#### Cómo adaptar un archivo .resx o .resources asociado a un formulario  
  
1.  Escriba `winres` en el símbolo del sistema para desarrolladores para ejecutar Winres.exe.  
  
2.  Para abrir los recursos predeterminados de un formulario que desee adaptar, haga clic en el comando **Abrir** del menú **Archivo** y navegue hasta el archivo que desee abrir.  
    ...
    > [!NOTE]
    >  Si el formulario cuyos recursos va a editar es un formulario heredado, tanto el ensamblado que contiene el formulario heredado como el que contiene el formulario que hereda \(derivado\) deben estar registrados en la caché global de ensamblados \(GAC\), o deben residir en el mismo directorio que WinRes.exe.  Para obtener más información acerca de cómo instalar componentes de .NET Framework en la caché global de ensamblados, consulte [Caché global de ensamblados](../../../docs/framework/app-domains/gac.md).  
