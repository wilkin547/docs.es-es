...
description: Referencia del lenguaje F#
...
Esta sección es una referencia del lenguaje F#, un lenguaje de programación multiparadigma destinado a la plataforma .NET. El lenguaje F# admite los modelos de programación funcional, orientada a objetos e imperativa.
...
|[Resource Management: The `use` Keyword](resource-management-the-use-keyword.md) (Administración de recursos: la palabra clave `use`)|Describe las palabras clave `use` y `using`, que permiten controlar la inicialización y la liberación de los recursos.|
...
|[Expresiones con procesamiento diferido](lazy-computations.md)|Describe los cálculos diferidos, que son cálculos que no se evalúan de inmediato sino cuando realmente se necesita el resultado.|
