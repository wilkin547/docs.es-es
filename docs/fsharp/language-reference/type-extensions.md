...
* Cambiar `member this.Sum` a `member inline this.Sum` generará un error que no coinciden las restricciones de tipo.
...
Por último, los métodos de extensión (a veces denominado "C# miembros de extensión de estilo") se pueden declarar en F# como un método de miembro estático en una clase.
...
* Puede definir extensiones que sobrecargan otros métodos del mismo nombre, pero el F# compilador da preferencia a los métodos que no sean de extensión si hay una llamada ambigua.
