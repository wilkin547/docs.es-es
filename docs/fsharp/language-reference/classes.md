...
El `inherit` cláusula identifica la clase base directa, si hay alguno. En F#, sólo uno directa se permite la clase base. Las interfaces que implementa una clase no se consideran clases base. Las interfaces se describen en la [Interfaces](Interfaces.md) tema.
...
Puede definir estático o métodos de instancia, propiedades, implementaciones de interfaz, los miembros abstractos, declaraciones de eventos y constructores adicionales en esta sección. Let y realizan enlaces no puede aparecer en esta sección. Dado que se pueden agregar miembros a una variedad de F# tipos además de clases, se tratan en otro tema, [miembros](members/index.md).
