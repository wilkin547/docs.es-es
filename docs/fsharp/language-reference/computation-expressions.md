...
`let!` está definido por el miembro `Bind(x, f)` en el tipo de generador.
...
`return` está definido por el miembro `Return(x)` en el tipo de generador, donde `x` es el elemento que se va a ajustar.
