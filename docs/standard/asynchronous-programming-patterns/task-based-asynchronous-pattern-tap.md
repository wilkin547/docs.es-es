...
## <a name="naming-parameters-and-return-types"></a>Nombres, parámetros y tipos de valores devueltos  
 TAP usa un solo método para representar el inicio y la finalización de una operación asincrónica. Esto contrasta con el modelo de programación asincrónica (APM o `IAsyncResult`), que necesita los métodos `Begin` y `End`, y también con el patrón asincrónico basado en eventos (EAP), que necesita un método que tenga el sufijo `Async` y también necesita uno o más eventos, tipos de delegado de controlador de eventos y tipos derivados de `EventArg`. Métodos asincrónicos de TAP incluyen el `Async` sufijo después del nombre de la operación; por ejemplo, `GetAsync` para un `Get` operación. Si va a agregar un método de TAP a una clase que ya contiene ese nombre de método con el sufijo `Async`, use el sufijo `TaskAsync` en su lugar. Por ejemplo, si la clase ya tiene un método `GetAsync`, use el nombre `GetTaskAsync`.  
  
 El método de TAP devuelve <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> o <xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType>, en función de si el método sincrónico correspondiente devuelve void o un tipo `TResult`.  
  
 Los parámetros de un método de TAP deben coincidir con los parámetros de su homólogo sincrónico y se deben proporcionar en el mismo orden.  Sin embargo, los parámetros `out` y `ref` están exentos de esta regla y se deben evitar completamente. En su lugar, los datos que se hubieran devuelto con un parámetro `out` o `ref` se deben devolver como parte del tipo `TResult` devuelto por <xref:System.Threading.Tasks.Task%601> y deben usar una tupla o una estructura de datos personalizada para incluir varios valores. Métodos que están dedicados exclusivamente a la creación, manipulación o combinación de tareas (donde el intento asincrónico del método es borrar en el nombre del método o el nombre del tipo al que pertenece el método) no necesitan seguir este patrón de nombres; Estos métodos se conocen a menudo como *combinadores*. Ejemplos de combinadores <xref:System.Threading.Tasks.Task.WhenAll%2A> y <xref:System.Threading.Tasks.Task.WhenAny%2A>y se describen en la [mediante el Combinadores basado en tareas integradas](../../../docs/standard/asynchronous-programming-patterns/consuming-the-task-based-asynchronous-pattern.md#combinators) sección del artículo [utilizar el modelo asincrónico basado en tareas](../../../docs/standard/asynchronous-programming-patterns/consuming-the-task-based-asynchronous-pattern.md).  
  
 Para obtener ejemplos de cómo la sintaxis TAP difiere de la sintaxis usada en patrones de programación asincrónicas heredadas, como la programación modelo asincrónica (APM) y el patrón asincrónico basado en eventos (EAP), consulte [patrones de programación asincrónica ](../../../docs/standard/asynchronous-programming-patterns/index.md).  
  
## <a name="initiating-an-asynchronous-operation"></a>Iniciar una operación asincrónica  
...
## <a name="choosing-the-overloads-to-provide"></a>Elegir las sobrecargas que se van a proporcionar  
