...
## Cifrar un archivo  
 Esta tarea implica dos métodos: el método del controlador de eventos del botón `Encrypt File` \(`buttonEncryptFile_Click`\) y el método `EncryptFile`.  El primer método muestra un cuadro de diálogo para seleccionar un archivo y pasa el nombre del archivo al segundo método, que lleva a cabo el cifrado.  
