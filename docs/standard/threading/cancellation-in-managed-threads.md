...
-   Un agente de escucha puede escuchar varios tokens simultáneamente mediante la combinación de ellos en una *token vinculado*.  
...
> [!NOTE]
>  En el ejemplo se usa el método <xref:System.Threading.ThreadPool.QueueUserWorkItem%2A> para demostrar que el nuevo marco de cancelación es compatible con las API heredadas. Para obtener un ejemplo que usa el nuevo, preferido <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> los tipos, vea [Cómo: cancelar una tarea y sus elementos secundarios](../../../docs/standard/parallel-programming/how-to-cancel-a-task-and-its-children.md).  
...
## <a name="listening-and-responding-to-cancellation-requests"></a>Escucha y respuesta a solicitudes de cancelación  
 ...
 La clase <xref:System.Threading.Tasks.Task> administra <xref:System.OperationCanceledException> de esta manera. Para obtener más información, consulta [Task Cancellation](../../../docs/standard/parallel-programming/task-cancellation.md).  
...
### <a name="listening-by-polling"></a>Escuchas mediante sondeo  
 ...
 Para obtener un ejemplo más completo, vea [Cómo: realizar escuchas de solicitudes mediante sondeo](../../../docs/standard/threading/how-to-listen-for-cancellation-requests-by-polling.md).  
...
### <a name="listening-by-registering-a-callback"></a>Escuchas mediante el registro de una devolución de llamada  
 ...
 El <xref:System.Threading.CancellationToken.Register%2A> método devuelve un <xref:System.Threading.CancellationTokenRegistration> objeto que se utiliza específicamente para este propósito. En el ejemplo siguiente se muestra cómo usar el método <xref:System.Threading.CancellationToken.Register%2A> para cancelar una solicitud web asincrónica.  
...
-   Las devoluciones de llamada no deben realizar ningún subproceso manual ni uso de o <xref:System.Threading.SynchronizationContext> en una devolución de llamada. Si una devolución de llamada debe ejecutarse en un subproceso concreto, use el constructor <xref:System.Threading.CancellationTokenRegistration?displayProperty=nameWithType> que le permite especificar que la clase syncContext de destino es el <xref:System.Threading.SynchronizationContext.Current%2A?displayProperty=nameWithType> activo. Si se realiza un subproceso manual en una devolución de llamada, puede producirse un interbloqueo.  
 ...
 Para obtener un ejemplo más completo, vea [Cómo: registrar devoluciones de llamada para las solicitudes de cancelación](../../../docs/standard/threading/how-to-register-callbacks-for-cancellation-requests.md).  
...
### <a name="listening-by-using-a-wait-handle"></a>Escuchas mediante un identificador de espera  
 ...
 En el nuevo código destinado a [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)], <xref:System.Threading.ManualResetEventSlim?displayProperty=nameWithType> y <xref:System.Threading.SemaphoreSlim?displayProperty=nameWithType> admiten el nuevo marco de cancelación en sus métodos `Wait`. Puede pasar el <xref:System.Threading.CancellationToken> al método, y cuando se solicita la cancelación, el evento se activa y se produce un <xref:System.OperationCanceledException>.  
  
 [!code-csharp[Cancellation#6](../../../samples/snippets/csharp/VS_Snippets_Misc/cancellation/cs/cancellationex10.cs#6)]
 [!code-vb[Cancellation#6](../../../samples/snippets/visualbasic/VS_Snippets_Misc/cancellation/vb/cancellationex10.vb#6)]  
  
 Para obtener un ejemplo más completo, vea [Cómo: realizar escuchas de cancelación las solicitudes que tienen identificadores de espera](../../../docs/standard/threading/how-to-listen-for-cancellation-requests-that-have-wait-handles.md).  
...
### <a name="listening-to-multiple-tokens-simultaneously"></a>Escucha de varios tokens simultáneamente  
 ...
 Tenga en cuenta que debe llamar a `Dispose` en el origen de tokens vinculados cuando haya terminado con él. Para obtener un ejemplo más completo, vea [Cómo: realizar escuchas de varias solicitudes de cancelación](../../../docs/standard/threading/how-to-listen-for-multiple-cancellation-requests.md).  
