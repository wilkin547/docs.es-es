---
title: "Utilizar la clase StringBuilder en .NET Framework | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
helpviewer_keywords: 
  - "Remove (método)"
  - "cadenas [.NET Framework], capacidades"
  - "StringBuilder (objeto)"
  - "Replace (método)"
  - "AppendFormat (método)"
  - "Append (método)"
  - "Insert (método)"
  - "cadenas [.NET Framework], objeto StringBuilder"
...
El objeto <xref:System.String> es inmutable. Cada vez que se usa uno de los métodos de la clase <xref:System.String?displayProperty=fullName>, se crea un nuevo objeto de cadena en la memoria, lo que requiere una nueva asignación de espacio para ese objeto. En las situaciones en las que es necesario realizar modificaciones repetidas en una cadena, la sobrecarga asociada a la creación de un nuevo objeto <xref:System.String> puede disminuir el rendimiento. La clase <xref:System.Text.StringBuilder?displayProperty=fullName> se puede usar para modificar una cadena sin crear un objeto nuevo. Por ejemplo, el uso de la clase <xref:System.Text.StringBuilder> puede mejorar el rendimiento al concatenar muchas cadenas en un bucle.  
  
## Importar el espacio de nombres System.Type  
...
## Crear instancias de un objeto StringBuilder  
 Para crear una nueva instancia de la clase <xref:System.Text.StringBuilder>, inicialice la variable con uno de los métodos de constructor sobrecargado, como se muestra en el ejemplo siguiente.  
...
|-----------------------|---------|  
|<xref:System.Text.StringBuilder.Append%2A?displayProperty=fullName>|Anexa información al final del objeto **StringBuilder** actual.|  
|<xref:System.Text.StringBuilder.AppendFormat%2A?displayProperty=fullName>|Reemplaza a un especificador de formato que se pasa en una cadena con texto con formato|  
|<xref:System.Text.StringBuilder.Insert%2A?displayProperty=fullName>|Inserta una cadena o un objeto en el índice especificado del elemento **StringBuilder** actual.|  
|<xref:System.Text.StringBuilder.Remove%2A?displayProperty=fullName>|Quita el número de caracteres especificado del objeto **StringBuilder** actual.|  
|<xref:System.Text.StringBuilder.Replace%2A?displayProperty=fullName>|Reemplaza un carácter concreto en un índice especificado.|  
  
### Anexar  
 El método **Append** se puede usar para agregar texto o la representación de cadena de un objeto al final de una cadena representada por el objeto **StringBuilder** actual. En el ejemplo siguiente, se inicializa **StringBuilder** en "Hello World" y, a continuación, se anexa texto al final del objeto. El espacio se asigna automáticamente según sea necesario.  
...
## Convertir un objeto StringBuilder en String  
 Para poder pasar la cadena representada por el objeto <xref:System.Text.StringBuilder> a un método con un parámetro <xref:System.String> o mostrarla en la interfaz de usuario, antes debe convertir el objeto <xref:System.Text.StringBuilder> en un objeto <xref:System.String>. Para hacer esta conversión, llame al método <xref:System.Text.StringBuilder.ToString%2A?displayProperty=fullName>. En el ejemplo siguiente se llama a varios métodos de <xref:System.Text.StringBuilder> y después se llama al método <xref:System.Text.StringBuilder.ToString?displayProperty=fullName> para mostrar la cadena.  
