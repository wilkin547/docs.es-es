---
title: "Metadatos y componentes autodescriptivos | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "Common Language Runtime, metadatos"
  - "componentes [.NET Framework], metadatos"
  - "lenguajes, interoperabilidad"
  - "metadatos, acerca de metadatos"
  - "PE (archivos), metadatos"
  - "archivos ejecutables portables, metadatos"
  - "tiempo de ejecución, metadatos"
...
```  
 ...
 El byte superior \(`0x06`\) indica que éste es un token de **MethodDef**.  Los tres bytes inferiores \(`000004`\) indican a Common Language Runtime que busque en la cuarta fila de la tabla **MethodDef** la información que describe la definición de este método.  
...
|----------------------------|--------------------------------------------|  
|Encabezado del archivo PE|El índice de las secciones principales del archivo PE y la dirección del punto de entrada.<br /><br /> El motor en tiempo de ejecución usa esta información para identificar el archivo como archivo PE y para determinar dónde comienza la ejecución al cargar el programa en la memoria.|  
...
```  
 ...
 El compilador JIT lee el MSIL de todo el método, lo analiza exhaustivamente y genera instrucciones nativas efectivas para ese método.  En `IL_000d` se encuentra un token de metadatos del método `Add` \(`/*` `06000003 */`\), y el motor en tiempo de ejecución usa dicho token para consultar la tercera fila de la tabla **MethodDef**.  
