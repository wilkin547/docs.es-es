...
## <a name="avoid-calls-to-non-thread-safe-methods"></a>Evitar llamadas a métodos que no son seguros para subprocesos  
 La escritura en métodos de instancia que no son seguros para subprocesos de un bucle en paralelo puede producir daños en los datos, que pueden pasar o no inadvertidos para el programa. También puede dar lugar a excepciones. En el ejemplo siguiente, varios subprocesos estarían intentando llamar a la <xref:System.IO.FileStream.WriteByte%2A?displayProperty=nameWithType> método simultáneamente, lo que no es compatible con la clase.  
...
## <a name="avoid-executing-parallel-loops-on-the-ui-thread"></a>Evitar la ejecución de bucles en paralelo en el subproceso de la interfaz de usuario  
 ...
 Si ejecuta bucles en paralelo en el subproceso de la interfaz de usuario, tenga cuidado de evitar la actualización de los controles de la interfaz de usuario desde el interior del bucle. Si se intenta actualizar los controles de la interfaz de usuario desde dentro de un bucle en paralelo que se está ejecutando en el subproceso de la interfaz de usuario, se pueden provocar daños en el estado, excepciones, retrasos en las actualizaciones e incluso interbloqueos, dependiendo de cómo se invoque la actualización de la interfaz de usuario. En el siguiente ejemplo, el bucle en paralelo bloquea el subproceso de la interfaz de usuario en el que se está ejecutando hasta que todas las iteraciones se completan. Sin embargo, si una iteración del bucle está ejecutando en un subproceso en segundo plano (como <xref:System.Threading.Tasks.Parallel.For%2A> pueden hacer), la llamada a Invoke hace que un mensaje que se envíen al subproceso de interfaz de usuario y se bloquea en espera para que procesar ese mensaje. Puesto que se bloquea el subproceso de interfaz de usuario ejecutando el <xref:System.Threading.Tasks.Parallel.For%2A>, nunca se puede procesar el mensaje y los interbloqueos de subproceso de interfaz de usuario.  
