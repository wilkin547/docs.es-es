---
title: "Validaci&#243;n de esquemas con XPathNavigator | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
```  
 ...
 Se producirá una <xref:System.Xml.Schema.XmlSchemaException> siempre que el <xref:System.Xml.XPath.XPathNavigator> obtenga acceso a un atributo o elemento con <xref:System.Xml.XPath.XPathNavigator.TypedValue%2A> establecido en `invalid`.  
...
### Validación de un documento con el método Validate  
 El método <xref:System.Xml.XmlDocument.Validate%2A> de la clase <xref:System.Xml.XmlDocument> valida el documento XML contenido en un objeto <xref:System.Xml.XmlDocument> con los esquemas que se especifican en la propiedad <xref:System.Xml.XmlDocument.Schemas%2A> del objeto <xref:System.Xml.XmlDocument> y aumenta el conjunto de información.  El resultado es que se reemplaza un documento XML que previamente no tiene información de tipos del objeto <xref:System.Xml.XmlDocument> por un documento con información de tipos.  
 ...
 En el siguiente ejemplo se valida el archivo `contosoBooks.xml` contenido en el objeto <xref:System.Xml.XmlDocument> con el esquema `contosoBooks.xsd` contenido en la propiedad <xref:System.Xml.XmlDocument.Schemas%2A> del objeto <xref:System.Xml.XmlDocument>.  
...
### Validación de solo lectura  
 ...
 Para obtener más información sobre los objetos <xref:System.Xml.XPath.XPathNavigator> editables y de solo lectura, vea el tema [Lectura de datos XML con XPathDocument y XmlDocument](../../../../docs/standard/data/xml/reading-xml-data-using-xpathdocument-and-xmldocument.md).  
