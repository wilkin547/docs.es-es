---
title: "Covarianza y contravarianza en gen&#233;ricos | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
helpviewer_keywords: 
  ...
  - "genéricos, covarianza y contravarianza"
  - "genéricos, varianza"
...
-   `Contravariance`  
     ...
     Puede asignar una instancia de `IEnumerable<Base>` \(`IEnumerable(Of Base)` en Visual Basic\) a una variable de tipo `IEnumerable<Derived>`.  
...
-   `Invariance`  
     ...
     No se puede asignar una instancia de `IEnumerable<Base>` \(`IEnumerable(Of Base)` en Visual Basic\) a una variable de tipo `IEnumerable<Derived>` o viceversa.  
  
 Los parámetros de tipo covariante permiten realizar asignaciones muy similares a [Polimorfismo](../Topic/Polymorphism%20\(C%23%20Programming%20Guide\).md), como se muestra en el código siguiente.  
...
>  Si un método de una interfaz tiene un parámetro que es un tipo de delegado genérico, se puede usar un parámetro de tipo covariante del tipo de interfaz para especificar un parámetro de tipo contravariante del tipo de delegado.  
 ...
 Para obtener información y código de ejemplo, vea [Varianza en interfaces genéricas](../Topic/Variance%20in%20Generic%20Interfaces%20\(C%23%20and%20Visual%20Basic\).md).  
