...
Esta sección contiene instrucciones para usar tipos comunes en las API de acceso público. Trata sobre el uso directo de tipos de marco integrados (por ejemplo, atributos de serialización) y la sobrecarga de operadores comunes.
  
La interfaz de <xref:System.IDisposable?displayProperty=nameWithType> no se trata en esta sección, pero se describe en la sección [Patrón de eliminación](../garbage-collection/implementing-dispose.md).
