...
2. Inicializar un proyecto de C#:
    * Abra el Terminal integrado de Visual Studio Code escribiendo <kbd>CTRL</kbd>+<kbd>\`</kbd> (comilla simple). De manera alternativa, puede seleccionar **Ver** > **Integrated Terminal** (Terminal integrado) en el menú principal.
...
3. Resolver los recursos de compilación:
    ...
    * En **.NET Core 1.1**, escriba `dotnet restore`. Al ejecutar `dotnet restore`, se concede acceso a los paquetes de .NET Core necesarios para compilar el proyecto.

   ![El comando de restauración de dotnet](media/with-visual-studio-code/dotnetrestore.png)

    * Este paso es opcional en **.NET Core 2.0**. El comando `dotnet restore` se ejecuta automáticamente cuando se crea un proyecto nuevo.
