---
title: "Arquitectura de las herramientas de la línea de comandos de .NET Core Preview 3"
description: Preview 3 plantea ciertos cambios en la forma en que las herramientas generales de .NET Core se superponen.
...
<a name="high-level-overview-of-changes-in-cli-preview-3"></a>Introducción de alto nivel de cambios en la CLI Preview 3
-----------------------------------------------
...
> **Nota:** Este artículo **no es necesario** para usar las herramientas de la línea de comandos de .NET Preview 3. Puede seguir usando las herramientas que suele utilizar. Este artículo viene a completar la imagen de cómo el paso a MSBuild cambia "la disposición en capas" y la arquitectura de las herramientas de línea de comandos. 

# <a name="moving-away-from-projectjson"></a>Abandono de project.json
El cambio más importante en las herramientas de Preview 3 para .NET Core es ciertamente el [abandono de project.json en favor de csproj](https://blogs.msdn.microsoft.com/dotnet/2016/05/23/changes-to-project-json/) como sistema de proyectos. La versión Preview 3 de las herramientas de línea de comandos es la primera versión de las herramientas de línea de comandos de .NET Core que no contiene ninguna compatibilidad con project.json. Esto significa que no puede utilizarse para compilar, ejecutar o publicar bibliotecas y aplicaciones basadas en project.json. Para poder utilizar esta versión de las herramientas, debe migrar los proyectos existentes o iniciar otros nuevos. 
...
> **Nota:** Si está familiarizado con MSBuild y quiere aprender más al respecto, puede leer para comenzar la [documentación existente](https://msdn.microsoft.com/en-us/library/dd637714.aspx). 
...
![Arquitectura de alto nivel de herramientas de Preview 3](media/p3-arch.png)
...
> **Nota:** Un "destino" es un término de MSBuild que indica una operación con nombre que puede invocar MSBuild. Normalmente está unido a una o varias tareas que ejecutan alguna lógica que se supone que debe hacer el destino. MSBuild admite michos destinos predefinidos, como `Copy` o `Execute`; también permite a los usuarios escribir sus propias tareas mediante código administrado y definir destinos para ejecutar esas tareas. Puede leer más acerca de las tareas de MSBuild en [MSDN](https://msdn.microsoft.com/en-us/library/ms171466.aspx). 
...
## <a name="preview-3-cli-commands"></a>Comandos de la CLI de Preview 3
...
Estos comandos todavía hacen lo que se espera que hagan (crear un nuevo proyecto, compilarlo, publicarlo, empaquetarlo, etc.). La mayoría de las opciones no varían, y siguen ahí, y puede consultar las pantallas de ayuda de los comandos (mediante `dotent <command> --help`) o la documentación de Preview 3 de este sitio para familiarizarse con los cambios. 
...
Este documento es una descripción de alto nivel de los cambios que se están produciendo en la arquitectura y el funcionamiento de las herramientas de la CLI a nivel global que se incluyen con Preview 3. Se ha presentado la noción de componente de SDK compartido; también se ha explicado cómo funcionan los comandos de la CLI, desde una perspectiva técnica, en Preview 3. 
