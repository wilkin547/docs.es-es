...
2. Después de ejecutar el comando de configuración, se le pedirá que escriba un host. La dirección URL del host usa el formato: **https://<\Ubicación>.azuredatabricks.net**. Por ejemplo, si ha seleccionado **eastus2** durante la creación del Servicio de Azure Databricks, el host sería **https://eastus2.azuredatabricks.net** .
...
2. Realice las tareas siguientes para comprimir los archivos de la aplicación publicados para que pueda cargarlos fácilmente en el clúster de Spark de Databricks.
   ...
   **En Windows:**
   ...
   Vaya a mySparkApp/bin/Release/netcoreapp3.0/ubuntu.16.04-x64. Luego, haga clic con el botón derecho en la carpeta **Publicar** y seleccione **Enviar a > Carpeta comprimida (en zip)** . Asigne el nombre **publish.zip** a la carpeta nueva.
...
4. Seleccione **Registros de controladores** para ver la salida del trabajo. Cuando la aplicación termine de ejecutarse, verá la misma tabla de recuento de palabras de la ejecución local de introducción escrita en la consola de salida estándar.
   ...
   Enhorabuena, ha ejecutado su primera aplicación de .NET para Apache Spark en la nube.
